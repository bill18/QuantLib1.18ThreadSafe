...
void HestonSLVModelTest::testBlackScholesFokkerPlanckFwdEquation() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, q, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, r, dc));
    const Handle<BlackVolTermStructure> vTS(flatVol(settings, v, dc));

    const ext::shared_ptr<GeneralizedBlackScholesProcess> process(
		ext::make_shared<GeneralizedBlackScholesProcess>(settings, spot, qTS, rTS, vTS));

    const ext::shared_ptr<PricingEngine> engine(
		ext::make_shared<AnalyticEuropeanEngine>(settings, process));

    const ext::shared_ptr<FdmMesher> uniformMesher(
		ext::make_shared<FdmMesherComposite>(
			ext::make_shared<FdmBlackScholesMesher>(settings, xGrid, process, maturity, s0)));
...
namespace {
    ...
    Real hestonPxBoundary(
        ...
        const AnalyticPDFHestonEngine pdfEngine(settings, model);
    ...
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
        ...
        const ext::shared_ptr<HestonProcess> process(
			ext::make_shared<HestonProcess>(settings, rTS, qTS, spot, v0, kappa, theta, sigma, rho));

        const ext::shared_ptr<HestonModel> model(ext::make_shared<HestonModel>(settings, process));

        const ext::shared_ptr<PricingEngine> engine(
			ext::make_shared<AnalyticHestonEngine>(settings, model));
        ...
        const Real sLowerBound
            = std::log(hestonPxBoundary(settings, maturity, sEps, model));
        const Real sUpperBound
            = std::log(hestonPxBoundary(settings, maturity, 1-sEps,model));
...
namespace {
    ext::shared_ptr<Matrix> createLocalVolMatrixFromProcess(
        ...
        const Date todaysDate = settings->evaluationDate();
    ...
    boost::tuple<std::vector<Real>, std::vector<Date>,
        ...
        createSmoothImpliedVol(Settings* settings, const DayCounter& dc, const Calendar& cal) {
        ...
        const ext::shared_ptr<BlackVarianceSurface> volTS(
			ext::make_shared<BlackVarianceSurface>(settings, todaysDate, cal,
                                     dates,
...
void HestonSLVModelTest::testHestonFokkerPlanckFwdEquationLogLVLeverage() {
    ...
    const Handle<Quote> spot(ext::make_shared<SimpleQuote>(s0));
    const Handle<YieldTermStructure> rTS(flatRate(settings, todaysDate, r, dayCounter));
    const Handle<YieldTermStructure> qTS(flatRate(settings, todaysDate, q, dayCounter));
    ...
    const boost::tuple<std::vector<Real>, std::vector<Date>,
        ...
        createSmoothImpliedVol(settings, dayCounter, calendar);
    const ext::shared_ptr<BlackScholesMertonProcess> lvProcess(
		ext::make_shared<BlackScholesMertonProcess>(settings, spot, qTS, rTS,
    ...
    ext::shared_ptr<Matrix> m = createLocalVolMatrixFromProcess(settings, 
        lvProcess, ds, dates, times);

    const ext::shared_ptr<FixedLocalVolSurface> leverage(
		ext::make_shared<FixedLocalVolSurface>(settings, todaysDate, dates, ds, m, dc));

    const ext::shared_ptr<PricingEngine> lvEngine(
		ext::make_shared<AnalyticEuropeanEngine>(settings, lvProcess));
    ...
    const ext::shared_ptr<PricingEngine> fdmEngine(
		ext::make_shared<FdBlackScholesVanillaEngine>(settings, lvProcess, 50, 201, 0,
...
void HestonSLVModelTest::testBlackScholesFokkerPlanckFwdEquationLocalVol() {
    ...
    const Handle<YieldTermStructure> rTS(
            flatRate(settings, todaysDate, r, dayCounter));
    const Handle<YieldTermStructure> qTS(
            flatRate(settings, todaysDate, q, dayCounter));
    ...
    const Handle<BlackVolTermStructure> vTS = Handle<BlackVolTermStructure>(
            createSmoothImpliedVol(settings, dayCounter, calendar).get<2>());
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> process(
		ext::make_shared<BlackScholesMertonProcess>(settings, spot, qTS, rTS, vTS));

    const ext::shared_ptr<LocalVolTermStructure> localVol(
        ext::make_shared<NoExceptLocalVolSurface>(settings, vTS, rTS, qTS, spot, 0.2));
    ...
    for (Size i = 1; i < dates.size(); i += 2) {
        for (Size j = 3; j < strikes.size() - 3; j += 2) {
            ...
            const ext::shared_ptr<FdmMesher> uniformMesher(
				ext::make_shared<FdmMesherComposite>(
					ext::make_shared<FdmBlackScholesMesher>(settings, xGrid, process,
                            maturity, s0)));
...
namespace {
    ...
    void lsvCalibrationTest(Settings* settings,const HestonSLVTestCase& testCase) {
        ...
        const Handle<LocalVolTermStructure> localVol(
            ext::make_shared<LocalConstantVol>(settings, todaysDate, lv, dc));

        const HestonSLVFDMModel slvModel(settings, 
        ...
        const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
			ext::make_shared<GeneralizedBlackScholesProcess>(settings, spot, qTS, rTS,
                Handle<BlackVolTermStructure>(flatVol(settings, lv, dc))));

        const ext::shared_ptr<PricingEngine> analyticEngine(
			ext::make_shared<AnalyticEuropeanEngine>(settings, bsProcess));
        ...
        for (Size t=0; t < LENGTH(times); ++t) {
            ...
            const ext::shared_ptr<PricingEngine> slvEngine(
                (times[t] <= 3) ?
				ext::make_shared<FdHestonVanillaEngine>(settings, hestonModel.currentLink(),
            ...
            for (Size s=0; s < LENGTH(strikes); ++s) {
                ...
                const ext::shared_ptr<GeneralizedBlackScholesProcess> bp(
					ext::make_shared<GeneralizedBlackScholesProcess>(settings, spot, qTS, rTS,
                        Handle<BlackVolTermStructure>(flatVol(settings, lv,
                                                      dc))));
...
void HestonSLVModelTest::testFDMCalibration() {
    ...
    for (Size i=0; i < LENGTH(testCases); ++i) {
        ...
        lsvCalibrationTest(settings, testCases[i]);
    }
...
void HestonSLVModelTest::testLocalVolsvSLVPropDensity() {
    ...
    const Handle<LocalVolTermStructure> localVol(
        ext::make_shared<NoExceptLocalVolSurface>(settings, vTS, rTS, qTS, spot, 0.3));
...
void HestonSLVModelTest::testBarrierPricingViaHestonLocalVol() {
    ...
    const Handle<BlackVolTermStructure> surf(
        ext::make_shared<HestonBlackVolSurface>(settings, hestonModel));
    ...
    const ext::shared_ptr<LocalVolSurface> localVolSurface(
		ext::make_shared<LocalVolSurface>(settings, surf, rTS, qTS, spot));

    const ext::shared_ptr<PricingEngine> hestonEngine(
		ext::make_shared<AnalyticHestonEngine>(settings, hestonModel.currentLink(), 164));
        ...
        for (Size j=0; j < LENGTH(maturities); ++j) {
            ...
            const ext::shared_ptr<GeneralizedBlackScholesProcess>
                bsProcess(ext::make_shared<GeneralizedBlackScholesProcess>(
                    settings, 
                    spot, qTS, rTS,
                    Handle<BlackVolTermStructure>(flatVol(settings, impliedVol, dc))));
            ...
            const ext::shared_ptr<PricingEngine> localVolEngine(
				ext::make_shared<FdBlackScholesVanillaEngine>(settings, bsProcess, 201, 801, 0,
...
void HestonSLVModelTest::testBarrierPricingMixedModels() {
    ...
    const Handle<LocalVolTermStructure> localVolSurf(
        ext::make_shared<NoExceptLocalVolSurface>(settings, 
            impliedVolSurf, rTS, qTS, spot, 0.3));

    const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
        ext::make_shared<GeneralizedBlackScholesProcess>(settings, 
...
void HestonSLVModelTest::testMonteCarloVsFdmPricing() {
    ...
    const ext::shared_ptr<LocalVolTermStructure> leverageFct
        = ext::make_shared<LocalConstantVol>(settings, todaysDate, 0.25, dc);
    ...
    const ext::shared_ptr<PricingEngine> mcEngine
        = MakeMCEuropeanHestonEngine<
            PseudoRandom, GeneralStatistics, HestonSLVProcess>(settings, slvProcess)
...
void HestonSLVModelTest::testMonteCarloCalibration() {
    ...
    const ext::shared_ptr<LocalVolTermStructure> localVol
          = ext::make_shared<LocalConstantVol>(settings, todaysDate, 0.3, dc);
    ...
    for (Size m=0; m < LENGTH(nSims); ++m) {
        ...
        const ext::shared_ptr<PricingEngine> bsEngine(
            ext::make_shared<AnalyticEuropeanEngine>(settings, 
                ext::make_shared<GeneralizedBlackScholesProcess>(
                    settings, 
                    spot, qTS, rTS,
                    Handle<BlackVolTermStructure>(flatVol(settings, 0.3, dc)))));
...
void HestonSLVModelTest::testForwardSkewSLV() {
    ...
    const Handle<LocalVolTermStructure> localVol(
        ext::make_shared<LocalConstantVol>(settings, todaysDate, flatLocalVol, dc));
    ...
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, todaysDate, vol, dc));

    const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
		ext::make_shared<GeneralizedBlackScholesProcess>(settings, spot, qTS, rTS, volTS));

    const ext::shared_ptr<PricingEngine> fwdEngine(
        ext::make_shared<ForwardVanillaEngine<AnalyticEuropeanEngine> >(settings, 
            bsProcess));
    ...
    for (Size j=0; j < LENGTH(strikes); ++j) {
        for (Size k= 0; k < 2; ++k) {
            ...
            const ext::shared_ptr<ForwardVanillaOption> fwdOption(
                ext::make_shared<ForwardVanillaOption>(settings, 
...
namespace {
    ext::shared_ptr<LocalVolTermStructure> getFixedLocalVolFromHeston(
        ...
        const Handle<BlackVolTermStructure> trueImpliedVolSurf(
            ext::make_shared<HestonBlackVolSurface>(settings, 
                Handle<HestonModel>(hestonModel),
        ...
        return ext::make_shared<FixedLocalVolSurface>(settings, 
...
void HestonSLVModelTest::testMoustacheGraph() {
    ...
    */
    ...
    VanillaOption vanillaOption(settings, 
    ...
    const Volatility implVol = vanillaOption.impliedVolatility(
        vanillaOption.NPV(),
        ext::make_shared<GeneralizedBlackScholesProcess>(settings, spot, qTS, rTS,
            Handle<BlackVolTermStructure>(flatVol(settings, std::sqrt(theta), dc))));

    const ext::shared_ptr<PricingEngine> analyticEngine(
        ext::make_shared<AnalyticDoubleBarrierBinaryEngine>(settings, 
            ext::make_shared<GeneralizedBlackScholesProcess>(settings, 
                spot, qTS, rTS,
                Handle<BlackVolTermStructure>(flatVol(settings, implVol, dc)))));
    ...
    const Handle<LocalVolTermStructure> localVol(
        getFixedLocalVolFromHeston(settings, hestonModel, timeGrid));
    ...
    const ext::shared_ptr<LocalVolTermStructure> leverageFct =
        HestonSLVMCModel(settings, localVol, modHestonModel, sobolGeneratorFactory,
                         maturityDate, 182, xGrid, nSim)
            .leverageFunction();

    const ext::shared_ptr<PricingEngine> fdEngine( 
        ext::make_shared<FdHestonDoubleBarrierEngine>(settings, 
    ...
    for (Size i=0; i < 18; ++i) {
        ...
        DoubleBarrierOption doubleBarrier(settings, 
