...
void BasketOptionTest::testEuroTwoValues() {
    ...
    */
    ...
    ext::shared_ptr<YieldTermStructure> qTS1 = flatRate(settings, today, qRate1, dc);
    ext::shared_ptr<SimpleQuote> qRate2(new SimpleQuote(0.0));
    ext::shared_ptr<YieldTermStructure> qTS2 = flatRate(settings, today, qRate2, dc);
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS1 = flatVol(settings, today, vol1, dc);
    ext::shared_ptr<SimpleQuote> vol2(new SimpleQuote(0.0));
    ext::shared_ptr<BlackVolTermStructure> volTS2 = flatVol(settings, today, vol2, dc);
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        switch(values[i].basketType) {
          ...
          case MinBasket:
            ...
            analyticEngine=ext::shared_ptr<PricingEngine>(
                new StulzEngine(settings,p1, p2, values[i].rho));
            break;
          case SpreadBasket:
              p1 = ext::shared_ptr<GeneralizedBlackScholesProcess>(
                  new BlackProcess(settings,Handle<Quote>(spot1),
                                   Handle<YieldTermStructure>(rTS),
                                   Handle<BlackVolTermStructure>(volTS1)));
              p2 = ext::shared_ptr<GeneralizedBlackScholesProcess>(
                  new BlackProcess(settings,Handle<Quote>(spot2),
                                   Handle<YieldTermStructure>(rTS),
                                   Handle<BlackVolTermStructure>(volTS2)));
              
              analyticEngine=ext::shared_ptr<PricingEngine>(
                  new KirkEngine(settings,ext::dynamic_pointer_cast<BlackProcess>(p1), 
        ...
        ext::shared_ptr<PricingEngine> mcEngine =
            MakeMCEuropeanBasketEngine<PseudoRandom, Statistics>(settings, process)
            .withStepsPerYear(1)
            .withSamples(10000)
            .withSeed(42);

        ext::shared_ptr<PricingEngine> fdEngine(
                    new Fd2dBlackScholesVanillaEngine(settings, p1, p2, values[i].rho,
                                                      50, 50, 15));
        
        BasketOption basketOption(settings, basketTypeToPayoff(values[i].basketType,
                                                     payoff),
                                  exercise);
...
*/
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS3 = flatVol(settings, today, vol3, dc);
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<StochasticProcess1D> stochProcess1(new
            BlackScholesMertonProcess(settings, Handle<Quote>(spot1),
                                      Handle<YieldTermStructure>(qTS),
                                      Handle<YieldTermStructure>(rTS),
                                      Handle<BlackVolTermStructure>(volTS1)));

        ext::shared_ptr<StochasticProcess1D> stochProcess2(new
            BlackScholesMertonProcess(settings, Handle<Quote>(spot2),
                                      Handle<YieldTermStructure>(qTS),
                                      Handle<YieldTermStructure>(rTS),
                                      Handle<BlackVolTermStructure>(volTS2)));

        ext::shared_ptr<StochasticProcess1D> stochProcess3(new
            BlackScholesMertonProcess(settings, Handle<Quote>(spot3),
                                      Handle<YieldTermStructure>(qTS),
                                      Handle<YieldTermStructure>(rTS),
        ...
        ext::shared_ptr<PricingEngine> mcQuasiEngine =
            MakeMCEuropeanBasketEngine<LowDiscrepancy>(settings, process)
            .withStepsPerYear(1)
            .withSamples(8091)
            .withSeed(42);

        BasketOption euroBasketOption(settings, basketTypeToPayoff(values[i].basketType,
                                                         payoff),
                                      exercise);
        ...
        ext::shared_ptr<PricingEngine> mcLSMCEngine =
            MakeMCAmericanBasketEngine<>(settings, process)
            .withSteps(timeSteps)
        ...
        BasketOption amBasketOption(settings, basketTypeToPayoff(values[i].basketType,
                                                       payoff),
                                    amExercise);
...
void BasketOptionTest::testTavellaValues() {
    ...
    */
    ...
    BasketOption basketOption(settings, basketTypeToPayoff(values[0].basketType,
                                                 payoff),
                              exercise);
...
void BasketOptionTest::testLocalVolatilitySpreadOption() {
    ...
    const Handle<YieldTermStructure> riskFreeRate(flatRate(settings, today, 0.07, dc));
    const Handle<YieldTermStructure> dividendYield(flatRate(settings, today, 0.03, dc));
    ...
    const Handle<BlackVolTermStructure> vol1(
        ext::make_shared<HestonBlackVolSurface>(settings, Handle<HestonModel>(hm1)));

    const Handle<BlackVolTermStructure> vol2(
        ext::make_shared<HestonBlackVolSurface>(settings, Handle<HestonModel>(hm2)));
    ...
    basketOption.setPricingEngine(
        ext::shared_ptr<Fd2dBlackScholesVanillaEngine>(
            new Fd2dBlackScholesVanillaEngine(settings, 
...
void BasketOptionTest::test2DPDEGreeks() {
    ...
    const Handle<BlackVolTermStructure> vTS(flatVol(settings, today, v, dc));

    const ext::shared_ptr<BlackProcess> p1(
        ext::make_shared<BlackProcess>(settings, Handle<Quote>(spot1), rTS, vTS));

    const ext::shared_ptr<BlackProcess> p2(
        ext::make_shared<BlackProcess>(settings, Handle<Quote>(spot2), rTS, vTS));

    BasketOption option(settings, 
        ext::make_shared<SpreadBasketPayoff>(
            ext::make_shared<PlainVanillaPayoff>(Option::Call, strike)),
        ext::make_shared<EuropeanExercise>(maturity));

    option.setPricingEngine(
        ext::make_shared<Fd2dBlackScholesVanillaEngine>(settings, p1, p2, rho));

    const Real calculatedDelta = option.delta();
    const Real calculatedGamma = option.gamma();

    option.setPricingEngine(ext::make_shared<KirkEngine>(settings, p1, p2, rho));
