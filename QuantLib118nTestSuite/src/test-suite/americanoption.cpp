    ...
    */
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                      new BaroneAdesiWhaleyApproximationEngine(settings, stochProcess));
...
void AmericanOptionTest::testBjerksundStenslandValues() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                     new BjerksundStenslandApproximationEngine(settings, stochProcess));
...
void AmericanOptionTest::testJuValues() {
    ...
    for (Size i=0; i<LENGTH(juValues); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                            new JuQuadraticApproximationEngine(settings, stochProcess));
...
void AmericanOptionTest::testFdValues() {
    ...
    for (Size i=0; i<LENGTH(juValues); i++) {
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess =
            ext::make_shared<BlackScholesMertonProcess>(settings, Handle<Quote>(spot),
                                                        Handle<YieldTermStructure>(qTS),
                                                        Handle<YieldTermStructure>(rTS),
                                                        Handle<BlackVolTermStructure>(volTS));

        ext::shared_ptr<PricingEngine> engine =
            ext::make_shared<FdBlackScholesVanillaEngine>(settings, stochProcess, 100, 100);
...
namespace {
    ...
    void testFdGreeks(Settings* settings) {
        ...
        for (Size i=0; i<LENGTH(types); i++) {
          for (Size j=0; j<LENGTH(strikes); j++) {
            for (Size k=0; k<LENGTH(years); k++) {
                ...
                ext::shared_ptr<PricingEngine> engine(
                                                    new Engine(settings, stochProcess));
