...
namespace inflation_capfloored_coupon_test {
    ...
    std::vector<ext::shared_ptr<BootstrapHelper<T> > > makeHelpers(
        ...
        for (Size i=0; i<N; i++) {
            ...
            ext::shared_ptr<BootstrapHelper<T> > anInstrument(new U(settings,
    ...
    struct CommonVars {
        ...
        : nominals(1,1000000) {
            ...
            Schedule rpiSchedule = MakeSchedule(settings->evaluationDate()).from(from).to(to)
            .withTenor(1*Months)
            ...
            bool interp = false;
            iir = ext::make_shared<YYUKRPIr>(settings, interp, hy);
            for (Size i=0; i<rpiSchedule.size();i++) {
                iir->addFixing(rpiSchedule[i], fixData[i]);
            }

            ext::shared_ptr<YieldTermStructure> nominalFF(
                        new FlatForward(settings, evaluationDate, 0.05, ActualActual()));
            ...
            YoYInflationIndex>(settings, yyData, LENGTH(yyData), iir,
                               observationLag,
        ...
        Leg makeYoYLeg(const Date& startDate, Integer length,
            ...
            Schedule schedule(settings->evaluationDate(), startDate, endDate, Period(frequency), calendar,
            ...
            return yoyInflationLeg(settings, schedule, calendar, ii, observationLag)
        ...
        Leg makeFixedLeg(const Date& startDate,
            ...
            return FixedRateLeg(settings, schedule)
            .withNotionals(nominals)
        ...
        Leg makeYoYCapFlooredLeg(Size which, const Date& startDate,
            ...
            vol(ext::make_shared<ConstantYoYOptionletVolatility>(settings, 
                            volatility,
                                settlementDays,
                                calendar,
                                convention,
            ...
            switch (which) {
                case 0:
                    pricer = ext::shared_ptr<YoYInflationCouponPricer>(
                            new BlackYoYInflationCouponPricer(settings, vol, nominalTS));
                    break;
                case 1:
                    pricer = ext::shared_ptr<YoYInflationCouponPricer>(
                            new UnitDisplacedBlackYoYInflationCouponPricer(settings, vol, nominalTS));
                    break;
                case 2:
                    pricer = ext::shared_ptr<YoYInflationCouponPricer>(
                            new BachelierYoYInflationCouponPricer(settings, vol, nominalTS));
                    break;
                default:
            ...
            Leg yoyLeg =  yoyInflationLeg(settings, schedule, calendar, ii, observationLag)
            .withNotionals(nominals)
            .withPaymentDayCounter(dc)
            .withPaymentAdjustment(convention)
            .withGearings(gearingVector)
            ...
            switch (which) {
                case 0:
                    return ext::shared_ptr<PricingEngine>(
                            new YoYInflationBlackCapFloorEngine(settings, iir, vol, nominalTS));
                    break;
                case 1:
                    return ext::shared_ptr<PricingEngine>(
                            new YoYInflationUnitDisplacedBlackCapFloorEngine(settings, iir, vol, nominalTS));
                    break;
                case 2:
                    return ext::shared_ptr<PricingEngine>(
                            new YoYInflationBachelierCapFloorEngine(settings, iir, vol, nominalTS));
                    break;
                default:
            ...
            switch (type) {
                case YoYInflationCapFloor::Cap:
                    result = ext::shared_ptr<YoYInflationCapFloor>(
                                new YoYInflationCap(settings, leg, std::vector<Rate>(1, strike)));
                    break;
                case YoYInflationCapFloor::Floor:
                    result = ext::shared_ptr<YoYInflationCapFloor>(
                                new YoYInflationFloor(settings, leg, std::vector<Rate>(1, strike)));
                    break;
                default:
...
void InflationCapFlooredCouponTest::testDecomposition() {
    ...
    Swap vanillaLeg(vars.settings, fixedLeg,floatLeg);
    // Swap with null fixed leg and floating leg with positive gearing and spread<>0
    Swap vanillaLeg_p(vars.settings, fixedLeg,floatLeg_p);
    // Swap with null fixed leg and floating leg with negative gearing and spread<>0
    Swap vanillaLeg_n(vars.settings, fixedLeg,floatLeg_n);

    ext::shared_ptr<PricingEngine> engine(
            new DiscountingSwapEngine(vars.settings, vars.nominalTS));
    ...
    Swap capLeg(vars.settings, fixedLeg,cappedLeg);
    capLeg.setPricingEngine(engine);
    YoYInflationCap cap(vars.settings, floatLeg, std::vector<Rate>(1, capstrike));
    ...
    Swap floorLeg(vars.settings, fixedLeg,flooredLeg);
    floorLeg.setPricingEngine(engine);
    YoYInflationFloor floor(vars.settings, floatLeg, std::vector<Rate>(1, floorstrike));
    ...
    Swap collarLeg(vars.settings, fixedLeg,collaredLeg);
    collarLeg.setPricingEngine(engine);
    YoYInflationCollar collar(vars.settings, floatLeg,
    ...
    Swap capLeg_p(vars.settings, fixedLeg,cappedLeg_p);
    capLeg_p.setPricingEngine(engine);
    YoYInflationCap cap_p(vars.settings, floatLeg_p,std::vector<Rate>(1,capstrike));
    ...
    Swap capLeg_n(vars.settings, fixedLeg,cappedLeg_n);
    capLeg_n.setPricingEngine(engine);
    YoYInflationFloor floor_n(vars.settings, floatLeg,std::vector<Rate>(1,(capstrike-spread_n)/gearing_n));
    ...
    vars.makeYoYCapFlooredLeg(whichPricer,vars.startDate,vars.length,caps0,floors,
                           vars.volatility,gearing_p,spread_p);
    Swap floorLeg_p1(vars.settings, fixedLeg,flooredLeg_p1);
    floorLeg_p1.setPricingEngine(engine);
    YoYInflationFloor floor_p1(vars.settings, floatLeg_p,std::vector<Rate>(1,floorstrike));
    ...
    vars.makeYoYCapFlooredLeg(whichPricer,vars.startDate,vars.length,caps0,floors,
                           vars.volatility,gearing_n,spread_n);
    Swap floorLeg_n(vars.settings, fixedLeg,flooredLeg_n);
    floorLeg_n.setPricingEngine(engine);
    YoYInflationCap cap_n(vars.settings, floatLeg,std::vector<Rate>(1,(floorstrike-spread_n)/gearing_n));
    ...
    vars.makeYoYCapFlooredLeg(whichPricer,vars.startDate,vars.length,caps,floors,
                           vars.volatility,gearing_p,spread_p);
    Swap collarLeg_p1(vars.settings, fixedLeg,collaredLeg_p);
    collarLeg_p1.setPricingEngine(engine);
    YoYInflationCollar collar_p(vars.settings, floatLeg_p,
                    std::vector<Rate>(1,capstrike),
                    std::vector<Rate>(1,floorstrike));
    ...
    vars.makeYoYCapFlooredLeg(whichPricer,vars.startDate,vars.length,caps,floors,
                           vars.volatility,gearing_n,spread_n);
    Swap collarLeg_n1(vars.settings, fixedLeg,collaredLeg_n);
    collarLeg_n1.setPricingEngine(engine);
    YoYInflationCollar collar_n(vars.settings, floatLeg,
        ...
        for (Size i=0; i<LENGTH(lengths); i++) {
            for (Size j=0; j<LENGTH(strikes); j++) {
                for (Size k=0; k<LENGTH(vols); k++) {
                    ...
                    Schedule yoySchedule = MakeSchedule(vars.settings->evaluationDate()).from(from).to(to)
                    .withTenor(1*Years)
                    .withCalendar(UnitedKingdom())
                    ...
                    YearOnYearInflationSwap swap(vars.settings, YearOnYearInflationSwap::Payer,
                                                    1000000.0,
                    ...
                    ext::shared_ptr<PricingEngine> sppe(new DiscountingSwapEngine(vars.settings, hTS));
                    ...
                    Real capped = CashFlows::npv(vars.settings, leg2,(**vars.nominalTS),false);
                    ...
                    // N.B. nominals are 10e6
                    Real floored = CashFlows::npv(vars.settings, leg3,(**vars.nominalTS),false);
