...
void DefaultProbabilityCurveTest::testDefaultProbability() {
    ...
    FlatHazardRate flatHazardRate(settings, startDate, hazardRateQuote, dayCounter);
...
void DefaultProbabilityCurveTest::testFlatHazardRate() {
    ...
    FlatHazardRate flatHazardRate(settings, today, hazardRateQuote, dayCounter);
...
namespace {
    ...
    void testBootstrapFromSpread(Settings* settings) {
        ...
        for(Size i=0; i<n.size(); i++)
            helpers.push_back(
                ext::shared_ptr<DefaultProbabilityHelper>(
                    new SpreadCdsHelper(settings, quote[i], Period(n[i], Years),
...
//        //SavedSettings backup;
        ...
        for (Size i=0; i<n.size(); i++) {
            ...
            CreditDefaultSwap cds(settings, Protection::Buyer, notional, quote[i],
                                  schedule, convention, dayCounter,
                                  true, true, protectionStart);
            cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
                           new MidPointCdsEngine(settings, piecewiseCurve, recoveryRate,
                                                 discountCurve)));
    ...
    template <class T, class I>
    void testBootstrapFromUpfront(Settings* settings) {
        ...
        for(Size i=0; i<n.size(); i++)
            helpers.push_back(
                ext::shared_ptr<DefaultProbabilityHelper>(
                    new UpfrontCdsHelper(settings, quote[i], fixedRate,
                                         Period(n[i], Years),
                                         settlementDays, calendar,
                                         frequency, convention, rule,
                                         dayCounter, recoveryRate,
...
//        //SavedSettings backup;
        ...
        for (Size i=0; i<n.size(); i++) {
            ...
            CreditDefaultSwap cds(settings, Protection::Buyer, notional,
                                  quote[i], fixedRate,
                                  schedule, convention, dayCounter,
                                  true, true, protectionStart,
                                  upfrontDate,
...
void DefaultProbabilityCurveTest::testFlatHazardConsistency() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testBootstrapFromSpread<HazardRate,BackwardFlat>(&env);
    testBootstrapFromUpfront<HazardRate,BackwardFlat>(&env);
}
...
void DefaultProbabilityCurveTest::testFlatDensityConsistency() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testBootstrapFromSpread<DefaultDensity,BackwardFlat>(&env);
    testBootstrapFromUpfront<DefaultDensity,BackwardFlat>(&env);
}
...
void DefaultProbabilityCurveTest::testLinearDensityConsistency() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testBootstrapFromSpread<DefaultDensity,Linear>(&env);
    testBootstrapFromUpfront<DefaultDensity,Linear>(&env);
}
...
void DefaultProbabilityCurveTest::testLogLinearSurvivalConsistency() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testBootstrapFromSpread<SurvivalProbability,LogLinear>(&env);
    testBootstrapFromUpfront<SurvivalProbability,LogLinear>(&env);
}
...
void DefaultProbabilityCurveTest::testSingleInstrumentBootstrap() {
    ...
    helpers[0] = ext::shared_ptr<DefaultProbabilityHelper>(
                        new SpreadCdsHelper(settings, quote, tenor,
                                            settlementDays, calendar,
                                            frequency, convention, rule,
                                            dayCounter, recoveryRate,
                                            discountCurve));

    PiecewiseDefaultCurve<HazardRate,BackwardFlat> defaultCurve(settings, today, helpers,
                                                                dayCounter);
...
void DefaultProbabilityCurveTest::testUpfrontBootstrap() {
    ...
    boost::optional<bool> flag = settings->includeTodaysCashFlows();
    if (flag != false)
