...
void FdSabrTest::testFdmSabrOp() {
    ...
    VanillaOption optionPut(settings, putPayoff, exercise);
    VanillaOption optionCall(settings, callPayoff, exercise);

    const Handle<YieldTermStructure> rTS =
        Handle<YieldTermStructure>(flatRate(settings, today, 0.0, dc));
    ...
    const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess =
        ext::make_shared<GeneralizedBlackScholesProcess>(settings, 
            ...
            rTS, rTS, Handle<BlackVolTermStructure>(flatVol(settings, 0.2, dc)));
    ...
    for (Size j=0; j < LENGTH(betas); ++j) {
        ...
        const ext::shared_ptr<PricingEngine> pdeEngine =
            ext::make_shared<FdSabrVanillaEngine>(settings, 
...
void FdSabrTest::testFdmSabrCevPricing() {
    ...
    const Handle<YieldTermStructure> rTS = Handle<YieldTermStructure>(
        flatRate(settings, today, 0.05, dc));
    ...
    for (Size i=0; i < LENGTH(optionTypes); ++i) {
        ...
        for (Size j=0; j < LENGTH(strikes); ++j) {
            ...
            for (Size k=0; k < LENGTH(betas); ++k) {
                ...
                option.setPricingEngine(ext::make_shared<FdSabrVanillaEngine>(settings, 
                    f0, alpha, beta, nu, rho, rTS, 100, 400, 3));

                const Real calculated = option.NPV();

                option.setPricingEngine(ext::make_shared<AnalyticCEVEngine>(settings, 
                    f0, alpha, beta, rTS));
