        ...
        CommonVars() {
            ...
            for (Size i=0; i<deposits; i++) {
                instruments[i] = ext::shared_ptr<RateHelper>(new
                    DepositRateHelper(settings, depositData[i].rate/100,
                                      depositData[i].n*depositData[i].units,
                                      settlementDays, calendar,
                                      ModifiedFollowing, true,
                                      Actual360()));
            }
            ext::shared_ptr<IborIndex> index(new IborIndex(settings, "dummy",
                                                             6*Months,
                                                             settlementDays,
                                                             Currency(),
                                                             calendar,
            ...
            for (Size i=0; i<swaps; ++i) {
                instruments[i+deposits] = ext::shared_ptr<RateHelper>(new
                    SwapRateHelper(settings, swapData[i].rate/100,
            ...
            termStructure = ext::shared_ptr<YieldTermStructure>(new
                PiecewiseYieldCurve<Discount,LogLinear>(settings, settlement,
...
void TermStructureTest::testReferenceChange() {
    ...
    vars.termStructure = ext::shared_ptr<YieldTermStructure>(
                          new FlatForward(vars.settings, vars.settlementDays, NullCalendar(),
    ...
    vars.settings->evaluationDate() = today+30;
...
void TermStructureTest::testImplied() {
    ...
    ext::shared_ptr<YieldTermStructure> implied(
        new ImpliedTermStructure(vars.settings, Handle<YieldTermStructure>(vars.termStructure),
                                 newSettlement));
...
void TermStructureTest::testImpliedObs() {
    ...
    ext::shared_ptr<YieldTermStructure> implied(
                                  new ImpliedTermStructure(vars.settings, h, newSettlement));
    Flag flag;
...
void TermStructureTest::testFSpreaded() {
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ForwardSpreadedTermStructure(vars.settings, 
...
void TermStructureTest::testFSpreadedObs() {
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ForwardSpreadedTermStructure(vars.settings, h,mh));
    Flag flag;
...
void TermStructureTest::testZSpreaded() {
    ...
    Handle<Quote> mh(me);
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ZeroSpreadedTermStructure(vars.settings, 
            Handle<YieldTermStructure>(vars.termStructure),mh));
    Date testDate = vars.termStructure->referenceDate() + 5*Years;
...
void TermStructureTest::testZSpreadedObs() {
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ZeroSpreadedTermStructure(vars.settings, h,mh));
    Flag flag;
    flag.registerWith(spreaded);
    h.linkTo(vars.termStructure);
    if (!flag.isUp())
...
void TermStructureTest::testCreateWithNullUnderlying() {
    ...
    ext::shared_ptr<YieldTermStructure> spreaded(
        new ZeroSpreadedTermStructure(vars.settings, underlying,spread));
...
void TermStructureTest::testCompositeZeroYieldStructures() {
    ...
    settings->evaluationDate() = Date(10, Nov, 2017);
    ...
    ext::shared_ptr<YieldTermStructure> termStructure1 = ext::shared_ptr<YieldTermStructure>(
        new ForwardCurve(settings, dates, rates, Actual365Fixed(), NullCalendar()));
    ...
    ext::shared_ptr<YieldTermStructure> compoundCurve = ext::shared_ptr<YieldTermStructure>(
        new CompositeZeroYieldStructure<binary_f>(settings, Handle<YieldTermStructure>(termStructure1), Handle<YieldTermStructure>(termStructure2), sub));
