...
namespace cms_test {
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            termStructure.linkTo(flatRate(settings, referenceDate, 0.05,
                                          Actual365Fixed()));
            ...
            atmVol = Handle<SwaptionVolatilityStructure>(
                ext::shared_ptr<SwaptionVolatilityStructure>(new
                    SwaptionVolatilityMatrix(settings, calendar,
                                             Following,
                                             atmOptionTenors,
                                             atmSwapTenors,
                                             m,
            ...
            iborIndex = ext::shared_ptr<IborIndex>(new Euribor6M(settings, termStructure));
            ext::shared_ptr<SwapIndex> swapIndexBase(new
                EuriborSwapIsdaFixA(settings, 10*Years, termStructure));
            ext::shared_ptr<SwapIndex> shortSwapIndexBase(new
                EuriborSwapIsdaFixA(settings, 2*Years, termStructure));

            bool vegaWeightedSmileFit = false;

            SabrVolCube2 = Handle<SwaptionVolatilityStructure>(
                ext::make_shared<SwaptionVolCube2>(
                                     settings, 
                                     atmVol,
                                     optionTenors,
                                     swapTenors,
            ...
            SabrVolCube1 = Handle<SwaptionVolatilityStructure>(
                ext::shared_ptr<SwaptionVolCube1>(new
                    SwaptionVolCube1(settings, atmVol,
                                     optionTenors,
                                     swapTenors,
                                     strikeSpreads,
                                     volSpreads,
            ...
            Handle<Quote> zeroMeanRev(ext::shared_ptr<Quote>(new SimpleQuote(0.0)));
            ...
            for (Size j = 0; j < yieldCurveModels.size(); ++j) {
                if (j < yieldCurveModels.size() - 1)
                    numericalPricers.push_back(
                        ext::shared_ptr<CmsCouponPricer>(new NumericHaganPricer(settings, 
                            atmVol, yieldCurveModels[j], zeroMeanRev)));
                else
                    numericalPricers.push_back(ext::shared_ptr<CmsCouponPricer>(
                        new LinearTsrPricer(settings, atmVol, zeroMeanRev)));

                analyticPricers.push_back(ext::shared_ptr<CmsCouponPricer>(new
                    AnalyticHaganPricer(settings, atmVol, yieldCurveModels[j],
                                        zeroMeanRev)));
            }
        }
    };
...
void CmsTest::testFairRate()  {
    ...
    ext::shared_ptr<SwapIndex> swapIndex(new SwapIndex(vars.settings, "EuriborSwapIsdaFixA",
                                                       10*Years,
    ...
    Real gearing = 1.0;
    Spread spread = 0.0;
    CappedFlooredCmsCoupon coupon(vars.settings, paymentDate, nominal,
                                  startDate, endDate,
...
void CmsTest::testCmsSwap() {
    ...
    for (Size i=0; i<n; ++i)
        // no cap, floor
        // no gearing, spread
        cms[i] = MakeCms(vars.settings, Period(swapLengths[i], Years),
                         swapIndex,
...
void CmsTest::testParity() {
    ...
    ext::shared_ptr<SwapIndex> swapIndex(new
        EuriborSwapIsdaFixA(vars.settings, 10*Years,
    ...
    CappedFlooredCmsCoupon swaplet(vars.settings, paymentDate, nominal,
                                   startDate, endDate,
    ...
    for (Rate strike = .02; strike<.12; strike+=0.05) {
        CappedFlooredCmsCoupon   caplet(vars.settings, paymentDate, nominal,
                                        startDate, endDate,
                                        swapIndex->fixingDays(),
                                        swapIndex,
                                        gearing, spread,
                                        ...
                                        startDate, endDate,
                                        vars.iborIndex->dayCounter());
        CappedFlooredCmsCoupon floorlet(vars.settings, paymentDate, nominal,
                                        startDate, endDate,
                                        swapIndex->fixingDays(),
                                        swapIndex,
                                        gearing, spread,
