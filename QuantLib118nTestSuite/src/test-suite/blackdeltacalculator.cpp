...
void BlackDeltaCalculatorTest::testDeltaPriceConsistency() {
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS(
                        new BlackConstantVol(settings, today, calendar, volHandle, dc));
    ...
    for(Size i=0; i<LENGTH(values);++i){
        ...
        stochProcess=ext::make_shared<BlackScholesMertonProcess> (settings, spotHandle,
