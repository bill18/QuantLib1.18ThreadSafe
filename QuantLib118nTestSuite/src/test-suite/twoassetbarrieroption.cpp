...
void TwoAssetBarrierOptionTest::testHaugValues() {
    ...
    ext::shared_ptr<YieldTermStructure> qTS1 = flatRate(settings, today, q1, dc);
    ...
    ext::shared_ptr<BlackScholesMertonProcess> process1(
        new BlackScholesMertonProcess(settings, Handle<Quote>(s1),
    ...
    ext::shared_ptr<YieldTermStructure> qTS2 = flatRate(settings, today, q2, dc);
    ...
    ext::shared_ptr<BlackScholesMertonProcess> process2(
        new BlackScholesMertonProcess(settings, Handle<Quote>(s2),
    ...
    ext::shared_ptr<PricingEngine> engine(
                       new AnalyticTwoAssetBarrierEngine(settings, process1, process2,
                                                         Handle<Quote>(rho)));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        TwoAssetBarrierOption barrierOption(settings, values[i].barrierType,
                                            values[i].barrier,
                                            payoff,
                                            exercise);
