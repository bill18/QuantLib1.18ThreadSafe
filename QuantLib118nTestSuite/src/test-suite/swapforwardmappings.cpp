...
namespace {
    ...
    public:
        MarketModelData(Settings* settings);
    ...
    MarketModelData::MarketModelData(Settings* settings){
        // Times
        Calendar calendar = NullCalendar();
        Date todaysDate = settings->evaluationDate();
        Date endDate = todaysDate + 9*Years; // change back
        Schedule dates(settings->evaluationDate(), todaysDate, endDate, Period(Semiannual),
...
void SwapForwardMappingsTest::testForwardSwapJacobians()
{
    ...
    Settings* settings = &env;
    {
        ...
        MarketModelData marketData(settings);
