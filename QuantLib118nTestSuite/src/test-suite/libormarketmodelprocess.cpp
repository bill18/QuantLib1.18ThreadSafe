...
namespace libor_market_model_process_test {
    ...
    ext::shared_ptr<IborIndex> makeIndex(Settings* settings) {
        ...
        ext::shared_ptr<IborIndex> index(new Euribor1Y(settings, termStructure));
    ...
    makeCapVolCurve(Settings* settings, const Date& todaysDate) {
        ...
        ext::shared_ptr<LiborForwardModelProcess> process(
                            new LiborForwardModelProcess(settings, len+1, makeIndex(settings)));
...
void LiborMarketModelProcessTest::testInitialisation() {
    ...
    DayCounter dayCounter = Actual360();
    RelinkableHandle<YieldTermStructure> termStructure(
        flatRate(settings, Date::todaysDate(), 0.04, dayCounter));
    ...
    ext::shared_ptr<OptionletVolatilityStructure> capletVol(new
        ConstantOptionletVolatility(settings, termStructure->referenceDate(),
    ...
    for (Integer daysOffset=0; daysOffset < 1825 /* 5 year*/; daysOffset+=8) {
        ...
        termStructure.linkTo(flatRate(settings, settlementDate, 0.04, dayCounter));

        LiborForwardModelProcess process(settings, 60, index);
...
void LiborMarketModelProcessTest::testLambdaBootstrapping() {
    ...
    ext::shared_ptr<LiborForwardModelProcess> process = makeProcess(settings);
...
void LiborMarketModelProcessTest::testMonteCarloCapletPricing() {
    ...
    ext::shared_ptr<LiborForwardModelProcess> process1 = makeProcess(settings);
    ext::shared_ptr<LiborForwardModelProcess> process2 = makeProcess(settings,
                                                                    volaComp);
