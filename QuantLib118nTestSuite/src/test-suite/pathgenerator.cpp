...
void PathGeneratorTest::testPathGenerator() {
    ...
    settings->evaluationDate() = Date(26,April,2005);

    Handle<Quote> x0(ext::shared_ptr<Quote>(new SimpleQuote(100.0)));
    Handle<YieldTermStructure> r(flatRate(settings, 0.05, Actual360()));
    Handle<YieldTermStructure> q(flatRate(settings, 0.02, Actual360()));
    Handle<BlackVolTermStructure> sigma(flatVol(settings, 0.20, Actual360()));
    // commented values must be used when Halley's correction is enabled
    testSingle(ext::shared_ptr<StochasticProcess1D>(
                                 new BlackScholesMertonProcess(settings, x0,q,r,sigma)),
