...
namespace european_option_test {
    ...
    makeProcess(Settings* settings, const ext::shared_ptr<Quote>& u,
        ...
        return ext::make_shared<BlackScholesMertonProcess>(settings, 
           Handle<Quote>(u),
    ...
    makeOption(Settings* settings, const ext::shared_ptr<StrikedTypePayoff>& payoff,
        ...
        ext::shared_ptr<GeneralizedBlackScholesProcess> stochProcess =
            makeProcess(settings, u,q,r,vol);

        ext::shared_ptr<PricingEngine> engine;
        switch (engineType) {
          case Analytic:
            engine = ext::shared_ptr<PricingEngine>(
                                    new AnalyticEuropeanEngine(settings, stochProcess));
            break;
          case JR:
            engine = ext::shared_ptr<PricingEngine>(
                        new BinomialVanillaEngine<JarrowRudd>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case CRR:
            engine = ext::shared_ptr<PricingEngine>(
                 new BinomialVanillaEngine<CoxRossRubinstein>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case EQP:
            engine = ext::shared_ptr<PricingEngine>(
                 new BinomialVanillaEngine<AdditiveEQPBinomialTree>(settings, 
                                                              stochProcess,
                                                              binomialSteps));
            break;
          case TGEO:
            engine = ext::shared_ptr<PricingEngine>(
                        new BinomialVanillaEngine<Trigeorgis>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case TIAN:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<Tian>(settings, stochProcess, binomialSteps));
            break;
          case LR:
            engine = ext::shared_ptr<PricingEngine>(
                      new BinomialVanillaEngine<LeisenReimer>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case JOSHI:
            engine = ext::shared_ptr<PricingEngine>(
              new BinomialVanillaEngine<Joshi4>(settings, stochProcess, binomialSteps));
            break;
          case FiniteDifferences:
            engine = ext::shared_ptr<PricingEngine>(
                            new FdBlackScholesVanillaEngine(settings, stochProcess,
                                                            binomialSteps,
                                                            samples));
            break;
          case Integral:
            engine = ext::shared_ptr<PricingEngine>(
                                            new IntegralEngine(settings, stochProcess));
            break;
          case PseudoMonteCarlo:
            engine = MakeMCEuropeanEngine<PseudoRandom>(settings, stochProcess)
                .withSteps(1)
                .withSamples(samples)
                .withSeed(42);
            break;
          case QuasiMonteCarlo:
            engine = MakeMCEuropeanEngine<LowDiscrepancy>(settings, stochProcess)
                .withSteps(1)
                .withSamples(samples);
            break;
          case FFT:
              engine = ext::shared_ptr<PricingEngine>(
                                          new FFTVanillaEngine(settings, stochProcess));
            break;
          default:
        ...
        ext::shared_ptr<VanillaOption> option(
            new EuropeanOption(settings, payoff, exercise));
...
//    //SavedSettings backup;
    ...
    */
    ...
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, today, qRate, dc);
    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.0));
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, rRate, dc);
    ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.0));
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, today, vol, dc);
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new
            BlackScholesMertonProcess(settings, Handle<Quote>(spot),
        ...
        engine = ext::shared_ptr<PricingEngine>(
                    new FdBlackScholesVanillaEngine(settings, stochProcess,200,400));
        option.setPricingEngine(engine);
...
void EuropeanOptionTest::testGreeks() {
    ...
    ext::shared_ptr<SimpleQuote> spot(new SimpleQuote(0.0));
    ext::shared_ptr<SimpleQuote> qRate(new SimpleQuote(0.0));
    Handle<YieldTermStructure> qTS(flatRate(settings, qRate, dc));
    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.0));
    Handle<YieldTermStructure> rTS(flatRate(settings, rRate, dc));
    ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.0));
    Handle<BlackVolTermStructure> volTS(flatVol(settings, vol, dc));
        ...
        for (Size k=0; k<LENGTH(residualTimes); k++) {
          ...
          for (Size kk=0; kk<4; kk++) {
              ...
              ext::shared_ptr<BlackScholesMertonProcess> stochProcess(
                            new BlackScholesMertonProcess(settings, Handle<Quote>(spot),
                                                          qTS, rTS, volTS));
              ext::shared_ptr<PricingEngine> engine(
                                    new AnalyticEuropeanEngine(settings, stochProcess));
              EuropeanOption option(settings, payoff, exercise);
                  ...
                  for (Size n=0; n<LENGTH(rRates); n++) {
                    for (Size p=0; p<LENGTH(vols); p++) {
                      ...
                      if (value > spot->value()*1.0e-5) {
                          ...
                          settings->evaluationDate() = today-1;
                          value_m = option.NPV();
                          settings->evaluationDate() = today+1;
                          value_p = option.NPV();
                          settings->evaluationDate() = today;
    ...
    for (Size i=0; i<LENGTH(types); i++) {
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
          ...
          ext::shared_ptr<VanillaOption> option =
              makeOption(settings, payoff, exercise, spot, qTS, rTS, volTS,
                         Analytic, Null<Size>(), Null<Size>());

          ext::shared_ptr<GeneralizedBlackScholesProcess> process =
              makeProcess(settings, spot, qTS, rTS,volTS);
...
void EuropeanOptionTest::testImpliedVolContainment() {
    ...
    Handle<BlackVolTermStructure> volTS(flatVol(settings, today, vol, dc));
    ...
    ext::shared_ptr<BlackScholesMertonProcess> process(
                  new BlackScholesMertonProcess(settings, underlying, qTS, rTS, volTS));
    ext::shared_ptr<PricingEngine> engine(
                                        new AnalyticEuropeanEngine(settings, process));
...
namespace {
    ...
    void testEngineConsistency(Settings* settings, european_option_test::EngineType engine,
                               Size binomialSteps,
                               Size samples,
...
//    //SavedSettings backup;
    ...
    testEngineConsistency(settings, engine,steps,samples,relativeTol,true);
}
...
void EuropeanOptionTest::testFdEngines() {
    ...
    testEngineConsistency(settings, engine,timeSteps,gridPoints,relativeTol,true);
}
...
void EuropeanOptionTest::testIntegralEngines() {
    ...
    testEngineConsistency(settings, engine,timeSteps,gridPoints,relativeTol);
}
...
void EuropeanOptionTest::testMcEngines() {
    ...
    testEngineConsistency(settings, engine,steps,samples,relativeTol);
}
...
void EuropeanOptionTest::testLocalVolatility() {
    ...
    const ext::shared_ptr<YieldTermStructure> rTS(
                                   new ZeroCurve(settings, dates, rates, dayCounter));
    ...
    for (Size i=2; i < dates.size(); i+=2) {
        for (Size j=3; j < strikes.size()-5; j+=5) {
            ...
            option.setPricingEngine(ext::shared_ptr<PricingEngine>(
                         new FdBlackScholesVanillaEngine(settings, process, 200, 400)));
            ...
            for (Size i=0; i < LENGTH(schemeDescs); ++i) {
                option.setPricingEngine(
                    ext::make_shared<FdBlackScholesVanillaEngine>(settings, 
...
void EuropeanOptionTest::testAnalyticEngineDiscountCurve() {
    ...
    ext::shared_ptr<YieldTermStructure> discTS = flatRate(settings, today, discRate, dc);
...
void EuropeanOptionTest::testPDESchemes() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.06, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.10, dc));
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, 0.35, dc));

    const Date maturity = today + Period(6, Months);

    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(settings, 
            spot, qTS, rTS, volTS);
    ...
    DividendVanillaOption dividendOption(settings, 
        payoff, exercise,
...
void EuropeanOptionTest::testFdEngineWithNonConstantParameters() {
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, today,v,dc);
    ...
    ext::shared_ptr<YieldTermStructure> rTS =
        ext::make_shared<ForwardCurve>(settings, dates, rates, dc);
    Rate r = rTS->zeroRate(dates[4], dc, Continuous);

    ext::shared_ptr<BlackScholesProcess> process =
        ext::make_shared<BlackScholesProcess>(settings, Handle<Quote>(spot),
                                              Handle<YieldTermStructure>(rTS),
                                              Handle<BlackVolTermStructure>(volTS));
    ...
    option.setPricingEngine(ext::make_shared<AnalyticEuropeanEngine>(settings, process));
    Real expected = option.NPV();

    Size timeSteps = 200;
    Size gridPoints = 201;
    option.setPricingEngine(ext::make_shared<FdBlackScholesVanillaEngine>(settings, 
...
void EuropeanOptionTest::testDouglasVsCrankNicolson() {
    ...
    const Handle<Quote> spot(ext::make_shared<SimpleQuote>(100.0));
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.02, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.075, dc));
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, 0.25, dc));
