...
void CdsOptionTest::testCached() {
    ...
    Schedule schedule(settings->evaluationDate(), startDate,maturity, Period(Quarterly),
    ...
    Handle<DefaultProbabilityTermStructure> defaultProbability(
        ext::shared_ptr<DefaultProbabilityTermStructure>(
                    new FlatHazardRate(settings, 0, calendar, hazardRate, dayCounter)));

    ext::shared_ptr<PricingEngine> swapEngine(
           new MidPointCdsEngine(settings, defaultProbability, recoveryRate, riskFree));

    CreditDefaultSwap swap(settings, Protection::Seller, notional, 0.001, schedule,
    ...
    ext::shared_ptr<CreditDefaultSwap> underlying(
         new CreditDefaultSwap(settings, Protection::Seller, notional, strike, schedule,
                               convention, dayCounter));
    underlying->setPricingEngine(swapEngine);

    ext::shared_ptr<Exercise> exercise(new EuropeanExercise(expiry));
    CdsOption option1(settings, underlying, exercise);
    option1.setPricingEngine(ext::shared_ptr<PricingEngine>(
                    new BlackCdsOptionEngine(settings, defaultProbability, recoveryRate,
                                             riskFree, cdsVol)));
    ...
    underlying = ext::make_shared<CreditDefaultSwap>(settings, 
         Protection::Buyer, notional, strike, schedule,
                               convention, dayCounter);
    underlying->setPricingEngine(swapEngine);

    CdsOption option2(settings, underlying, exercise);
