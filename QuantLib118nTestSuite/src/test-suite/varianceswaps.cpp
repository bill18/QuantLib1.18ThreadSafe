...
void VarianceSwapTest::testReplicatingVarianceSwap() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<BlackVolTermStructure> volTS(new
            BlackVarianceSurface(settings, today, NullCalendar(),
        ...
        ext::shared_ptr<PricingEngine> engine(
                          new ReplicatingVarianceSwapEngine(settings, stochProcess, 5.0,
                                                            callStrikes,
                                                            putStrikes));

        VarianceSwap varianceSwap(settings, values[i].type,
                                  values[i].varStrike,
                                  values[i].nominal,
                                  today,
                                  exDate);
...
void VarianceSwapTest::testMCVarianceSwap() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<BlackVolTermStructure> volTS(
                        new BlackVarianceCurve(settings,today, dates, vols, dc, true));
        ...
        ext::shared_ptr<PricingEngine> engine;
        engine =
            MakeMCVarianceSwapEngine<PseudoRandom>(settings, stochProcess)
