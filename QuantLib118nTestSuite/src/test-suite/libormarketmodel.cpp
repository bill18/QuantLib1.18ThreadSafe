...
namespace libor_market_model_test {
    ...
    ext::shared_ptr<IborIndex> makeIndex(Settings* settings, std::vector<Date> dates,
                                           std::vector<Rate> rates) {
        DayCounter dayCounter = Actual360();

        RelinkableHandle<YieldTermStructure> termStructure;

        ext::shared_ptr<IborIndex> index(new Euribor6M(settings, termStructure));
    ...
    ext::shared_ptr<IborIndex> makeIndex(Settings* settings) {
    ...
    makeCapVolCurve(Settings* settings, const Date& todaysDate) {
        ...
        ext::shared_ptr<LiborForwardModelProcess> process(
                               new LiborForwardModelProcess(settings, 10, makeIndex(settings)));
...
void LiborMarketModelTest::testSimpleCovarianceModels() {
    ...
    ext::shared_ptr<LiborForwardModelProcess> process(
                             new LiborForwardModelProcess(settings, size, makeIndex(settings)));

    ext::shared_ptr<LiborForwardModel> liborModel(
                        new LiborForwardModel(settings, process, volaModel, corrModel));
...
void LiborMarketModelTest::testCapletPricing() {
    ...
    ext::shared_ptr<IborIndex> index = makeIndex(settings);
    ext::shared_ptr<LiborForwardModelProcess> process(
        new LiborForwardModelProcess(settings, size, index));
    ...
    ext::shared_ptr<AnalyticCapFloorEngine> engine1(
                            new AnalyticCapFloorEngine(settings, model, termStructure));

    ext::shared_ptr<Cap> cap1(
        new Cap(settings, process->cashFlows(),
...
void LiborMarketModelTest::testCalibration() {
    ...
    for (i=2; i < size; ++i) {
        ...
        ext::shared_ptr<BlackCalibrationHelper> caphelper(
            new CapHelper(settings, maturity, capVol, index, Annual,
        ...
        if (i<= size/2) {
            ...
            for (Size j=1; j <= size/2; ++j) {
                ...
                ext::shared_ptr<BlackCalibrationHelper> swaptionHelper(
                    new SwaptionHelper(settings, maturity, len, swaptionVol, index,
                ...
                swaptionHelper->setPricingEngine(
                     ext::shared_ptr<PricingEngine>(
                                 new LfmSwaptionEngine(settings, model,termStructure)));
...
void LiborMarketModelTest::testSwaptionPricing() {
    ...
    ext::shared_ptr<IborIndex> index = makeIndex(settings, dates, rates);
   ...
   // set-up pricing engine
    ...
    ext::shared_ptr<LiborForwardModel>
        liborModel(new LiborForwardModel(settings, process, volaModel, corrModel));
    ...
    for (i=1; i < size; ++i) {
        for (Size j=1; j <= size-i; ++j) {
            ...
            Schedule schedule(settings->evaluationDate(), fwdStart, fwdMaturity, index->tenor(), calendar,
                               convention, convention, DateGeneration::Forward, false);

            Rate swapRate  = 0.0404;
            ext::shared_ptr<VanillaSwap> forwardSwap(
                new VanillaSwap(settings, VanillaSwap::Receiver, 1.0,
                                schedule, swapRate, dayCounter,
                                schedule, index, 0.0, index->dayCounter()));
            forwardSwap->setPricingEngine(ext::shared_ptr<PricingEngine>(
                new DiscountingSwapEngine(settings, index->forwardingTermStructure())));
            ...
            if (i == j && i<=size/2) {
                ext::shared_ptr<PricingEngine> engine(
                     new LfmSwaptionEngine(settings, liborModel,
                                           index->forwardingTermStructure()));
                ext::shared_ptr<Exercise> exercise(
                    new EuropeanExercise(process->fixingDates()[i]));

                ext::shared_ptr<Swaption> swaption(
                    new Swaption(settings, forwardSwap, exercise));
