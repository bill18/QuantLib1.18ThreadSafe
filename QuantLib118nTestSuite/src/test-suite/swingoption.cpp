...
void SwingOptionTest::testExtOUJumpVanillaEngine() {
    ...
    ext::shared_ptr<YieldTermStructure> rTS(flatRate(settings, today, irRate, dc));
    ext::shared_ptr<StrikedTypePayoff> payoff(
                                     new PlainVanillaPayoff(Option::Call, 30));
    ext::shared_ptr<Exercise> exercise(new EuropeanExercise(maturityDate));

    ext::shared_ptr<PricingEngine> engine(
                 new FdExtOUJumpVanillaEngine(settings, jumpProcess, rTS, 25, 200, 50));
...
void SwingOptionTest::testFdBSSwingOption() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.14, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.02, dayCounter));
    Handle<BlackVolTermStructure> volTS(
                                    flatVol(settings, settlementDate, 0.4, dayCounter));

    Handle<Quote> s0(ext::shared_ptr<Quote>(new SimpleQuote(30.0)));

    ext::shared_ptr<BlackScholesMertonProcess> process(
            new BlackScholesMertonProcess(settings, s0, dividendTS, riskFreeTS, volTS));
    ext::shared_ptr<PricingEngine> engine(
                                new FdSimpleBSSwingEngine(settings, process, 50, 200));
    
    VanillaOption bermudanOption(settings, payoff, swingExercise);
    bermudanOption.setPricingEngine(ext::shared_ptr<PricingEngine>(
                          new FdBlackScholesVanillaEngine(settings, process, 50, 200)));
    const Real bermudanOptionPrices = bermudanOption.NPV();
    
    for (Size i=0; i < exerciseDates.size(); ++i) {
        const Size exerciseRights = i+1;
        
        VanillaSwingOption swingOption(settings, forward, swingExercise,
        		                       0, exerciseRights);
        ...
        for (Size j=exerciseDates.size()-i-1; j < exerciseDates.size(); ++j) {
            VanillaOption europeanOption(settings, payoff, ext::shared_ptr<Exercise>(
...
void SwingOptionTest::testExtOUJumpSwingOption() {
    ...
    ext::shared_ptr<YieldTermStructure> rTS(
                                flatRate(settings, settlementDate, irRate, dayCounter));

    ext::shared_ptr<PricingEngine> swingEngine(
                new FdSimpleExtOUJumpSwingEngine(settings, jumpProcess, rTS, 25, 50, 25));

    ext::shared_ptr<PricingEngine> vanillaEngine(
                new FdExtOUJumpVanillaEngine(settings, jumpProcess, rTS, 25, 50, 25));
...
namespace swing_option_test {
    class SwingPdePricing {
      public:
        ...
        SwingPdePricing(Settings* settings, const ext::shared_ptr<ExtOUWithJumpsProcess>& process,
                        const ext::shared_ptr<VanillaOption>& option,
                        const ext::shared_ptr<Shape>& shape)
        : process_(process), option_(option), shape_(shape), env_(settings) {}

        Real operator()(Real x) const {
            const ext::shared_ptr<YieldTermStructure> rTS(
                flatRate(env_, 0.0, Actual365Fixed()));
            ...
            option_->setPricingEngine(
                ext::make_shared<FdExtOUJumpVanillaEngine>(env_,
                    process_, rTS,
...
void SwingOptionTest::testKlugeChFVanillaPricing() {
    ...
    const Real expected =
        RichardsonExtrapolation(
            SwingPdePricing(settings, klugeProcess, option, shape), 4.0)(2.0, 1.5);
