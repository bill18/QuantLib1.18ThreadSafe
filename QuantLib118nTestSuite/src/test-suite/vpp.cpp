...
void VPPTest::testGemanRoncoroniProcess() {
    ...
    */
    ...
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, 0.03, dc);
...
void VPPTest::testSimpleExtOUStorageEngine() {
    ...
    ext::shared_ptr<PricingEngine> storageEngine(
               new FdSimpleExtOUStorageEngine(settings, ouProcess, rTS, 1, 25));

    VanillaStorageOption storageOption(settings, bermudanExercise, 50, 0, 1);
...
void VPPTest::testKlugeExtOUSpreadOption() {
    ...
    BasketOption option(settings, basketPayoff, exercise);
    option.setPricingEngine(ext::shared_ptr<PricingEngine>(
        new FdKlugeExtOUSpreadEngine(settings, klugeOUProcess, rTS,
                                     5, 200, 50, 20)));
...
void VPPTest::testVPPIntrinsicValue() {
    ...
    for (Size i=0; i < LENGTH(efficiency); ++i) {
        ...
        VanillaVPPOption option(settings, heatRate, pMin, pMax, tMinUp, tMinDown,
                                startUpFuel, startUpFixCost, exercise);

        option.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new DynProgVPPIntrinsicValueEngine(settings,
                std::vector<Real>(fuelPrices,fuelPrices+LENGTH(fuelPrices)),
                std::vector<Real>(powerPrices,powerPrices+LENGTH(powerPrices)),
                fuelCostAddon, flatRate(settings, 0.0, dc))));
...
void VPPTest::testVPPPricing() {
    ...
    VanillaVPPOption vppOption(settings, heatRate, pMin, pMax, tMinUp, tMinDown,
                               startUpFuel, startUpFixCost, exercise);
    ...
    const ext::shared_ptr<YieldTermStructure> rTS
        = flatRate(settings, today, irRate, dc);
    ...
    const ext::shared_ptr<PricingEngine> engine(
        new FdSimpleKlugeExtOUVPPEngine(settings, klugeOUProcess, rTS,
...
void VPPTest::testKlugeExtOUMatrixDecomposition() {
    ...
    const ext::shared_ptr<FdmLinearOpComposite> op(
        new FdmKlugeExtOUOp(settings, mesher, klugeOUProcess,
                            flatRate(settings, today, 0.0, ActualActual()),
