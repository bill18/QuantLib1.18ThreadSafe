...
void LookbackOptionTest::testAnalyticContinuousFloatingLookback() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                  new AnalyticContinuousFloatingLookbackEngine(settings, stochProcess));

        ContinuousFloatingLookbackOption option(settings, values[i].minmax,
                                                payoff,
                                                exercise);
...
void LookbackOptionTest::testAnalyticContinuousFixedLookback() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                     new AnalyticContinuousFixedLookbackEngine(settings, stochProcess));

        ContinuousFixedLookbackOption option(settings, values[i].minmax,
                                             payoff,
                                             exercise);
        option.setPricingEngine(engine);
...
void LookbackOptionTest::testAnalyticContinuousPartialFloatingLookback() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                  new AnalyticContinuousPartialFloatingLookbackEngine(settings, stochProcess));

        Date lookbackEnd = today + Integer(values[i].t1*360+0.5);
        ContinuousPartialFloatingLookbackOption option(settings, values[i].minmax,
                                                values[i].l,
                                                lookbackEnd,
                                                payoff,
                                                exercise);
...
void LookbackOptionTest::testAnalyticContinuousPartialFixedLookback() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                     new AnalyticContinuousPartialFixedLookbackEngine(settings, stochProcess));

        Date lookbackStart = today + Integer(values[i].t1*360+0.5);
        ContinuousPartialFixedLookbackOption option(settings, lookbackStart,
                                             payoff,
                                             exercise);
        option.setPricingEngine(engine);
