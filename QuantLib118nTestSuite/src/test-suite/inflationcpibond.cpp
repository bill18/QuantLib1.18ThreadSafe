    ...
    std::vector<ext::shared_ptr<Helper> > makeHelpers(
        ...
        for (Size i=0; i<N; i++) {
            ...
            ext::shared_ptr<Helper> h(
                      new ZeroCouponInflationSwapHelper(settings, quote, observationLag,
                                                        maturity, calendar,
                                                        bdc, dc, ii, yTS));
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            Schedule rpiSchedule =
                MakeSchedule(settings->evaluationDate()).from(from).to(to)
                .withTenor(1*Months)
                .withCalendar(UnitedKingdom())
                .withConvention(ModifiedFollowing);

            bool interp = false;
            ii = ext::make_shared<UKRPI>(settings, interp, cpiTS);
            ...
            yTS.linkTo(ext::shared_ptr<YieldTermStructure>(
                          new FlatForward(settings, evaluationDate, 0.05, dayCounter)));
            ...
            std::vector<ext::shared_ptr<Helper> > helpers =
                makeHelpers(settings, zciisData, LENGTH(zciisData), ii,
...
void InflationCPIBondTest::testCleanPrice() {
    ...
    Schedule fixedSchedule =
        MakeSchedule(common.settings->evaluationDate()).from(startDate).to(endDate)
                      .withTenor(Period(6,Months))
                      .withCalendar(UnitedKingdom())
                      .withConvention(Unadjusted)
                      .backwards();

    CPIBond bond(common.settings, settlementDays, notional, growthOnly,
    ...
    ext::shared_ptr<DiscountingBondEngine> engine(
                                 new DiscountingBondEngine(common.settings, common.yTS));
