...
void QuantoOptionTest::testValues() {
    ...
    */
    ...
    Handle<YieldTermStructure> fxrTS(flatRate(settings, today, fxRate, dc));
    ext::shared_ptr<SimpleQuote> fxVol(new SimpleQuote(0.0));
    Handle<BlackVolTermStructure> fxVolTS(flatVol(settings, today, fxVol, dc));
    ...
    ext::shared_ptr<PricingEngine> engine(
        new QuantoEngine<VanillaOption, AnalyticEuropeanEngine>(settings,
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        QuantoVanillaOption option(settings, payoff, exercise);
...
void QuantoOptionTest::testGreeks() {
    ...
    ext::shared_ptr<SimpleQuote> fxRate(new SimpleQuote(0.0));
    Handle<YieldTermStructure> fxrTS(flatRate(settings, fxRate, dc));
    ext::shared_ptr<SimpleQuote> fxVol(new SimpleQuote(0.0));
    Handle<BlackVolTermStructure> fxVolTS(flatVol(settings, fxVol, dc));
    ext::shared_ptr<SimpleQuote> correlation(new SimpleQuote(0.0));
...
void QuantoOptionTest::testForwardValues() {
    ...
    ext::shared_ptr<PricingEngine> engine(
        new QuantoEngine<ForwardVanillaOption,
                         ForwardVanillaEngine<AnalyticEuropeanEngine> >(settings, 
                                                 stochProcess, fxrTS, fxVolTS,
                                                 Handle<Quote>(correlation)));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        QuantoForwardVanillaOption option(settings, values[i].moneyness, reset,
                                          payoff, exercise);
        option.setPricingEngine(engine);
      ...
      for (Size j=0; j<LENGTH(moneyness); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
          for (Size h=0; h<LENGTH(startMonths); h++) {
            ...
            QuantoForwardVanillaOption option(settings, moneyness[j], reset,
                                              payoff, exercise);
            option.setPricingEngine(engine);
...
void QuantoOptionTest::testBarrierValues()  {
    ...
    ext::shared_ptr<PricingEngine> engine(
        new QuantoEngine<BarrierOption, AnalyticBarrierEngine>(settings, 
                                                 stochProcess, fxrTS, fxVolTS,
                                                 Handle<Quote>(correlation)));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        QuantoBarrierOption option(settings, values[i].barrierType,
                                   values[i].barrier,
                                   values[i].rebate,
                                   payoff,
                                   exercise);
...
void QuantoOptionTest::testDoubleBarrierValues()  {
    ...
    ext::shared_ptr<PricingEngine> engine(
        new QuantoEngine<DoubleBarrierOption, AnalyticDoubleBarrierEngine>(settings, 
                                                 stochProcess, fxrTS, fxVolTS,
                                                 Handle<Quote>(correlation)));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        QuantoDoubleBarrierOption option(settings, values[i].barrierType,
                                   values[i].barrier_lo,
                                   values[i].barrier_hi,
                                   values[i].rebate,
                                   payoff,
...
void QuantoOptionTest::testFDMQuantoHelper()  {
    ...
    const Handle<YieldTermStructure> domesticTS(
        flatRate(settings, today, domesticR, dc));

    const Handle<YieldTermStructure> divTS(
        flatRate(settings, today, q, dc));

    const Handle<BlackVolTermStructure> volTS(
        flatVol(settings, today, vol, dc));
    ...
    const ext::shared_ptr<YieldTermStructure> foreignTS
        = flatRate(settings, today, foreignR, dc);

    const ext::shared_ptr<BlackVolTermStructure> fxVolTS
        = flatVol(settings, today, fxVol, dc);
    ...
    const ext::shared_ptr<FdmBlackScholesMesher> mesher(
        new FdmBlackScholesMesher(settings, 
...
void QuantoOptionTest::testPDEOptionValues()  {
    ...
    for (Size i=0; i < LENGTH(values); ++i) {
        ...
        const Handle<YieldTermStructure> domesticTS(
            flatRate(settings, today, values[i].r, dc));

        const Handle<YieldTermStructure> divTS(
            flatRate(settings, today, values[i].q, dc));

        const Handle<BlackVolTermStructure> volTS(
            flatVol(settings, today, values[i].v, dc));
        ...
        const Handle<YieldTermStructure> foreignTS(
            flatRate(settings, today, values[i].fxr, dc));

        const Handle<BlackVolTermStructure> fxVolTS(
            flatVol(settings, today, values[i].fxv, dc));
        ...
        const ext::shared_ptr<PricingEngine> analyticEngine
            = ext::make_shared<QuantoEngine<
                VanillaOption, AnalyticEuropeanEngine> >(settings, 
...
void QuantoOptionTest::testAmericanQuantoOption()  {
    ...
    DividendVanillaOption option(settings, 
    ...
    const ext::shared_ptr<LocalVolTermStructure> localConstVol =
        ext::make_shared<LocalConstantVol>(settings, today, 2.0, dc);
