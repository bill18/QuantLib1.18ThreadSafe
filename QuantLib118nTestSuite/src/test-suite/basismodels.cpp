...
namespace {
    ...
    Handle<YieldTermStructure> getYTS(Settings* settings, const std::vector<Period>& terms,
    ...
    Handle<OptionletVolatilityStructure> getOptionletTS(Settings* settings) {
        Date today = settings->evaluationDate();
        std::vector<Date> dates;
        ...
        Handle<YieldTermStructure> curve3m = getYTS(settings, terms, proj3mRates);
        ext::shared_ptr<IborIndex> index(new Euribor3M(settings, curve3m));
        ext::shared_ptr<StrippedOptionletBase> tmp1(
            new StrippedOptionlet(settings, 2, TARGET(), Following, index, dates, capletStrikes,
                                  capletVolQuotes, Actual365Fixed(), Normal, 0.0));
        ext::shared_ptr<StrippedOptionletAdapter> tmp2(new StrippedOptionletAdapter(settings, tmp1));
    ...
    Handle<SwaptionVolatilityStructure> getSwaptionVTS(Settings* settings) {
        ...
        ext::shared_ptr<SwaptionVolatilityStructure> tmp(
            new SwaptionVolatilityMatrix(settings, TARGET(), Following, swaptionVTSTerms, swaptionVTSTerms,
                                         swaptionVolQuotes, Actual365Fixed(), true, Normal));
        return RelinkableHandle<SwaptionVolatilityStructure>(tmp);
    }

    void testSwaptioncfs(Settings* settings, bool contTenorSpread) {
        // market data and floating rate index
        Handle<YieldTermStructure> discYTS = getYTS(settings, terms, discRates);
        Handle<YieldTermStructure> proj6mYTS = getYTS(settings, terms, proj6mRates);
        ext::shared_ptr<IborIndex> euribor6m(new Euribor6M(settings, proj6mYTS));
        ...
        Schedule fixedSchedule(settings->evaluationDate(), swapStart, swapEnd, Period(1, Years), TARGET(), ModifiedFollowing,
                               ModifiedFollowing, DateGeneration::Backward, false);
        Schedule floatSchedule(settings->evaluationDate(), swapStart, swapEnd, Period(6, Months), TARGET(), ModifiedFollowing,
                               ModifiedFollowing, DateGeneration::Backward, false);
        ext::shared_ptr<VanillaSwap> swap(
            new VanillaSwap(settings, VanillaSwap::Payer, 10000.0, fixedSchedule, 0.03, Thirty360(),
                            floatSchedule, euribor6m, 0.0, euribor6m->dayCounter()));
        swap->setPricingEngine(ext::shared_ptr<PricingEngine>(new DiscountingSwapEngine(settings, discYTS)));
        ...
        ext::shared_ptr<Swaption> swaption(
            new Swaption(settings, swap, europeanExercise, Settlement::Physical));
        // calculate basis model swaption cash flows, discount and conmpare with swap
        SwaptionCashFlows cashFlows(settings, swaption, discYTS, contTenorSpread);
        ...
        SwaptionCashFlows singleCurveCashFlows(settings, swaption, proj6mYTS, contTenorSpread);
...
void BasismodelsTest::testSwaptioncfsSimpleCompSpread() {
    ...
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    Settings *settings = &env;
    testSwaptioncfs(settings, false);
}
...
void BasismodelsTest::testTenoroptionletvts() {
    ...
    Real spread = 0.01;
    Handle<YieldTermStructure> discYTS = getYTS(settings, terms, discRates);
    Handle<YieldTermStructure> proj3mYTS = getYTS(settings, terms, proj3mRates);
    Handle<YieldTermStructure> proj6mYTS = getYTS(settings, terms, proj3mRates, spread);
    ext::shared_ptr<IborIndex> euribor3m(new Euribor6M(settings, proj3mYTS));
    ext::shared_ptr<IborIndex> euribor6m(new Euribor6M(settings, proj6mYTS));
    // 3m optionlet VTS
    Handle<OptionletVolatilityStructure> optionletVTS3m = getOptionletTS(settings);
    {
        ...
        ext::shared_ptr<OptionletVolatilityStructure> optionletVTS6m(
            new TenorOptionletVTS(settings, optionletVTS3m, euribor3m, euribor6m, corr));
...
void BasismodelsTest::testTenorswaptionvts() {
    ...
    ext::shared_ptr<IborIndex> euribor6m(new Euribor6M(settings, proj6mYTS));
    // Euribor6m ATM vols
    Handle<SwaptionVolatilityStructure> euribor6mSwVTS = getSwaptionVTS(settings);
    {
        ext::shared_ptr<TenorSwaptionVTS> euribor3mSwVTS(
            new TenorSwaptionVTS(settings, euribor6mSwVTS, discYTS, euribor6m, euribor3m, Period(1, Years),
    ...
    }
    {
        ext::shared_ptr<TenorSwaptionVTS> euribor6mSwVTS2(
            new TenorSwaptionVTS(settings, euribor6mSwVTS, discYTS, euribor6m, euribor6m, Period(1, Years),
                                 Period(1, Years), Thirty360(), Thirty360()));
