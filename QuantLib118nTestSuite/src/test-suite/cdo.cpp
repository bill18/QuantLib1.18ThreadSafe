...
void CdoTest::testHW(unsigned dataSet) {
    ...
    Schedule schedule = MakeSchedule(settings->evaluationDate()).from(Date (1, September, 2006))
    ...
    settings->evaluationDate() = asofDate;

    ext::shared_ptr<YieldTermStructure> yieldPtr(
                                              new FlatForward (settings, asofDate, rate,
                                                               daycount, cmp));
    ...
    ext::shared_ptr<DefaultProbabilityTermStructure> ptr (
               new FlatHazardRate (settings, asofDate,
                                   hazardRate,
                                   ActualActual()));
    ...
    ext::shared_ptr<PricingEngine> midPCDOEngine( new MidPointCDOEngine(settings, 
        yieldHandle));
    ext::shared_ptr<PricingEngine> integralCDOEngine( new IntegralCDOEngine(settings, 
        yieldHandle));
    ...
    if (hwData7[i].nm == -1 && hwData7[i].nz == -1){
        ...
        basketModels.push_back(ext::shared_ptr<DefaultLossModel>(new
            RandomDefaultLM<GaussianCopulaPolicy>(settings, gaussKtLossLM, numSims)));
        absoluteTolerance.push_back(1.);
        ...
        basketModels.push_back(ext::shared_ptr<DefaultLossModel>(new
            RandomDefaultLM<TCopulaPolicy>(settings, TKtLossLM, numSims))); // ???  
        absoluteTolerance.push_back(1.);
        relativeToleranceMidp.push_back(0.07);
        relativeTolerancePeriod.push_back(0.07);
        // SECOND MC
    ...
    for (Size j = 0; j < LENGTH(hwAttachment); j ++) {
        ext::shared_ptr<Basket> basketPtr (
            new Basket(settings, asofDate, names, nominals, pool,
                hwAttachment[j], hwDetachment[j]));
        ostringstream trancheId;
        trancheId << "[" << hwAttachment[j] << " , " << hwDetachment[j]
            << "]";
        SyntheticCDO cdoe(settings, basketPtr, Protection::Seller,
