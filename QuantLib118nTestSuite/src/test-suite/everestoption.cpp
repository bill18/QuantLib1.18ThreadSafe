...
void EverestOptionTest::testCached() {
    ...
    Real notional = 1.0;
    Rate guarantee = 0.0;
    EverestOption option(settings, notional, guarantee, exercise);

    Handle<YieldTermStructure> riskFreeRate(flatRate(settings, today, 0.05, dc));
    ...
    processes[0] = ext::shared_ptr<StochasticProcess1D>(
        new BlackScholesMertonProcess(settings, 
                    dummyUnderlying,
                    Handle<YieldTermStructure>(flatRate(settings, today, 0.01, dc)),
                    riskFreeRate,
                    Handle<BlackVolTermStructure>(flatVol(settings, today, 0.30, dc))));
    processes[1] = ext::shared_ptr<StochasticProcess1D>(
        new BlackScholesMertonProcess(settings, 
                    dummyUnderlying,
                    Handle<YieldTermStructure>(flatRate(settings, today, 0.05, dc)),
                    riskFreeRate,
                    Handle<BlackVolTermStructure>(flatVol(settings, today, 0.35, dc))));
    processes[2] = ext::shared_ptr<StochasticProcess1D>(
        new BlackScholesMertonProcess(settings, 
                    dummyUnderlying,
                    Handle<YieldTermStructure>(flatRate(settings, today, 0.04, dc)),
                    riskFreeRate,
                    Handle<BlackVolTermStructure>(flatVol(settings, today, 0.25, dc))));
    processes[3] = ext::shared_ptr<StochasticProcess1D>(
        new BlackScholesMertonProcess(settings, 
                    dummyUnderlying,
                    Handle<YieldTermStructure>(flatRate(settings, today, 0.03, dc)),
                    riskFreeRate,
                    Handle<BlackVolTermStructure>(flatVol(settings, today, 0.20, dc))));
    ...
    option.setPricingEngine(MakeMCEverestEngine<PseudoRandom>(settings, process)
                            .withStepsPerYear(1)
