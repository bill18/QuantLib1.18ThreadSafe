...
void GJRGARCHModelTest::testEngines() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, today, 0.05, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, today, 0.0, dayCounter));
    ...
    for (Size k = 0; k < 3; ++k) {
        ...
        ext::shared_ptr<PricingEngine> engine1 =
            MakeMCEuropeanGJRGARCHEngine<PseudoRandom>(settings, process)
            .withStepsPerYear(20)
            .withAbsoluteTolerance(0.02)
            .withSeed(1234);

        ext::shared_ptr<PricingEngine> engine2(
            new AnalyticGJRGARCHEngine(settings, ext::make_shared<GJRGARCHModel>(
                                               process)));
        for (Size i = 0; i < 2; ++i) {
            for (Size j = 0; j < 6; ++j) {
                Real x = strike[j];
...
void GJRGARCHModelTest::testDAXCalibration() {
    ...
    */
    ...
    ext::shared_ptr<PricingEngine> engine(
        new AnalyticGJRGARCHEngine(settings, ext::shared_ptr<GJRGARCHModel>(model)));
