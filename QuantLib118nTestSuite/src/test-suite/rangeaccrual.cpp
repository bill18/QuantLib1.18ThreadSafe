    ...
    struct CommonVars {
        ...
        void createYieldCurve() {
            ...
            termStructure.linkTo( ext::shared_ptr<YieldTermStructure>(
                new ZeroCurve(settings, dates, zeroRates, Actual365Fixed())));
        }
        ...
        void createVolatilityStructures() {
            ...
            ext::shared_ptr<SwapIndex> shortSwapIndexBase(new
                EuriborSwapIsdaFixA(settings, 1*Years, termStructure));
            ...
            flatSwaptionVolatilityCube1ptr(new SwaptionVolCube1(settings, 
                atmVol,
                optionTenors,
                swapTenors,
                strikeSpreads,
            ...
            flatSwaptionVolatilityCube2ptr(new SwaptionVolCube2(settings, atmVol,
                                                             optionTenors,
                                                             swapTenors,
                                                             strikeSpreads,
                                                             nullVolSpreads,
            ...
            ext::shared_ptr<SwaptionVolCube1>
            swaptionVolatilityCubeBySabrPtr(new SwaptionVolCube1(settings, 
                atmVol,
                optionTenors,
                swapTenors,
                strikeSpreads,
        ...
        void createSmileSections() {
            ...
            smilesOnExpiry.push_back(ext::shared_ptr<SmileSection>(
                new FlatSmileSection(settings, startDate, flatVol, rangeCouponDayCount)));
            Real dummyAtmLevel = 0;
            smilesOnExpiry.push_back(ext::shared_ptr<SmileSection>(new
                InterpolatedSmileSection<Linear>(settings, startDate,
                                                 strikes,
                                                 stdDevsOnExpiry,
                                                 dummyAtmLevel,
                                                 rangeCouponDayCount)));
            ...
            smilesOnPayment.push_back(ext::shared_ptr<SmileSection>(
                new FlatSmileSection(settings, endDate, flatVol, rangeCouponDayCount)));
            smilesOnPayment.push_back(ext::shared_ptr<SmileSection>(new
                InterpolatedSmileSection<Linear>(settings, endDate,
                                                 strikes,
                                                 stdDevsOnPayment,
                                                 dummyAtmLevel,
                                                 rangeCouponDayCount)));
        ...
        CommonVars() {
            ...
            // Ibor index
            iborIndex =
                ext::shared_ptr<IborIndex>(new Euribor6M(settings, termStructure));
            ...
            observationSchedule = ext::make_shared<Schedule>(settings->evaluationDate(), startDate,endDate,
...
void RangeAccrualTest::testInfiniteRange()  {
    ...
    //Coupon
    RangeAccrualFloatersCoupon coupon(vars.settings, vars.paymentDate,
                                      1.0,
                                      vars.iborIndex,
                                      vars.startDate,
                                      vars.endDate,
    ...
    for (Size z = 0; z < vars.smilesOnPayment.size(); z++) {
        for (Size i = 0; i < vars.byCallSpread.size(); i++){
            ...
            for (Size k = 1; k < 100; k++){
                ...
                RangeAccrualFloatersCoupon coupon(vars.settings,
                                            vars.paymentDate,
                                            1.,
                                            vars.iborIndex,
                                            vars.startDate,
