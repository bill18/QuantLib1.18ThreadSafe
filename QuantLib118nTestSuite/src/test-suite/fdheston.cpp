...
namespace fd_heston_test {
    ...
    class ParableLocalVolatility : public LocalVolTermStructure {
      public:
        ...
        : LocalVolTermStructure(settings,
...
void FdHestonTest::testFdmHestonVarianceMesher() {
    ...
    const ext::shared_ptr<HestonProcess> process(
        ext::make_shared<HestonProcess>(settings, 
            Handle<YieldTermStructure>(flatRate(settings, 0.02, dc)),
            Handle<YieldTermStructure>(flatRate(settings, 0.02, dc)),
    ...
    const ext::shared_ptr<LocalVolTermStructure> lVol =
        ext::make_shared<LocalConstantVol>(settings, today, 2.5, dc);
    ...
    const ext::shared_ptr<LocalVolTermStructure> leverageFct
        = ext::make_shared<ParableLocalVolatility>(settings, today, 100.0, alpha, dc);
...
void FdHestonTest::testFdmHestonBarrierVsBlackScholes() {
    ...
    ext::shared_ptr<PricingEngine> analyticEngine(
                                        new AnalyticBarrierEngine(settings, bsProcess));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        BarrierOption barrierOption(settings, values[i].barrierType, values[i].barrier,
                                    values[i].rebate, payoff, exercise);

        const Real v0 = vol->value()*vol->value();
        ext::shared_ptr<HestonProcess> hestonProcess(
             new HestonProcess(settings, rTS, qTS, spot, v0, 1.0, v0, 0.005, 0.0));

        barrierOption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdHestonBarrierEngine(settings, ext::make_shared<HestonModel>(settings, 
...
void FdHestonTest::testFdmHestonBarrier() {
    ...
    Handle<YieldTermStructure> rTS(flatRate(settings, 0.05, Actual365Fixed()));
    Handle<YieldTermStructure> qTS(flatRate(settings, 0.0 , Actual365Fixed()));

    ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(settings, rTS, qTS, s0, 0.04, 2.5, 0.04, 0.66, -0.8));

    settings->evaluationDate() = Date(28, March, 2004);
    ...
    BarrierOption barrierOption(settings, Barrier::UpOut, 135, 0.0, payoff, exercise);
...
void FdHestonTest::testFdmHestonAmerican() {
    ...
    ext::shared_ptr<PricingEngine> engine(
         new FdHestonVanillaEngine(settings, ext::make_shared<HestonModel>(settings, 
...
void FdHestonTest::testFdmHestonIkonenToivanen() {
    ...
    */
    ...
    Handle<YieldTermStructure> rTS(flatRate(settings, 0.10, Actual360()));
    Handle<YieldTermStructure> qTS(flatRate(settings, 0.0 , Actual360()));
    ...
    for (Size i=0; i < LENGTH(strikes); ++i) {
        ...
        ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, rTS, qTS, s0, 0.0625, 5, 0.16, 0.9, 0.1));
...
void FdHestonTest::testFdmHestonBlackScholes() {
    ...
    Handle<BlackVolTermStructure> volTS(
                    flatVol(settings, rTS->referenceDate(), 0.25, rTS->dayCounter()));
    ...
    for (Size i=0; i < LENGTH(strikes); ++i) {
        ...
        ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
                       new GeneralizedBlackScholesProcess(settings, s0, qTS, rTS, volTS));
        ...
        ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, rTS, qTS, s0, 0.0625, 1, 0.0625, 0.0001, 0.0));
...
void FdHestonTest::testFdmHestonEuropeanWithDividends() {
    ...
    DividendVanillaOption option(settings, payoff, exercise, dividendDates, dividends);
    ext::shared_ptr<PricingEngine> engine(
         new FdHestonVanillaEngine(settings, ext::make_shared<HestonModel>(settings, 
            ...
            for (Size j=0; j < LENGTH(tn); ++j) {
                for (Size k=0; k < LENGTH(v0); ++k) {
                    Handle<YieldTermStructure> rTS(
                        flatRate(settings, values[i].r, Actual365Fixed()));
                    Handle<YieldTermStructure> qTS( 
                        flatRate(settings, values[i].q, Actual365Fixed()));
                
                    ext::shared_ptr<HestonProcess> hestonProcess(
                        new HestonProcess(settings, rTS, qTS, s0, 
                                          v0[k], 
                                          values[i].kappa, 
                                          values[i].theta, 
                                          values[i].sigma, 
...
void FdHestonTest::testMethodOfLinesAndCN() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.0, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.0, dc));
    ...
    const ext::shared_ptr<PricingEngine> fdmDefault(
        ext::make_shared<FdHestonVanillaEngine>(settings, model, 10, xGrid, vGrid, 0));
    ...
    barrierOption.setPricingEngine(
        ext::make_shared<FdHestonBarrierEngine>(settings, model, 100, 31, 11));

    const Real expectedBarrier = barrierOption.NPV();

    barrierOption.setPricingEngine(
        ext::make_shared<FdHestonBarrierEngine>(settings, model, 100, 31, 11, 0,
...
void FdHestonTest::testSpuriousOscillations() {
    ...
    const Handle<Quote> spot(ext::make_shared<SimpleQuote>(100.0));
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.1, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.0, dc));
