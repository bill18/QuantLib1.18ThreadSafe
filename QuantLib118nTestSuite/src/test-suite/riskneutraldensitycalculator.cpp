...
void RiskNeutralDensityCalculatorTest::testDensityAgainstOptionPrices() {
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
        new BlackScholesMertonProcess(settings,
            spot, qTS, rTS,
            Handle<BlackVolTermStructure>(flatVol(settings, v, dayCounter))));
...
void RiskNeutralDensityCalculatorTest::testBSMagainstHestonRND() {
    ...
    const BSMRNDCalculator bsm(bsmProcess);
    const HestonRNDCalculator heston(settings,
...
void RiskNeutralDensityCalculatorTest::testLocalVolatilityRND() {
    ...
    const ext::shared_ptr<LocalVolRNDCalculator> constVolCalc(
        new LocalVolRNDCalculator(
            spot, rTS, qTS,
            ext::make_shared<LocalConstantVol>(settings, todaysDate, v, dayCounter),
            timeGrid, 201));
    ...
    const ext::shared_ptr<DumasParametricVolSurface> dumasVolSurface(
        new DumasParametricVolSurface(settings, b1, b2, b3, b4, b5, spot, rTS, qTS));
    ...
    for (Size i=0; i < maturities.size(); ++i) {
        ...
        const ext::shared_ptr<PricingEngine> engine(
            new FdBlackScholesVanillaEngine(settings,
...
void RiskNeutralDensityCalculatorTest::testBlackScholesWithSkew() {
    ...
    const Handle<YieldTermStructure> rTS(flatRate(settings, todaysDate, r, dc));
    const Handle<YieldTermStructure> qTS(flatRate(settings, todaysDate, q, dc));
    ...
    const Handle<BlackVolTermStructure> hestonSurface(
        ext::make_shared<HestonBlackVolSurface>(settings,
            Handle<HestonModel>(ext::make_shared<HestonModel>(settings, hestonProcess)),
    ...
    const HestonRNDCalculator hestonCalc(settings, hestonProcess);
