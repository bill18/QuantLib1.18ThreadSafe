...
namespace inflation_test {
    ...
    ext::shared_ptr<YieldTermStructure> nominalTermStructure(Settings* settings) {
        Date evaluationDate(13, August, 2007);
        return ext::shared_ptr<YieldTermStructure>(
            new FlatForward(settings, evaluationDate, 0.05, Actual360()));
    }
...
void InflationTest::testZeroIndex() {
    ...
    bool interp = false;
    ext::shared_ptr<UKRPI> iir(new UKRPI(settings, interp));
...
void InflationTest::testZeroTermStructure() {
    ...
    bool interp = false;
    ext::shared_ptr<UKRPI> iiUKRPI(new UKRPI(settings, interp, hz));
    for (Size i=0; i<LENGTH(fixData); i++) {
    ...
    ext::shared_ptr<YieldTermStructure> nominalTS = nominalTermStructure(settings);
    ...
    makeHelpers<ZeroInflationTermStructure,ZeroCouponInflationSwapHelper,
                ZeroInflationIndex>(settings, zcData, LENGTH(zcData), ii,
                                    observationLag,
                                    calendar, bdc, dc,
    ...
    Schedule testIndex = MakeSchedule(settings->evaluationDate()).from(from).to(to)
                            .withTenor(1*Months)
                            .withCalendar(UnitedKingdom())
                            .withConvention(ModifiedFollowing);
    ...
    IndexedCashFlow iicf(settings, notional,ind,baseDate,fixDate,payDate);
    ...
    ZeroCouponInflationSwap nzcis(settings, ZeroCouponInflationSwap::Payer,
                                     1000000.0,
                                     evaluationDate,
    ...
    ext::shared_ptr<PricingEngine> sppe(new DiscountingSwapEngine(settings, hTS));
    ...
    bool interpYES = true;
    ext::shared_ptr<UKRPI> iiUKRPIyes(new UKRPI(settings, interpYES, hz));
    for (Size i=0; i<LENGTH(fixData);i++) {
    ...
    makeHelpers<ZeroInflationTermStructure,ZeroCouponInflationSwapHelper,
    ZeroInflationIndex>(settings, zcData, LENGTH(zcData), iiyes,
                        observationLagyes,
                        calendar, bdc, dc,
                        Handle<YieldTermStructure>(nominalTS));
    ...
    to = hz->maxDate()-1*Months; // a bit of margin for adjustments
    testIndex = MakeSchedule(settings->evaluationDate()).from(from).to(to)
    .withTenor(1*Months)
    .withCalendar(UnitedKingdom())
    .withConvention(ModifiedFollowing);
    ...
    ZeroCouponInflationSwap nzcisyes(settings, ZeroCouponInflationSwap::Payer,
                                     1000000.0,
                                     evaluationDate,
                                     zcData[6].date,    // end date = maturity
                                     calendar, bdc, dc, zcData[6].rate/100.0, // fixed rate
...
void InflationTest::testYYIndex() {
    ...
    YYEUHICP yyeuhicp(settings, true);
    ...
    YYEUHICPr yyeuhicpr(settings, true);
    ...
    YYUKRPI yyukrpi(settings, false);
    ...
    YYUKRPIr yyukrpir(settings, false);
    ...
    bool interp = false;
    ext::shared_ptr<YYUKRPIr> iir(new YYUKRPIr(settings, interp));
    ext::shared_ptr<YYUKRPIr> iirYES(new YYUKRPIr(settings, true));
    for (Size i=0; i<LENGTH(fixData);i++) {
        iir->addFixing(rpiSchedule[i], fixData[i]);
...
void InflationTest::testYYTermStructure() {
    ...
    bool interp = false;
    ext::shared_ptr<YYUKRPIr> iir(new YYUKRPIr(settings, interp, hy));
    for (Size i=0; i<LENGTH(fixData); i++) {
        iir->addFixing(rpiSchedule[i], fixData[i]);
    }
    ...
    for (Size j = 1; j < LENGTH(yyData); j++) {
        ...
        to = yyData[j].date;
        Schedule yoySchedule = MakeSchedule(settings->evaluationDate()).from(from).to(to)
        ...
        YearOnYearInflationSwap yyS2(settings, YearOnYearInflationSwap::Payer,
                                        1000000.0,
    ...
    for (Size k = 0; k < 14; k++) {
        ...
        YearOnYearInflationSwap yyS3(settings, YearOnYearInflationSwap::Payer,
                                    1000000.0,
                                    yoySchedule,//fixed schedule, but same as yoy
