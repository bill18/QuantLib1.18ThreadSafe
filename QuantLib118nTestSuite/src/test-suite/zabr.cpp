...
void ZabrTest::testConsistency() {
    ...
    SabrSmileSection sabr(settings, tau, forward,
                          boost::assign::list_of(alpha)(beta)(nu)(rho));

    ZabrSmileSection<ZabrShortMaturityLognormal> zabr0(settings,
        tau, forward, boost::assign::list_of(alpha)(beta)(nu)(rho)(1.0));

    ZabrSmileSection<ZabrShortMaturityNormal> zabr1(settings,
        tau, forward, boost::assign::list_of(alpha)(beta)(nu)(rho)(1.0));

    ZabrSmileSection<ZabrLocalVolatility> zabr2(settings,
        tau, forward, boost::assign::list_of(alpha)(beta)(nu)(rho)(1.0));

    // for full finite prices reduce the number of intermediate points here
    // below the recommended value to speed up the test
    ZabrSmileSection<ZabrFullFd> zabr3(settings,
        tau, forward, boost::assign::list_of(alpha)(beta)(nu)(rho)(1.0),
        std::vector<Real>(), 2);
