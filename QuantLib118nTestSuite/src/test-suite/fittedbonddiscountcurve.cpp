...
void FittedBondDiscountCurveTest::testEvaluation() {
    ...
    ext::shared_ptr<Bond> bond = ext::make_shared<ZeroCouponBond>(settings, 3, TARGET(), 100.0,
                                                                  today + Period(10, Years));
    Handle<Quote> q(ext::make_shared<SimpleQuote>(100.0));

    std::vector<ext::shared_ptr<BondHelper> > helpers(1);
    helpers[0] = ext::make_shared<BondHelper>(settings, q, bond);

    ExponentialSplinesFitting fittingMethod(settings);
    ...
    FittedBondDiscountCurve curve(settings, 0, TARGET(), helpers, Actual365Fixed(),
