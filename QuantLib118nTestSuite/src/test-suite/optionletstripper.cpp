...
struct CommonVars {
        ...
        void setTermStructure() {
            ...
            yieldTermStructure.linkTo(
                ext::make_shared<FlatForward>(settings, 0,
                                                               calendar,
                                                               flatFwdRate,
                                                               dayCounter));
        }
        ...
        void setRealTermStructure() {
            ...
            discountingYTS.linkTo(
                ext::make_shared< InterpolatedZeroCurve< Linear > >(settings, 
                    dates, rates,
                                                        dayCounter, calendar));
        ...
        void setFlatTermVolCurve() {
          ...
          flatTermVolCurve = Handle<CapFloorTermVolCurve>(
              ext::make_shared<CapFloorTermVolCurve>(settings, 0, calendar, Following, optionTenors,
        ...
        void setFlatTermVolSurface() {
            ...
            flatTermVolSurface = ext::make_shared<CapFloorTermVolSurface>(settings, 0, calendar, Following,
        ...
        void setCapFloorTermVolCurve() {
          ...
          capFloorVolCurve = Handle<CapFloorTermVolCurve>(
            ext::make_shared<CapFloorTermVolCurve>(settings, 0, calendar, Following,
        ...
        void setCapFloorTermVolSurface() {
            ...
            capFloorVolSurface = ext::make_shared<CapFloorTermVolSurface>(settings, 0, calendar, Following,
                                       optionTenors, strikes,
                                       termV, dayCounter);
        }
...
void OptionletStripperTest::testFlatTermVolatilityStripping1() {
    ...
    CommonVars vars;
    vars.settings->evaluationDate() = Date(28, October, 2013);

    vars.setFlatTermVolSurface();

    ext::shared_ptr<IborIndex> iborIndex(new Euribor6M(vars.settings, vars.yieldTermStructure));

    ext::shared_ptr<OptionletStripper> optionletStripper1(new
        OptionletStripper1(vars.settings, vars.flatTermVolSurface,
                           iborIndex,
                           Null<Rate>(),
                           vars.accuracy));

    ext::shared_ptr<StrippedOptionletAdapter> strippedOptionletAdapter(new
        StrippedOptionletAdapter(vars.settings, optionletStripper1));
    ...
    ext::shared_ptr<BlackCapFloorEngine> strippedVolEngine(new
        BlackCapFloorEngine(vars.settings, vars.yieldTermStructure,
                            vol));
    ...
    for (Size tenorIndex=0; tenorIndex<vars.optionTenors.size(); ++tenorIndex) {
        for (Size strikeIndex=0; strikeIndex<vars.strikes.size(); ++strikeIndex) {
            cap = MakeCapFloor(vars.settings, CapFloor::Cap,
...
void OptionletStripperTest::testTermVolatilityStripping1() {
    ...
    ext::shared_ptr<OptionletStripper> optionletStripper1(new
        OptionletStripper1(vars.settings, vars.capFloorVolSurface,
                           iborIndex,
                           Null<Rate>(),
                           vars.accuracy));

    ext::shared_ptr<StrippedOptionletAdapter> strippedOptionletAdapter =
        ext::make_shared<StrippedOptionletAdapter>(vars.settings, optionletStripper1);
...
void OptionletStripperTest::testTermVolatilityStrippingNormalVol() {
    ...
    CommonVars vars;
    vars.settings->evaluationDate() = Date(30, April, 2015);

    vars.setRealCapFloorTermVolSurface();

    ext::shared_ptr< IborIndex > iborIndex(new Euribor6M(vars.settings, vars.forwardingYTS));

    ext::shared_ptr< OptionletStripper > optionletStripper1(
        new OptionletStripper1(vars.settings, vars.capFloorVolRealSurface, iborIndex,
    ...
    ext::shared_ptr< BachelierCapFloorEngine > strippedVolEngine(
        new BachelierCapFloorEngine(vars.settings, vars.discountingYTS, vol));
    ...
    for (Size tenorIndex = 0; tenorIndex < vars.optionTenors.size();
        ...
        for (Size strikeIndex = 0; strikeIndex < vars.strikes.size();
             ++strikeIndex) {
            cap = MakeCapFloor(vars.settings, CapFloor::Cap, vars.optionTenors[tenorIndex],
            ...
            ext::shared_ptr< PricingEngine >
                bachelierCapFloorEngineConstantVolatility(
                    new BachelierCapFloorEngine(vars.settings, 
                        vars.discountingYTS,
                        vars.termV[tenorIndex][strikeIndex]));
...
void OptionletStripperTest::testTermVolatilityStrippingShiftedLogNormalVol() {
    ...
    ext::shared_ptr< BlackCapFloorEngine > strippedVolEngine(
        new BlackCapFloorEngine(vars.settings, vars.discountingYTS, vol));
...
void OptionletStripperTest::testFlatTermVolatilityStripping2() {
  ...
  CommonVars vars;
  vars.settings->evaluationDate() = Date::todaysDate();
  ...
  ext::shared_ptr<OptionletStripper> optionletStripper2(new
        OptionletStripper2(vars.settings, optionletStripper1, vars.flatTermVolCurve));

  ext::shared_ptr<StrippedOptionletAdapter> strippedOptionletAdapter2(new
        StrippedOptionletAdapter(vars.settings, optionletStripper2));
...
void OptionletStripperTest::testTermVolatilityStripping2() {
  ...
  // optionletstripper2
  ext::shared_ptr<OptionletStripper> optionletStripper2(new
                OptionletStripper2(vars.settings, optionletStripper1,
...
void OptionletStripperTest::testSwitchStrike() {
    ...
    RelinkableHandle< YieldTermStructure > yieldTermStructure;
    yieldTermStructure.linkTo(ext::make_shared< FlatForward >(vars.settings, 
        0, vars.calendar, 0.03, vars.dayCounter));

    ext::shared_ptr< IborIndex > iborIndex(new Euribor6M(vars.settings, yieldTermStructure));

    ext::shared_ptr< OptionletStripper1 > optionletStripper1(
        new OptionletStripper1(vars.settings, vars.capFloorVolSurface, iborIndex,
