...
void BatesModelTest::testAnalyticVsBlack() {
    ...
    ext::shared_ptr<BatesProcess> process(
        new BatesProcess(settings, riskFreeTS, dividendTS, s0, v0, 
                         kappa, theta, sigma, rho, lambda, nu, delta));

    ext::shared_ptr<PricingEngine> engine(new BatesEngine(settings,
        ext::make_shared<BatesModel>(settings, process), 64));
    ...
    engine = ext::shared_ptr<PricingEngine>(new BatesDetJumpEngine(
        settings,
        ext::make_shared<BatesDetJumpModel>(settings, 
    ...
    engine = ext::shared_ptr<PricingEngine>(new BatesDoubleExpEngine(
        settings,
        ext::make_shared<BatesDoubleExpModel>(settings, 
...
void BatesModelTest::testAnalyticAndMcVsJumpDiffusion() {
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS =
        flatVol(settings, settlementDate, vol, dayCounter);
    ...
    ext::shared_ptr<BatesProcess> batesProcess(new BatesProcess(settings, 
        riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho,
        jumpIntensity->value(), meanLogJump->value(), jumpVol->value()));

    ext::shared_ptr<Merton76Process> mertonProcess(
        new Merton76Process(settings, s0, dividendTS, riskFreeTS,
    ...
    ext::shared_ptr<PricingEngine> batesEngine(new BatesEngine(
        settings,
        ext::make_shared<BatesModel>(settings, batesProcess), 160));

    const Real mcTol = 0.1;
    ext::shared_ptr<PricingEngine> mcBatesEngine =
        MakeMCEuropeanHestonEngine<PseudoRandom>(settings, batesProcess)
            .withStepsPerYear(2)
    ...
    ext::shared_ptr<PricingEngine> mertonEngine(
        new JumpDiffusionEngine(settings, mertonProcess, 1e-10, 1000));
    ...
    for (Integer i=1; i<=5; i+=2) {
        ...
        VanillaOption batesOption(settings, payoff, exercise);
        ...
        EuropeanOption mertonOption(settings, payoff, exercise);
...
void BatesModelTest::testAnalyticVsMCPricing() {
    ...
    for (Size i=0; i < LENGTH(hestonModels); ++i) { 
        Handle<YieldTermStructure> riskFreeTS(flatRate(settings, hestonModels[i].r, 
                                                       dayCounter));
        Handle<YieldTermStructure> dividendTS(flatRate(settings, hestonModels[i].q, 
                                                       dayCounter));
        Handle<Quote> s0(ext::shared_ptr<Quote>(new SimpleQuote(100)));
        ...
        ext::shared_ptr<PricingEngine> fdEngine(
                            new FdBatesVanillaEngine(settings, batesModel, 50, 100, 30));
    
        ext::shared_ptr<PricingEngine> analyticEngine(
                                             new BatesEngine(settings, batesModel, 160));
...
void BatesModelTest::testDAXCalibration() {
    ...
    */
    ...
    ext::shared_ptr<BatesModel> batesModel(new BatesModel(settings, process));

    ext::shared_ptr<PricingEngine> batesEngine(
                                            new BatesEngine(settings, batesModel, 64));
    ...
    for (Size s = 0; s < 13; ++s) {
        for (Size m = 0; m < 8; ++m) {
            ...
            options.push_back(ext::shared_ptr<BlackCalibrationHelper>(
                    new HestonModelHelper(settings, maturity, calendar,
    ...
    ext::shared_ptr<HestonProcess> hestonProcess(new HestonProcess(settings, 
                    riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho));
    ...
    pricingEngines.push_back(ext::shared_ptr<PricingEngine>(
        new BatesDoubleExpDetJumpEngine(
            settings,
            ext::make_shared<BatesDoubleExpDetJumpModel>(settings, 
                    hestonProcess, 1.0), 64)) );
