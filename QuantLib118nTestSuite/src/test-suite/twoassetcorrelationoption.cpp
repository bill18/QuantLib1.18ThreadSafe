...
void TwoAssetCorrelationOptionTest::testAnalyticEngine() {
    ...
    TwoAssetCorrelationOption option(settings, type, strike1, strike2, exercise);

    Handle<Quote> underlying1(ext::make_shared<SimpleQuote>(52.0));
    Handle<Quote> underlying2(ext::make_shared<SimpleQuote>(65.0));
    Handle<YieldTermStructure> dividendTS1(flatRate(settings, today, 0.0, dc));
    Handle<YieldTermStructure> dividendTS2(flatRate(settings, today, 0.0, dc));
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, today, 0.1, dc));
    Handle<BlackVolTermStructure> blackVolTS1(flatVol(settings, today, 0.2, dc));
    Handle<BlackVolTermStructure> blackVolTS2(flatVol(settings, today, 0.3, dc));
    Handle<Quote> correlation(ext::make_shared<SimpleQuote>(0.75));

    ext::shared_ptr<BlackScholesMertonProcess> process1 =
        ext::make_shared<BlackScholesMertonProcess>(settings, underlying1,
                                                      dividendTS1,
                                                      riskFreeTS,
                                                      blackVolTS1);

    ext::shared_ptr<BlackScholesMertonProcess> process2 =
        ext::make_shared<BlackScholesMertonProcess>(settings, underlying2,
                                                      dividendTS2,
                                                      riskFreeTS,
                                                      blackVolTS2);

    option.setPricingEngine(
          ext::make_shared<AnalyticTwoAssetCorrelationEngine>(settings, process1,
                                                                process2,
                                                                correlation));
