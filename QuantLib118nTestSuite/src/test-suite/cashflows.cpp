...
void CashFlowsTest::testSettings() {
    ...
    for (Integer i=0; i<3; ++i)
        leg.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(settings, 1.0, today+i)));
    ...
    do {                                                            \
        Real NPV = CashFlows::npv(settings, leg, no_discount, includeRef, today); \
...
void CashFlowsTest::testAccessViolation() {
    ...
    Handle<YieldTermStructure> rhTermStructure(
        flatRate(settings, settlementDate, 0.04875825, Actual365Fixed()));
    ...
    ext::shared_ptr<IborIndex> index3m (new USDLibor(settings, 3*Months,
                                                       rhTermStructure));
    ...
    Rate spread = 0.0115;
    ext::shared_ptr<IborCouponPricer> pricer(new BlackIborCouponPricer(settings, vol));
    ext::shared_ptr<FloatingRateCoupon> coupon(
        new FloatingRateCoupon(settings, payDate,100, startDate, endDate, 2,
...
void CashFlowsTest::testDefaultSettlementDate() {
    ...
    Schedule schedule =
        MakeSchedule(settings->evaluationDate())
    ...
    Leg leg = FixedRateLeg(settings, schedule)
              .withNotionals(100.0)
              .withCouponRates(0.03, Actual360())
              .withPaymentCalendar(TARGET())
              .withPaymentAdjustment(Following);

    Time accruedPeriod = CashFlows::accruedPeriod(settings, leg, false);
    if (accruedPeriod == 0.0)
        BOOST_ERROR("null accrued period with default settlement date");

    Date::serial_type accruedDays = CashFlows::accruedDays(settings, leg, false);
    if (accruedDays == 0)
        BOOST_ERROR("no accrued days with default settlement date");

    Real accruedAmount = CashFlows::accruedAmount(settings, leg, false);
...
void CashFlowsTest::testNullFixingDays() {
    ...
    ext::shared_ptr<IborIndex> index(new USDLibor(settings, 6*Months));
    Leg leg = IborLeg(settings, schedule, index)
        .withNotionals(100.0)
...
void CashFlowsTest::testPartialScheduleLegConstruction() {
    ...
    Schedule schedule = MakeSchedule(settings->evaluationDate())
    ...
    Leg leg = FixedRateLeg(settings, schedule).withNotionals(100.0).withCouponRates(
        0.01, ActualActual(ActualActual::ISMA));
    Leg leg2 = FixedRateLeg(settings, schedule2).withNotionals(100.0).withCouponRates(
        0.01, ActualActual(ActualActual::ISMA));
    Leg leg3 = FixedRateLeg(settings, schedule3).withNotionals(100.0).withCouponRates(
        0.01, ActualActual(ActualActual::ISMA));
    ...
    ext::shared_ptr<IborIndex> iborIndex =
        ext::make_shared<USDLibor>(settings, 3 * Months);
    Leg legf = IborLeg(settings, schedule, iborIndex)
                   .withNotionals(100.0)
                   .withPaymentDayCounter(ActualActual(ActualActual::ISMA));
    Leg legf2 = IborLeg(settings, schedule2, iborIndex)
                    .withNotionals(100.0)
                    .withPaymentDayCounter(ActualActual(ActualActual::ISMA));
    Leg legf3 = IborLeg(settings, schedule3, iborIndex)
                    .withNotionals(100.0)
                    .withPaymentDayCounter(ActualActual(ActualActual::ISMA));
