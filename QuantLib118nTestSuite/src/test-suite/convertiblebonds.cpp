...
namespace convertible_bonds_test {
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            dividendYield.linkTo(flatRate(settings, today, 0.02, dayCounter));
            riskFreeRate.linkTo(flatRate(settings, today, 0.05, dayCounter));
            volatility.linkTo(flatVol(settings, today, 0.15, dayCounter));

            process = ext::make_shared<BlackScholesMertonProcess>(settings, 
...
void ConvertibleBondTest::testBond() {
    ...
    ext::shared_ptr<PricingEngine> engine =
        ext::make_shared<BinomialConvertibleEngine<CoxRossRubinstein> >(vars.settings,
            vars.process, timeSteps);

    Handle<YieldTermStructure> discountCurve(
         ext::make_shared<ForwardSpreadedTermStructure>(vars.settings,vars.riskFreeRate,
                                                          vars.creditSpread));

    // zero-coupon

    Schedule schedule =
        MakeSchedule(vars.settings->evaluationDate()).from(vars.issueDate)
                      .to(vars.maturityDate)
                      .withFrequency(Once)
                      .withCalendar(vars.calendar)
                      .backwards();

    ConvertibleZeroCouponBond euZero(vars.settings, euExercise, vars.conversionRatio,
    ...
    ConvertibleZeroCouponBond amZero(vars.settings, amExercise, vars.conversionRatio,
                                     vars.no_dividends, vars.no_callability,
                                     vars.creditSpread,
                                     vars.issueDate, vars.settlementDays,
                                     vars.dayCounter, schedule,
    ...
    ZeroCouponBond zero(vars.settings, vars.settlementDays, vars.calendar,
                        100.0, vars.maturityDate,
                        Following, vars.redemption, vars.issueDate);

    ext::shared_ptr<PricingEngine> bondEngine =
        ext::make_shared<DiscountingBondEngine>(vars.settings, discountCurve);
    ...
    schedule = MakeSchedule(vars.settings->evaluationDate()).from(vars.issueDate)
                             .to(vars.maturityDate)
                             .withFrequency(vars.frequency)
                             .withCalendar(vars.calendar)
                             .backwards();

    ConvertibleFixedCouponBond euFixed(vars.settings, euExercise, vars.conversionRatio,
                                       vars.no_dividends, vars.no_callability,
                                       vars.creditSpread,
                                       vars.issueDate, vars.settlementDays,
    ...
    ConvertibleFixedCouponBond amFixed(vars.settings, amExercise, vars.conversionRatio,
                                       vars.no_dividends, vars.no_callability,
                                       vars.creditSpread,
                                       vars.issueDate, vars.settlementDays,
                                       coupons, vars.dayCounter,
    ...
    FixedRateBond fixed(vars.settings, vars.settlementDays, vars.faceAmount, schedule,
    ...
    ext::shared_ptr<IborIndex> index =
        ext::make_shared<Euribor1Y>(vars.settings, discountCurve);
    Natural fixingDays = 2;
    std::vector<Real> gearings(1, 1.0);
    std::vector<Rate> spreads;

    ConvertibleFloatingRateBond euFloating(vars.settings, euExercise, vars.conversionRatio,
                                           vars.no_dividends, vars.no_callability,
                                           vars.creditSpread,
                                           vars.issueDate, vars.settlementDays,
    ...
    ConvertibleFloatingRateBond amFloating(vars.settings, amExercise, vars.conversionRatio,
                                           vars.no_dividends, vars.no_callability,
                                           vars.creditSpread,
                                           vars.issueDate, vars.settlementDays,
                                           index, fixingDays, spreads,
    ...
    ext::shared_ptr<IborCouponPricer> pricer =
        ext::make_shared<BlackIborCouponPricer>(vars.settings,
            Handle<OptionletVolatilityStructure>());

    Schedule floatSchedule(vars.settings->evaluationDate(), vars.issueDate, vars.maturityDate,
                           Period(vars.frequency),
                           vars.calendar, Following, Following,
                           DateGeneration::Backward, false);

    FloatingRateBond floating(vars.settings, vars.settlementDays, vars.faceAmount, floatSchedule,
...
void ConvertibleBondTest::testOption() {
    ...
    ext::shared_ptr<PricingEngine> vanillaEngine =
        ext::make_shared<BinomialVanillaEngine<CoxRossRubinstein> >(vars.settings,
                                                     vars.process, timeSteps);
    ...
    Schedule schedule = MakeSchedule(vars.settings->evaluationDate()).from(vars.issueDate)
                                      .to(vars.maturityDate)
                                      .withFrequency(Once)
                                      .withCalendar(vars.calendar)
                                      .backwards();
    ...
    VanillaOption euOption(vars.settings, payoff, euExercise);
...
void ConvertibleBondTest::testRegression() {
    ...
    vars.settings->evaluationDate() = tomorrow;
    ...
    Handle<YieldTermStructure> r(
              ext::make_shared<ForwardCurve>(vars.settings, dates, forwards, Actual360()));

    Handle<BlackVolTermStructure> sigma(ext::make_shared<BlackConstantVol>(vars.settings, 
                                 tomorrow, NullCalendar(), 21.685235548092248,
                                 Thirty360(Thirty360::BondBasis)));

    ext::shared_ptr<BlackProcess> process =
        ext::make_shared<BlackProcess>(vars.settings, u,r,sigma);
    ...
    Schedule schedule = MakeSchedule(vars.settings->evaluationDate()).from(issueDate)
                                      .to(maturityDate)
                                      .withTenor(6*Months)
    ...
    ConvertibleFixedCouponBond bond(vars.settings, exercise, conversionRatio,
