...
void ForwardOptionTest::testValues() {
    ...
    */
    ...
    ext::shared_ptr<PricingEngine> engine(
              new ForwardVanillaEngine<AnalyticEuropeanEngine>(settings, stochProcess));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ForwardVanillaOption option(settings, values[i].moneyness, reset,
                                    payoff, exercise);
...
void ForwardOptionTest::testPerformanceValues() {
    ...
    */
    ...
    ext::shared_ptr<PricingEngine> engine(
        new ForwardPerformanceVanillaEngine<AnalyticEuropeanEngine>(settings,
                                                               stochProcess));
...
namespace {
    ...
    void testForwardGreeks(Settings* settings) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                            new Engine<AnalyticEuropeanEngine>(settings, stochProcess));
          ...
          for (Size j=0; j<LENGTH(moneyness); j++) {
            for (Size k=0; k<LENGTH(lengths); k++) {
              for (Size h=0; h<LENGTH(startMonths); h++) {
                ...
                ForwardVanillaOption option(settings, moneyness[j], reset,
                                            payoff, exercise);
                option.setPricingEngine(engine);
...
void ForwardOptionTest::testGreeks() {
    ...
    testForwardGreeks<ForwardVanillaEngine>(&settings);
}
...
void ForwardOptionTest::testPerformanceGreeks() {
    ...
    testForwardGreeks<ForwardPerformanceVanillaEngine>(&settings);
}
...
public:
   ...
   : BinomialVanillaEngine<CoxRossRubinstein>(settings, process, 300) // fixed steps
    {}
};
...
//   //SavedSettings backup;
   ...
   ext::shared_ptr<PricingEngine> engine(
                        new ForwardVanillaEngine<TestBinomialEngine>(settings, stochProcess));
   ...
   ForwardVanillaOption option(settings, 0.9, reset, payoff, exercise);
   option.setPricingEngine(engine);

   ext::shared_ptr<PricingEngine> ctrlengine(
                        new TestBinomialEngine(settings, stochProcess));
   VanillaOption ctrloption(settings, payoff, exercise);
