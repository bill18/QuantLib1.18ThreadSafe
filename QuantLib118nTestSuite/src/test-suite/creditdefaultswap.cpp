...
void CreditDefaultSwapTest::testCachedValue() {
    ...
    // Initialize curves
    settings->evaluationDate() = Date(9,June,2006);
    ...
    probabilityCurve.linkTo(
        ext::shared_ptr<DefaultProbabilityTermStructure>(
                   new FlatHazardRate(settings, 0, calendar, hazardRate, Actual360())));

    RelinkableHandle<YieldTermStructure> discountCurve;

    discountCurve.linkTo(ext::shared_ptr<YieldTermStructure>(
                            new FlatForward(settings, today,0.06,Actual360())));
    ...
    Schedule schedule(settings->evaluationDate(), issueDate, maturity, Period(frequency), calendar,
    ...
    CreditDefaultSwap cds(settings, Protection::Seller, notional, fixedRate,
                          schedule, convention, dayCount, true, true);
    cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
         new MidPointCdsEngine(settings, probabilityCurve,recoveryRate,discountCurve)));
    ...
    cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
                          new IntegralCdsEngine(settings, 1*Days,probabilityCurve,
    ...
    cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
                          new IntegralCdsEngine(settings, 1*Weeks,probabilityCurve,
                                                recoveryRate,discountCurve)));
...
void CreditDefaultSwapTest::testCachedMarketValue() {
    ...
    discountCurve.linkTo(
        ext::shared_ptr<YieldTermStructure>(
            new DiscountCurve(settings, discountDates, dfs, curveDayCounter)));
    ...
    piecewiseFlatHazardRate.linkTo(
        ext::shared_ptr<DefaultProbabilityTermStructure>(
               new InterpolatedHazardRateCurve<BackwardFlat>(settings, dates,
                                                             hazardRates,
                                                             Thirty360())));
    ...
    Schedule schedule(settings->evaluationDate(), issueDate, maturity, Period(cdsFrequency), calendar,
    ...
    CreditDefaultSwap cds(settings, Protection::Seller, cdsNotional, fixedRate,
                          schedule, cdsConvention, dayCount, true, true);
    cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
                          new MidPointCdsEngine(settings, piecewiseFlatHazardRate,
                                                recoveryRate,discountCurve)));
...
void CreditDefaultSwapTest::testImpliedHazardRate() {
    ...
    RelinkableHandle<YieldTermStructure> discountCurve;
    discountCurve.linkTo(ext::shared_ptr<YieldTermStructure>(
                            new FlatForward(settings, today,0.03,Actual360())));
    ...
    for (Integer n=6; n<=10; ++n) {
        ...
        CreditDefaultSwap cds(settings, Protection::Seller, notional, fixedRate,
                              ...
                              true, true);
        cds.setPricingEngine(ext::shared_ptr<PricingEngine>(
                         new MidPointCdsEngine(settings, probabilityCurve,
                                               recoveryRate, discountCurve)));
        ...
        probability.linkTo(ext::shared_ptr<DefaultProbabilityTermStructure>(
         new FlatHazardRate(settings, 
           today,
           Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(flatRate))),
           dayCounter)));

        CreditDefaultSwap cds2(settings, Protection::Seller, notional, fixedRate,
                               schedule, convention, cdsDayCount,
                               true, true);
        cds2.setPricingEngine(ext::shared_ptr<PricingEngine>(
                               new MidPointCdsEngine(settings, probability,recoveryRate,
                                                     discountCurve)));
...
void CreditDefaultSwapTest::testFairSpread() {
    ...
    Schedule schedule =
        MakeSchedule(settings->evaluationDate()).from(issueDate)
                      .to(maturity)
    ...
    CreditDefaultSwap fairCds(settings, Protection::Seller, notional, fairRate,
                              schedule, convention, dayCount, true, true);
...
void CreditDefaultSwapTest::testFairUpfront() {
    ...
    ext::shared_ptr<PricingEngine> engine(
          new MidPointCdsEngine(settings, probabilityCurve, recoveryRate,
                                discountCurve, true));

    CreditDefaultSwap cds(settings, Protection::Seller, notional, upfront, fixedRate,
                          schedule, convention, dayCount, true, true);
    cds.setPricingEngine(engine);

    Rate fairUpfront = cds.fairUpfront();

    CreditDefaultSwap fairCds(settings, Protection::Seller, notional,
    ...
    upfront = 0.0;
    CreditDefaultSwap cds2(settings, Protection::Seller, notional, upfront, fixedRate,
                           schedule, convention, dayCount, true, true);
    cds2.setPricingEngine(engine);

    fairUpfront = cds2.fairUpfront();

    CreditDefaultSwap fairCds2(settings, Protection::Seller, notional,
                               fairUpfront, fixedRate,
                               schedule, convention, dayCount, true, true);
...
void CreditDefaultSwapTest::testIsdaEngine() {
    ...
    for(size_t i = 0; i < sizeof(dep_tenors) / sizeof(int); i++) {
        isdaRateHelpers.push_back(ext::make_shared<DepositRateHelper>(settings, 
    ...
    ext::shared_ptr<IborIndex> isda_ibor = ext::make_shared<IborIndex>(settings, 
        "IsdaIbor", 3 * Months, 2, USDCurrency(), WeekendsOnly(),
        ModifiedFollowing, false, Actual360());
    for(size_t i = 0; i < sizeof(swap_tenors) / sizeof(int); i++) {
        isdaRateHelpers.push_back(ext::make_shared<SwapRateHelper>(settings, 
    ...
    for(size_t i = 0; i < sizeof(termDates) / sizeof(Date); i++) {
        for(size_t j = 0; j < 2; j++) {
            ...
            ext::shared_ptr<CreditDefaultSwap> quotedTrade =
                MakeCreditDefaultSwap(settings, termDates[i], spreads[j])
            ...
            probabilityCurve.linkTo(
                ext::make_shared<FlatHazardRate>(settings, 
            ...
            ext::shared_ptr<CreditDefaultSwap> conventionalTrade =
                MakeCreditDefaultSwap(settings, termDates[i], 0.01)
                .withNominal(10000000.)
