...
namespace {
    ...
    CalibrationMarketData getDAXCalibrationMarketData(Settings* settings) {
        ...
        */
        ...
        Handle<YieldTermStructure> riskFreeTS(
			ext::make_shared<ZeroCurve>(settings, dates, rates, dayCounter));
        
        Handle<YieldTermStructure> dividendYield(
                                    flatRate(settings, settlementDate, 0.0, dayCounter));
        ...
        for (Size s = 0; s < 13; ++s) {
            for (Size m = 0; m < 8; ++m) {
                ...
                options.push_back(ext::make_shared<HestonModelHelper>(settings, maturity, calendar,
                                              s0, strike[s], vol,
...
void HestonModelTest::testBlackCalibration() {
    ...
    Settings* settings = &env; // for convenience
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.04, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.50, dayCounter));
    ...
    for (Size i = 0; i < optionMaturities.size(); ++i) {
        ...
        options.push_back(ext::make_shared<HestonModelHelper>(settings, optionMaturities[i], calendar,
                                                s0, strikePrice, vol,
    ...
    for (Real sigma = 0.1; sigma < 0.7; sigma += 0.2) {
        ...
        ext::shared_ptr<HestonProcess> process(
			ext::make_shared<HestonProcess>(settings, riskFreeTS, dividendTS,
                              s0, v0, kappa, theta, sigma, rho));

        ext::shared_ptr<HestonModel> model(ext::make_shared<HestonModel>(settings, process));
        ext::shared_ptr<PricingEngine> engine( 
			ext::make_shared<AnalyticHestonEngine>(settings, model, 96));
...
void HestonModelTest::testDAXCalibration() {
    ...
    settings->evaluationDate() = settlementDate;

    CalibrationMarketData marketData = getDAXCalibrationMarketData(settings);
    ...
    const ext::shared_ptr<HestonProcess> process(
        ext::make_shared<HestonProcess>(settings, 
            riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho));

    const ext::shared_ptr<HestonModel> model(
        ext::make_shared<HestonModel>(settings, process));

    const ext::shared_ptr<PricingEngine> engines[] = {
        ext::make_shared<AnalyticHestonEngine>(settings, model, 64),
        ext::make_shared<COSHestonEngine>(settings, model, 12, 75)
    };
...
void HestonModelTest::testAnalyticVsBlack() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.1, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.04, dayCounter));
    ...
    ext::shared_ptr<HestonProcess> process(ext::make_shared<HestonProcess>(settings,
                   riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho));

    VanillaOption option(settings, payoff, exercise);
    // FLOATING_POINT_EXCEPTION
    ext::shared_ptr<PricingEngine> engine(
		ext::make_shared<AnalyticHestonEngine>(settings,
			ext::make_shared<HestonModel>(settings, process), 144));
    ...
    engine = 
		ext::make_shared<FdHestonVanillaEngine>(settings,
			ext::make_shared<HestonModel>(settings, process),
              200,200,100);
    option.setPricingEngine(engine);
...
void HestonModelTest::testAnalyticVsCached() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.0225, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.02, dayCounter));
    ...
    ext::shared_ptr<AnalyticHestonEngine> engine(
		ext::make_shared<AnalyticHestonEngine>(settings, 
			ext::make_shared<HestonModel>(settings, process), 64));
    ...
    for (i = 0; i < 6; ++i) {
        ...
        Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.05, dayCounter));
        Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.02, dayCounter));
...
void HestonModelTest::testMcVsCached() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.7, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.4, dayCounter));
    ...
    ext::shared_ptr<PricingEngine> engine;
    engine = MakeMCEuropeanHestonEngine<PseudoRandom>(settings, process)
        .withStepsPerYear(11)
...
void HestonModelTest::testFdBarrierVsCached() {
    ...
    Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.08, dc));
    Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.04, dc));
    ...
    ext::shared_ptr<PricingEngine> engine;
    engine = ext::make_shared<FdHestonBarrierEngine>(settings,
				ext::make_shared<HestonModel>(settings, process),
                    200,400,100);

    BarrierOption option(settings, Barrier::DownOut, 95.0, 3.0, payoff, exercise);
    option.setPricingEngine(engine);
    ...
    option = BarrierOption(settings, Barrier::DownIn, 95.0, 3.0, payoff, exercise);
    option.setPricingEngine(engine);
...
void HestonModelTest::testFdVanillaVsCached() {
    ...
    option.setPricingEngine(
        MakeFdHestonVanillaEngine(settings, ext::make_shared<HestonModel>(settings, process))
            .withTGrid(100)
            .withXGrid(200)
            .withVGrid(100)
        );
    ...
    riskFreeTS = Handle<YieldTermStructure>(flatRate(settings, 0.05, dayCounter));
    dividendTS = Handle<YieldTermStructure>(flatRate(settings, 0.0, dayCounter));
    ...
    DividendVanillaOption divOption(settings, payoff, exercise,
                                    dividendDates, dividends);
    process = ext::make_shared<HestonProcess>(settings,
    ...
    dividendTS = Handle<YieldTermStructure>(flatRate(settings, 0.03, dayCounter));
    process = ext::make_shared<HestonProcess>(settings, 
                   riskFreeTS, dividendTS, s0, 0.04, 1.0, 0.04, 0.001, 0.0);
    ...
    option = VanillaOption(settings, payoff, exercise);
    option.setPricingEngine(
        MakeFdHestonVanillaEngine(settings, ext::make_shared<HestonModel>(settings, process))
            .withTGrid(200)
            .withXGrid(400)
    ...
    Handle<BlackVolTermStructure> volTS(flatVol(settings, settlementDate, 0.2,
                                                  dayCounter));
    ext::shared_ptr<BlackScholesMertonProcess> ref_process(
		ext::make_shared<BlackScholesMertonProcess>(settings, s0, dividendTS, riskFreeTS, volTS));
    ext::shared_ptr<PricingEngine> ref_engine(
		ext::make_shared<FdBlackScholesVanillaEngine>(settings, ref_process, 200, 400));
...
void HestonModelTest::testKahlJaeckelCase() {
    ...
    */
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.0, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.0, dayCounter));
    ...
    for (Size i=0; i < LENGTH(descriptions); ++i) {
        const ext::shared_ptr<HestonProcess> process(
			ext::make_shared<HestonProcess>(settings, riskFreeTS, dividendTS, s0, v0,
                              kappa, theta, sigma, rho,
                              descriptions[i].discretization));

        const ext::shared_ptr<PricingEngine> engine =
            MakeMCEuropeanHestonEngine<PseudoRandom>(settings, process)
    ...
    option.setPricingEngine(
        MakeMCEuropeanHestonEngine<LowDiscrepancy>(settings, 
			ext::make_shared<HestonProcess>(settings, 
                    riskFreeTS, dividendTS, s0, v0, kappa, theta, sigma, rho,
    ...
    const ext::shared_ptr<HestonModel> hestonModel(
		ext::make_shared<HestonModel>(settings, 
			ext::make_shared<HestonProcess>(settings, 
                riskFreeTS, dividendTS, s0, v0,
                kappa, theta, sigma, rho)));

    option.setPricingEngine(
		ext::make_shared<FdHestonVanillaEngine>(settings, hestonModel, 200, 401, 101));
    ...
    option.setPricingEngine(
		ext::make_shared<AnalyticHestonEngine>(settings, hestonModel, 1e-6, 1000));
    ...
    option.setPricingEngine(
        ext::make_shared<COSHestonEngine>(settings, hestonModel, 16, 400));
    calculated = option.NPV();
    error = std::fabs(calculated - expected);
...
void HestonModelTest::testDifferentIntegrals() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.05, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.03, dayCounter));
    ...
    for (std::vector<HestonParameter>::const_iterator iter = params.begin();
        ...
        ext::shared_ptr<AnalyticHestonEngine> lobattoEngine(
			ext::make_shared<AnalyticHestonEngine>(settings, model, 1e-10,
                                                       1000000));
        ext::shared_ptr<AnalyticHestonEngine> laguerreEngine(
			ext::make_shared<AnalyticHestonEngine>(settings, model, 128));
...
void HestonModelTest::testMultipleStrikesEngine() {
    ...
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.06, dayCounter));
    Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.02, dayCounter));
    ...
    ext::shared_ptr<FdHestonVanillaEngine> singleStrikeEngine(
		ext::make_shared<FdHestonVanillaEngine>(settings, model, 20, 400, 50));
    ...
    for (Size i=0; i < strikes.size(); ++i) {
        ...
        VanillaOption aOption(settings, payoff, exercise);
...
void HestonModelTest::testAnalyticPiecewiseTimeDependent() {
    ...
    Handle<YieldTermStructure> riskFreeTS(
		ext::make_shared<ZeroCurve>(settings, dates, irates, dayCounter));

    std::vector<Rate> qrates;
    qrates.push_back(0.0); qrates.push_back(0.3);
    Handle<YieldTermStructure> dividendTS( 
		ext::make_shared<ZeroCurve>(settings, dates, qrates, dayCounter));
    ...
    ext::shared_ptr<HestonModel> hestonModel =
        ext::make_shared<HestonModel>(settings, hestonProcess);
    option.setPricingEngine(
        ext::make_shared<AnalyticHestonEngine>(settings, hestonModel));
    
    const Real expected = option.NPV();

    option.setPricingEngine(ext::shared_ptr<PricingEngine>(
         new AnalyticPTDHestonEngine(settings, model)));
    ...
    option.setPricingEngine(ext::shared_ptr<PricingEngine>(
         new AnalyticPTDHestonEngine(settings, 
             model,
...
void HestonModelTest::testDAXCalibrationOfTimeDependentModel() {
    ...
    const ext::shared_ptr<PricingEngine> engines[] = {
        ext::make_shared<AnalyticPTDHestonEngine>(settings, model),
        ext::make_shared<AnalyticPTDHestonEngine>(settings, 
            model,
            AnalyticPTDHestonEngine::AndersenPiterbarg,
...
void HestonModelTest::testAlanLewisReferencePrices() {
    ...
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.01, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.02, dayCounter));
    ...
    const ext::shared_ptr<PricingEngine> laguerreEngine(
		ext::make_shared<AnalyticHestonEngine>(settings, model, 128u));

    const ext::shared_ptr<PricingEngine> gaussLobattoEngine(
		ext::make_shared<AnalyticHestonEngine>(settings, model, QL_EPSILON, 100000u));

    const ext::shared_ptr<PricingEngine> cosEngine(
		ext::make_shared<COSHestonEngine>(settings, model, 20, 400));

    const ext::shared_ptr<PricingEngine> andersenPiterbargEngine(
        new AnalyticHestonEngine(settings, 
            model,
...
void HestonModelTest::testAnalyticPDFHestonEngine() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.07, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.185, dayCounter));
    ...
    const ext::shared_ptr<AnalyticPDFHestonEngine> pdfEngine(
		ext::make_shared<AnalyticPDFHestonEngine>(settings, model, tol));

    const ext::shared_ptr<PricingEngine> analyticEngine(
		ext::make_shared<AnalyticHestonEngine>(settings, model, 178));
    ...
    for (Real strike=40; strike < 190; strike+=20) {
        ...
        VanillaOption planVanillaOption(settings, vanillaPayoff, exercise);
    ...
    for (Real strike=40; strike < 190; strike+=10) {
        VanillaOption digitalOption(settings, 
...
void HestonModelTest::testExpansionOnAlanLewisReference() {
    ...
    const ext::shared_ptr<PricingEngine> lpp2Engine =
        ext::make_shared<HestonExpansionEngine>(settings, model,
...
void HestonModelTest::testAllIntegrationMethods() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.05, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.075, dayCounter));
    ...
    reportOnIntegrationMethodTest(settings, option, model,
...
void HestonModelTest::testCosHestonCumulants() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.15, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.075, dayCounter));
    ...
    const ext::shared_ptr<COSHestonEngine> cosEngine =
        ext::make_shared<COSHestonEngine>(settings, model);
...
void HestonModelTest::testCosHestonEngine() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.15, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.07, dayCounter));
    ...
    const ext::shared_ptr<PricingEngine> cosEngine(
        ext::make_shared<COSHestonEngine>(settings, model, 25, 600));
    ...
    for (Size i=0; i < LENGTH(payoffs); ++i) {
        VanillaOption option(settings, payoffs[i], exercise);
...
void HestonModelTest::testCharacteristicFct() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, .35, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.17, dayCounter));
    ...
    const COSHestonEngine cosEngine(settings, model);
    const AnalyticHestonEngine analyticEngine(settings, model);
...
void HestonModelTest::testAndersenPiterbargPricing() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> riskFreeTS(flatRate(settings, 0.10, dayCounter));
    const Handle<YieldTermStructure> dividendTS(flatRate(settings, 0.06, dayCounter));
...
void HestonModelTest::testAndersenPiterbargControlVariateIntegrand() {
    ...
    const Rate r = 0.075;
    const Rate q = 0.05;
    const Handle<YieldTermStructure> rTS(flatRate(settings, r, dayCounter));
    const Handle<YieldTermStructure> qTS(flatRate(settings, q, dayCounter));
    ...
    const ext::shared_ptr<COSHestonEngine> cosEngine( 
        ext::make_shared<COSHestonEngine>(settings, hestonModel));
...
void HestonModelTest::testAndersenPiterbargConvergence() {
    ...
    const DayCounter dayCounter = Actual365Fixed();
    const Handle<YieldTermStructure> rTS(flatRate(settings, 0.01, dayCounter));
    const Handle<YieldTermStructure> qTS(flatRate(settings, 0.02, dayCounter));
    ...
    for (Size n=10; n <= 50; n+=10) {
        option.setPricingEngine(ext::make_shared<AnalyticHestonEngine>(settings, 
...
void HestonModelTest::testPiecewiseTimeDependentComparison() {
    ...
    const Handle<YieldTermStructure> rTS(flatRate(settings, 0.05, dc));
    const Handle<YieldTermStructure> qTS(flatRate(settings, 0.08, dc));
    ...
    const ext::shared_ptr<AnalyticPTDHestonEngine> ptdHestonEngine( 
        ext::make_shared<AnalyticPTDHestonEngine>(settings, ptdModel));
...
void HestonModelTest::testPiecewiseTimeDependentChFAsymtotic() {
    ...
    const DayCounter dc = Actual365Fixed();
    const Time maturity = dc.yearFraction(settlementDate, maturityDate);
    const Handle<YieldTermStructure> rTS(flatRate(settings, 0.0, dc));
