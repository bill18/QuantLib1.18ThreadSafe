...
void ExchangeRateTest::testDirect() {
    ...
    ExchangeRate eur_usd = ExchangeRate(settings->evaluationDate(), EUR, USD, 1.2042);

    Money m1(settings->evaluationDate(), 50000.0  , EUR);
    Money m2(settings->evaluationDate(), 100000.0 , USD);

    Money::conversionType = Money::NoConversion;

    Money calculated = eur_usd.exchange(m1);
    Money expected(settings->evaluationDate(), m1.value()*eur_usd.rate(), USD);
    ...
    expected = Money(settings->evaluationDate(), m2.value()/eur_usd.rate(), EUR);
...
void ExchangeRateTest::testDerived() {
    ...
    ExchangeRate eur_usd = ExchangeRate(settings->evaluationDate(), EUR, USD, 1.2042);
    ExchangeRate eur_gbp = ExchangeRate(settings->evaluationDate(), EUR, GBP, 0.6612);

    ExchangeRate derived = ExchangeRate::chain(eur_usd, eur_gbp);

    Money m1(settings->evaluationDate(), 50000.0 , GBP);
    Money m2(settings->evaluationDate(), 100000.0, USD);

    Money::conversionType = Money::NoConversion;

    Money calculated = derived.exchange(m1);
    Money expected(settings->evaluationDate(), m1.value()*eur_usd.rate()/eur_gbp.rate(), USD);
    ...
    expected = Money(settings->evaluationDate(), m2.value()*eur_gbp.rate()/eur_usd.rate(), GBP);
...
void ExchangeRateTest::testDirectLookup() {
    ...
    ExchangeRate eur_usd1 = ExchangeRate(settings->evaluationDate(), EUR, USD, 1.1983);
    ExchangeRate eur_usd2 = ExchangeRate(settings->evaluationDate(), USD, EUR, 1.0/1.2042);
    ...
    ExchangeRate eur_usd = rateManager.lookup(EUR, USD,
                                              ...
                                              ExchangeRate::Direct);
    Money calculated = eur_usd.exchange(m1);
    Money expected(settings->evaluationDate(), m1.value()*eur_usd1.rate(), USD);
    ...
    expected = Money(settings->evaluationDate(), m1.value()/eur_usd2.rate(), USD);
    ...
    expected = Money(settings->evaluationDate(), m2.value()/eur_usd1.rate(), EUR);
    ...
    calculated = usd_eur.exchange(m2);
    expected = Money(settings->evaluationDate(), m2.value()*eur_usd2.rate(), EUR);
...
void ExchangeRateTest::testTriangulatedLookup() {
    ...
    ExchangeRate eur_usd1 = ExchangeRate(settings->evaluationDate(), EUR, USD, 1.1983);
    ExchangeRate eur_usd2 = ExchangeRate(settings->evaluationDate(), EUR, USD, 1.2042);
    rateManager.add(eur_usd1, Date(4,August,2004));
    rateManager.add(eur_usd2, Date(5,August,2004));

    Money m1(settings->evaluationDate(), 50000000.0 , ITL);
    Money m2(settings->evaluationDate(), 100000.0    , USD);
    ...
    Money expected(settings->evaluationDate(), m1.value()*eur_usd1.rate()/1936.27, USD);
    ...
    expected = Money(settings->evaluationDate(), m1.value()*eur_usd2.rate()/1936.27, USD);
    ...
    expected = Money(settings->evaluationDate(), m2.value()*1936.27/eur_usd1.rate(), ITL);
    ...
    calculated = usd_itl.exchange(m2);
    expected = Money(settings->evaluationDate(), m2.value()*1936.27/eur_usd2.rate(), ITL);
...
void ExchangeRateTest::testSmartLookup() {
    ...
    ExchangeRate eur_gbp1 = ExchangeRate(settings->evaluationDate(), GBP, EUR, 1.0/0.6596);
    ExchangeRate eur_gbp2 = ExchangeRate(settings->evaluationDate(), EUR, GBP, 0.6612);
    rateManager.add(eur_gbp1, Date(4,August,2004));
    rateManager.add(eur_gbp2, Date(5,August,2004));

    ExchangeRate usd_chf1 = ExchangeRate(settings->evaluationDate(),USD, CHF, 1.2847);
    ExchangeRate usd_chf2 = ExchangeRate(settings->evaluationDate(),CHF, USD, 1.0/1.2774);
    rateManager.add(usd_chf1, Date(4,August,2004));
    rateManager.add(usd_chf2, Date(5,August,2004));

    ExchangeRate chf_sek1 = ExchangeRate(settings->evaluationDate(),SEK, CHF, 0.1674);
    ExchangeRate chf_sek2 = ExchangeRate(settings->evaluationDate(),CHF, SEK, 1.0/0.1677);
    rateManager.add(chf_sek1, Date(4,August,2004));
    rateManager.add(chf_sek2, Date(5,August,2004));

    ExchangeRate jpy_sek1 = ExchangeRate(settings->evaluationDate(),SEK, JPY, 14.5450);
    ExchangeRate jpy_sek2 = ExchangeRate(settings->evaluationDate(),JPY, SEK, 1.0/14.6110);
    rateManager.add(jpy_sek1, Date(4,August,2004));
    rateManager.add(jpy_sek2, Date(5,August,2004));

    Money m1(settings->evaluationDate(), 100000.0 , USD);
    Money m2(settings->evaluationDate(), 100000.0 , EUR);
    Money m3(settings->evaluationDate(), 100000.0 , GBP);
    Money m4(settings->evaluationDate(), 100000.0 , CHF);
    Money m5(settings->evaluationDate(), 100000.0 , SEK);
    Money m6(settings->evaluationDate(), 100000.0 , JPY);
    ...
    Money expected(settings->evaluationDate(), m1.value()*usd_chf1.rate()/chf_sek1.rate(), SEK);
    ...
    expected = Money(settings->evaluationDate(), m5.value()*usd_chf2.rate()/chf_sek2.rate(), USD);
    ...
    expected = Money(settings->evaluationDate(), m2.value()*eur_usd1.rate()
    ...
    expected = Money(settings->evaluationDate(), m5.value()*eur_usd2.rate()
    ...
    expected = Money(settings->evaluationDate(), m2.value()*eur_usd1.rate()*usd_chf1.rate()
    ...
    expected = Money(settings->evaluationDate(), m6.value()*jpy_sek2.rate()*eur_usd2.rate()
                               *usd_chf2.rate()/chf_sek2.rate(), EUR);
    ...
    expected = Money(settings->evaluationDate(), m3.value()*eur_gbp1.rate()*eur_usd1.rate()*usd_chf1.rate()
                               *jpy_sek1.rate()/chf_sek1.rate(), JPY);
    ...
    expected = Money(settings->evaluationDate(), m6.value()*jpy_sek2.rate()*eur_usd2.rate()*usd_chf2.rate()
