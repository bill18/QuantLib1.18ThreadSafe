...
namespace extended_trees_test {
    ...
    makeOption(Settings* settings, const ext::shared_ptr<StrikedTypePayoff>& payoff,
        ...
        switch (engineType) {
          ...
          case JR:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedJarrowRudd>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case CRR:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedCoxRossRubinstein>(settings, 
                                                              stochProcess,
                                                              binomialSteps));
            break;
          case EQP:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedAdditiveEQPBinomialTree>(settings, 
                                                              stochProcess,
                                                              binomialSteps));
            break;
          case TGEO:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedTrigeorgis>(settings, stochProcess,
                                                              binomialSteps));
            break;
          case TIAN:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedTian>(settings, stochProcess,
                                                        binomialSteps));
            break;
          case LR:
            engine = ext::shared_ptr<PricingEngine>(
                      new BinomialVanillaEngine<ExtendedLeisenReimer>(settings, 
                                                              stochProcess,
                                                              binomialSteps));
            break;
          case JOSHI:
            engine = ext::shared_ptr<PricingEngine>(
                new BinomialVanillaEngine<ExtendedJoshi4>(settings, stochProcess,
                                                          binomialSteps));
            break;
          default:
...
namespace {
    ...
    void testEngineConsistency(Settings* settings, extended_trees_test::EngineType engine,
                               Size binomialSteps,
...
void ExtendedTreesTest::testJRBinomialEngines() {
    ...
    testEngineConsistency(settings, engine, steps, relativeTol);
}
