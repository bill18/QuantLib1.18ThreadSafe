...
namespace capfloored_coupon_test {
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            frequency = Annual;
            index = ext::shared_ptr<IborIndex>(new Euribor1Y(settings, termStructure));
            ...
            termStructure.linkTo(flatRate(settings, settlement,0.05,
        ...
        Leg makeFloatingLeg(const Date& startDate,
            ...
            return IborLeg(settings, schedule, index)
                .withNotionals(nominals)
        ...
        Leg makeCapFlooredLeg(const Date& startDate,
            ...
            Handle<OptionletVolatilityStructure> vol(
                ext::shared_ptr<OptionletVolatilityStructure>(new
                    ConstantOptionletVolatility(settings, 0, calendar, Following,
                                                volatility,Actual365Fixed())));

            ext::shared_ptr<IborCouponPricer> pricer(new
                BlackIborCouponPricer(settings, vol));
            std::vector<Rate> gearingVector(length, gearing);
            std::vector<Spread> spreadVector(length, spread);

            Leg iborLeg = IborLeg(settings, schedule, index)
                .withNotionals(nominals)
                .withPaymentDayCounter(index->dayCounter())
                .withPaymentAdjustment(convention)
                .withFixingDays(fixingDays)
        ...
        ext::shared_ptr<PricingEngine> makeEngine(Volatility volatility) {
            ...
            return ext::shared_ptr<PricingEngine>(
                                 new BlackCapFloorEngine(settings, termStructure, vol));
        }
            ...
            switch (type) {
              case CapFloor::Cap:
                result = ext::shared_ptr<CapFloor>(
                               new Cap(settings, leg, std::vector<Rate>(1, capStrike)));
                break;
              case CapFloor::Floor:
                result = ext::shared_ptr<CapFloor>(
                           new Floor(settings, leg, std::vector<Rate>(1, floorStrike)));
                break;
              case CapFloor::Collar:
                result = ext::shared_ptr<CapFloor>(
                               new Collar(settings, leg,
...
void CapFlooredCouponTest::testDecomposition() {
    ...
    */
    ...
    Cap cap(vars.settings, floatLeg, std::vector<Rate>(1, capstrike));
    ...
    */
    ...
    Floor floor(vars.settings, floatLeg, std::vector<Rate>(1, floorstrike));
    ...
    */
    ...
    Swap collarLeg(vars.settings, fixedLeg,collaredLeg);
    collarLeg.setPricingEngine(engine);
    Collar collar(vars.settings, floatLeg,
                  std::vector<Rate>(1, capstrike),
                  std::vector<Rate>(1, floorstrike));
    ...
    */
    ...
    Cap cap_p(vars.settings, floatLeg_p,std::vector<Rate>(1,capstrike));
    ...
    Floor floor_n(vars.settings, floatLeg,std::vector<Rate>(1,(capstrike-spread_n)/gearing_n));
    ...
    */
    ...
    Floor floor_p1(vars.settings, floatLeg_p,std::vector<Rate>(1,floorstrike));
    ...
    Cap cap_n(vars.settings, floatLeg,std::vector<Rate>(1,(floorstrike-spread_n)/gearing_n));
    ...
    */
    ...
    Collar collar_p(vars.settings, floatLeg_p,
                    std::vector<Rate>(1,capstrike),
                    std::vector<Rate>(1,floorstrike));
    ...
    Collar collar_n(vars.settings, floatLeg,
