...
namespace andreasen_huge_volatility_interpl_test {
    ...
    CalibrationData AndreasenHugeExampleData(Settings* settings) {
    ...
    void testAndreasenHugeVolatilityInterpolation(Settings* settings,
...
//        //SavedSettings backup;
        ...
        const ext::shared_ptr<AndreasenHugeVolatilityAdapter> volatilityAdapter(
            ext::make_shared<AndreasenHugeVolatilityAdapter>(settings,
                andreasenHugeVolInterplation, 1e-12));

        const ext::shared_ptr<AndreasenHugeLocalVolAdapter> localVolAdapter(
            ext::make_shared<AndreasenHugeLocalVolAdapter>(settings,
                andreasenHugeVolInterplation));
    ...
    CalibrationData BorovkovaExampleData(Settings* settings) {
        ...
        const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.025, dc));
        const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.085, dc));
        ...
        for (Size i=0; i < LENGTH(strikes); ++i) {
            ...
            for (Size j=0; j < LENGTH(maturityMonths); ++j) {
                ...
                if (std::fabs(mn) < 3.71*vol) {
                    ...
                    calibrationSet.push_back(std::make_pair(
                        ext::make_shared<VanillaOption>(settings,
                            ext::make_shared<PlainVanillaPayoff>(
                                Option::Call, strike),
    ...
    CalibrationData arbitrageData(Settings* settings) {

        const DayCounter dc = Actual365Fixed();
        const Date today = Date(4, January, 2018);

        const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.13, dc));
        const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.03, dc));
...
void AndreasenHugeVolatilityInterplTest::testAndreasenHugePut() {
    ...
    const CalibrationData data = AndreasenHugeExampleData(settings);
    ...
    testAndreasenHugeVolatilityInterpolation(settings, data, expected);
}
...
//    //SavedSettings backup;
    ...
    const Handle<YieldTermStructure> rTS(
        ext::make_shared<ZeroCurve>(settings, dates, r, dc));
    const Handle<YieldTermStructure> qTS(
        ext::make_shared<ZeroCurve>(settings, dates, q, dc));

    const CalibrationData origData = AndreasenHugeExampleData(settings);
    AndreasenHugeVolatilityInterpl::CalibrationSet calibrationSet =
    ...
    testAndreasenHugeVolatilityInterpolation(settings, irData, expected);
}
...
void AndreasenHugeVolatilityInterplTest::testArbitrageFree() {
    ...
    CalibrationData data[] = { BorovkovaExampleData(settings), arbitrageData(settings) };;
...
void AndreasenHugeVolatilityInterplTest::testBarrierOptionPricing() {
    ...
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.01, dc));
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.03, dc));
    ...
    BarrierOption barrierOption(settings, barrierType, barrier, rebate,
...
namespace andreasen_huge_volatility_interpl_test {
    std::pair<CalibrationData, std::vector<Real> > sabrData(Settings* settings) {
        ...
        const Handle<YieldTermStructure> rTS(flatRate(settings, today, forward, dc));
        const Handle<YieldTermStructure> qTS(flatRate(settings, today, forward, dc));
...
void AndreasenHugeVolatilityInterplTest::testPeterAndFabiensExample() {
    ...
    const std::pair<CalibrationData, std::vector<Real> > sd = sabrData(settings);
...
void AndreasenHugeVolatilityInterplTest::testDifferentOptimizers() {
    ...
    const CalibrationData& data = sabrData(settings).first;
...
void AndreasenHugeVolatilityInterplTest::testMovingReferenceDate() {
    ...
    Handle<YieldTermStructure> ts(flatRate(settings, 0.04, dc));
    ...
    settings->evaluationDate() = modToday;
...
void AndreasenHugeVolatilityInterplTest::testFlatVolCalibration() {
    ...
    const Handle<YieldTermStructure> rTS(flatRate(settings, ref, 0.02, dc));
    const Handle<YieldTermStructure> qTS(flatRate(settings, ref, 0.0, dc));
    ...
    testAndreasenHugeVolatilityInterpolation(settings, flatVolData, expected);
}
