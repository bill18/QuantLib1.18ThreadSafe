...
void CompoundOptionTest::testPutCallParity(){
    ...
    Date todaysDate = settings->evaluationDate();
    ...
    ext::shared_ptr<YieldTermStructure> rTS(
              new FlatForward(settings, 0, NullCalendar(), Handle<Quote>(rRate), dc));

    ext::shared_ptr<YieldTermStructure> qTS(
              new FlatForward(settings, 0, NullCalendar(), Handle<Quote>(qRate), dc));

    ext::shared_ptr<BlackVolTermStructure> volTS(
                              new BlackConstantVol(settings, todaysDate, NullCalendar(),
                                                   Handle<Quote>(vol), dc));
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        CompoundOption compoundOptionCall(settings, payoffMotherCall,exerciseCompound,
                                          payoffDaughter, exerciseDaughter);

        CompoundOption compoundOptionPut(settings, payoffMotherPut,exerciseCompound,
                                         payoffDaughter, exerciseDaughter);

        VanillaOption vanillaOption(EuropeanOption(settings, payoffDaughter,
                                                   exerciseDaughter));
        ...
        ext::shared_ptr<PricingEngine> engineCompound(
                              new AnalyticCompoundOptionEngine(settings, stochProcess));
...
void CompoundOptionTest::testValues(){
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        CompoundOption compoundOption(settings, payoffMother,exerciseMother,
                                      payoffDaughter, exerciseDaughter);
