...
namespace {
    ...
    Disposable<Array> priceReport(
        ...
        const ext::shared_ptr<YieldTermStructure> qTS
            = flatRate(settings, today, 0.025, dc);
    ...
    class FdmMispricingCostFunction : public CostFunction {
      public:
        FdmMispricingCostFunction(
            Settings* settings,
            const GridSetup& setup, const Array& strikes)
        : setup_(setup), strikes_(strikes), env_(settings) { }
        ...
        Disposable<Array> values(const Array& x) const {
            ...
            try {
                return priceReport(env_, g, strikes_);
            }
...
void NthOrderDerivativeOpTest::testHigerOrderBSOptionPricing() {
    ...
    FdmMispricingCostFunction costFct(settings, initSetup, strikes);
    ...
    const Array q = priceReport(settings, optimalSetup, strikes);
...
namespace {
    Real priceQuality(Settings* settings, Real h) {
        ...
        return  std::fabs(priceReport(settings, setup, strikes)[0]);
    }
}
...
void NthOrderDerivativeOpTest::testHigerOrderAndRichardsonExtrapolationg() {
    ...
    const Real n1 = priceQuality(settings, 1.0/50);
    auto func = [=](Real h) { return priceQuality(settings, h); };
    const Real n3
        = std::fabs(RichardsonExtrapolation(func,  1.0/50, 4.0)(2.0)); // priceQuality
