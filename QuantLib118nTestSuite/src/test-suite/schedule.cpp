...
void ScheduleTest::testDailySchedule() {
    ...
    Schedule s =
        MakeSchedule(settings->evaluationDate()).from(startDate).to(startDate+7)
                      .withCalendar(TARGET())
                      .withFrequency(Daily)
...
void ScheduleTest::testEndDateWithEomAdjustment() {
    ...
    Schedule s =
        MakeSchedule(settings->evaluationDate()).from(Date(30,September,2009))
    ...
    s = MakeSchedule(settings->evaluationDate()).from(Date(30,September,2009))
                      .to(Date(15,June,2012))
                      .withCalendar(Japan())
                      .withTenor(6*Months)
                      .withConvention(Following)
...
void ScheduleTest::testDatesPastEndDateWithEomAdjustment() {
    ...
    Schedule s =
        MakeSchedule(settings->evaluationDate()).from(Date(28,March,2013))
...
void ScheduleTest::testForwardDatesWithEomAdjustment() {
    ...
    Schedule s =
        MakeSchedule(settings->evaluationDate()).from(Date(31,August,1996))
...
void ScheduleTest::testBackwardDatesWithEomAdjustment() {
    ...
    Schedule s =
        MakeSchedule(settings->evaluationDate()).from(Date(22,August,1996))
...
void ScheduleTest::testCDS2015Convention() {
    ...
    Schedule s1 =
        MakeSchedule(settings->evaluationDate()).from(Date(12, December, 2016))
    ...
    Schedule s2 =
        MakeSchedule(settings->evaluationDate()).from(Date(1, March, 2017))
    ...
    Schedule s3 =
        MakeSchedule(settings->evaluationDate()).from(Date(20, March, 2017))
...
void ScheduleTest::testFourWeeksTenor() {
    ...
    try {
        Schedule s =
            MakeSchedule(settings->evaluationDate()).from(Date(13,January,2016))
                          .to(Date(4,May,2016))
                          .withCalendar(TARGET())
                          .withTenor(4*Weeks)
                          .withConvention(Following)
...
void ScheduleTest::testScheduleAlwaysHasAStartDate() {
    ...
    schedule = MakeSchedule(settings->evaluationDate())
        .from(Date(10, January, 2017))
        .to(Date(28, February, 2026))
        .withFrequency(Semiannual)
        .withCalendar(calendar)
...
void ScheduleTest::testShortEomSchedule() {
    ...
    BOOST_REQUIRE_NO_THROW(s = MakeSchedule(settings->evaluationDate())
...
void ScheduleTest::testTruncation() {
    ...
    Schedule s = MakeSchedule(settings->evaluationDate()).from(Date(30, September, 2009))
