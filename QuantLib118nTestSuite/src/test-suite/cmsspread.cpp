...
struct TestData {
    ...
    TestData() {
        ...
        yts2 = Handle<YieldTermStructure>(
            ext::make_shared<FlatForward>(settings, refDate, 0.02, Actual365Fixed()));
        ...
        cmsPricerLn =
            ext::make_shared<LinearTsrPricer>(settings, swLn, reversion, yts2);
        cmsPricerSln =
            ext::make_shared<LinearTsrPricer>(settings, swSln, reversion, yts2);
        cmsPricerN = ext::make_shared<LinearTsrPricer>(settings, swN, reversion, yts2);

        correlation = Handle<Quote>(ext::make_shared<SimpleQuote>(0.6));
        cmsspPricerLn = ext::make_shared<LognormalCmsSpreadPricer>(settings, 
            cmsPricerLn, correlation, yts2, 32);
        cmsspPricerSln = ext::make_shared<LognormalCmsSpreadPricer>(settings, 
            cmsPricerSln, correlation, yts2, 32);
        cmsspPricerN = ext::make_shared<LognormalCmsSpreadPricer>(settings, 
...
void CmsSpreadTest::testFixings() {
    ...
    ext::shared_ptr<SwapIndex> cms10y =
        ext::make_shared<EuriborSwapIsdaFixA>(d.settings, 10 * Years, d.yts2, d.yts2);
    ext::shared_ptr<SwapIndex> cms2y =
        ext::make_shared<EuriborSwapIsdaFixA>(d.settings, 2 * Years, d.yts2, d.yts2);
    ext::shared_ptr<SwapSpreadIndex> cms10y2y =
        ext::make_shared<SwapSpreadIndex>(d.settings, "cms10y2y", cms10y, cms2y);
...
void CmsSpreadTest::testCouponPricing() {
    ...
    ext::shared_ptr<CmsCoupon> cpn1a =
        ext::shared_ptr<CmsCoupon>(new CmsCoupon(d.settings,
    ...
    ext::shared_ptr<CmsCoupon> cpn1b = ext::shared_ptr<CmsCoupon>(
        new CmsCoupon(d.settings, payDate, 10000.0, valueDate, payDate, cms2y->fixingDays(),
                      cms2y, 1.0, 0.0, Date(), Date(), Actual360(), false));
    ext::shared_ptr<CmsSpreadCoupon> cpn1 =
        ext::shared_ptr<CmsSpreadCoupon>(new CmsSpreadCoupon(d.settings,
    ...
    ext::shared_ptr<CmsCoupon> cpn2a = ext::shared_ptr<CmsCoupon>(
        new CmsCoupon(d.settings, Date(23, February, 2029), 10000.0,
    ...
    ext::shared_ptr<CappedFlooredCmsSpreadCoupon> plainCpn =
        ext::shared_ptr<CappedFlooredCmsSpreadCoupon>(
            new CappedFlooredCmsSpreadCoupon(d.settings,
