...
namespace swaption_test {
    ...
    struct CommonVars {
        ...
        makeSwaption(const ext::shared_ptr<VanillaSwap>& swap, const Date& exercise,
            ...
            ext::shared_ptr<PricingEngine> engine(new BlackSwaptionEngine(settings, 
                termStructure, vol, Actual365Fixed(), 0.0, model));

            ext::shared_ptr<Swaption> result(new
                Swaption(settings, swap,
        ...
        makeEngine(Volatility volatility,
            ...
            return ext::shared_ptr<PricingEngine>(
                new BlackSwaptionEngine(settings, termStructure, h, Actual365Fixed(), 0.0, model));
        }
            ...
            for (Size k=0; k<LENGTH(type); k++) {
                ...
                for (Size l=0; l<LENGTH(strikes); l++) {
                    ext::shared_ptr<VanillaSwap> swap =
                        MakeVanillaSwap(vars.settings, lengths[j], vars.index, strikes[l])
        ...
        for (Size j=0; j<LENGTH(lengths); j++) {
            for (Size k=0; k<LENGTH(type); k++) {
                ...
                for (Size l=0; l<LENGTH(spreads); l++) {
                    ext::shared_ptr<VanillaSwap> swap =
                        MakeVanillaSwap(vars.settings, lengths[j], vars.index, 0.06)
                                .withFixedLegTenor(1*Years)
                                .withFixedLegDayCount(vars.fixedDayCount)
                                .withEffectiveDate(startDate)
            ...
            for (Size k=0; k<LENGTH(type); k++) {
                ...
                for (Size l=0; l<LENGTH(spreads); l++) {
                    ...
                    ext::shared_ptr<VanillaSwap> equivalentSwap =
                        MakeVanillaSwap(vars.settings, lengths[j], vars.index, 0.06+correction)
                                .withFixedLegTenor(1*Years)
                                .withFixedLegDayCount(vars.fixedDayCount)
                                .withEffectiveDate(startDate)
                                .withFloatingLegSpread(0.0)
...
void SwaptionTest::testCachedValue() {
    ...
    vars.termStructure.linkTo(flatRate(vars.settings, vars.settlement, 0.05, Actual365Fixed()));
    Date exerciseDate = vars.calendar.advance(vars.settlement, 5*Years);
    Date startDate = vars.calendar.advance(exerciseDate,
                                           vars.settlementDays, Days);
    ext::shared_ptr<VanillaSwap> swap =
        MakeVanillaSwap(vars.settings, 10*Years, vars.index, 0.06)
        .withEffectiveDate(startDate)
        .withFixedLegTenor(1*Years)
        .withFixedLegDayCount(vars.fixedDayCount);
        ...
        for (Size j=0; j<LENGTH(lengths); j++) {
            for (Size t=0; t<LENGTH(strikes); t++) {
                for (Size h=0; h<LENGTH(type); h++) {
                    ext::shared_ptr<VanillaSwap> swap =
                        MakeVanillaSwap(vars.settings, lengths[j], vars.index, strikes[t])
                                .withEffectiveDate(startDate)
                                .withFixedLegTenor(1*Years)
                                .withFixedLegDayCount(vars.fixedDayCount)
                                .withFloatingLegSpread(0.0)
...
void SwaptionTest::testCashSettledSwaptions() {
    ...
    for (Size i=0; i<LENGTH(exercises); i++) {
        for (Size j=0; j<LENGTH(lengths); j++) {
            ...
            Schedule floatSchedule(vars.settings->evaluationDate(), startDate, maturity, vars.floatingTenor,
                                   vars.calendar,vars.floatingConvention,
                                   vars.floatingConvention,
                                   DateGeneration::Forward, false);
            // Swap with fixed leg conventions: Business Days = Unadjusted, DayCount = 30/360
            Schedule fixedSchedule_u(vars.settings->evaluationDate(), startDate, maturity,
            ...
            ext::shared_ptr<VanillaSwap> swap_u360(
                new VanillaSwap(vars.settings, type[0], vars.nominal,
            ...
            Schedule fixedSchedule_a(vars.settings->evaluationDate(), startDate,maturity,
                                     Period(vars.fixedFrequency),
