...
void ForwardRateAgreementTest::testConstructionWithoutACurve() {
    ...
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
        ...
        ext::shared_ptr<IborIndex> index = ext::make_shared<USDLibor>(settings, Period(3, Months), curveHandle);
        ...
        helpers.push_back(ext::make_shared<FraRateHelper>(settings, Handle<Quote>(quotes[0]),
                                                          Period(1, Years), index,
                                                          Pillar::LastRelevantDate, Date(),
                                                          useIndexedFra));
        helpers.push_back(ext::make_shared<FraRateHelper>(settings, Handle<Quote>(quotes[1]),
                                                          Period(2, Years), index,
                                                          Pillar::LastRelevantDate, Date(),
                                                          useIndexedFra));
        helpers.push_back(ext::make_shared<FraRateHelper>(settings, Handle<Quote>(quotes[2]),
                                                          Period(3, Years), index,
                                                          Pillar::LastRelevantDate, Date(),
                                                          useIndexedFra));
        ext::shared_ptr<PiecewiseYieldCurve<ForwardRate, QuantLib::Cubic> > curve =
            ext::make_shared<PiecewiseYieldCurve<ForwardRate, QuantLib::Cubic> >(settings, 
                today, helpers, index->dayCounter());

        curveHandle.linkTo(curve);

        // set up the instrument to price
        ForwardRateAgreement fra(settings, settlementDate + Period(12, Months),
