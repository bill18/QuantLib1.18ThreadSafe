...
void NthToDefaultTest::testGauss() {
    ...
    ext::shared_ptr<YieldTermStructure> yieldPtr (
                                   new FlatForward (settings, asofDate, rate, dc, cmp));
    ...
    for (Size i = 0; i < lambda.size(); i++) {
        ...
        ext::shared_ptr<DefaultProbabilityTermStructure> ptr (
                                         new FlatHazardRate(settings, asofDate, h, dc));
    ...
    */
    ...
    ext::shared_ptr<Basket> basket(new Basket(settings, asofDate, namesIds, 
        std::vector<Real>(names, namesNotional/names), thePool, 0., 1.));

    ext::shared_ptr<PricingEngine> engine(
        new IntegralNtdEngine(settings, timeUnit, yieldHandle));

    vector<NthToDefault> ntd;
    for (Size i = 1; i <= probabilities.size(); i++) {
        ntd.push_back (NthToDefault (settings, basket, i, Protection::Seller, 
