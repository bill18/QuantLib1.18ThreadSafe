...
void SofrFuturesTest::testBootstrap() {
    ...
    ext::shared_ptr<OvernightIndex> index = ext::make_shared<Sofr>(settings);
    ...
    for (Size i = 0; i < LENGTH(sofrQuotes); i++) {
        helpers.push_back(ext::make_shared<SofrFutureRateHelper>(settings, 
    ...
    ext::shared_ptr<PiecewiseYieldCurve<Discount, Linear> > curve =
        ext::make_shared<PiecewiseYieldCurve<Discount, Linear> >(settings, today, helpers,
                                                                 Actual365Fixed());

    // test curve with one of the futures
    ext::shared_ptr<Payoff> payoff(new ForwardTypePayoff(Position::Long, 97.440));
    OvernightIndexFuture sf(settings, index, payoff,
