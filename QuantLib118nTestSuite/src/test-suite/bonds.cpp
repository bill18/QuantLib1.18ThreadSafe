...
namespace bonds_test {
    ...
    struct CommonVars {
        ...
        // setup
        CommonVars() {
            settings->evaluationDate() = EvaluationDateSingleton::instance().getDate();
            calendar = TARGET();
        ...
        for (Size k=0; k<LENGTH(coupons); k++) {
          for (Size l=0; l<LENGTH(frequencies); l++) {
            for (Size n=0; n<LENGTH(compounding); n++) {
              ...
              Schedule sch(vars.settings->evaluationDate(), dated, maturity,
                           Period(frequencies[l]), vars.calendar,
                           accrualConvention, accrualConvention,
                           DateGeneration::Backward, false);

              FixedRateBond bond(vars.settings, settlementDays, vars.faceAmount, sch,
              ...
              for (Size m=0; m<LENGTH(yields); m++) {
                ...
                Real price = BondFunctions::cleanPrice(vars.settings, bond, yields[m],
                                                       bondDayCount,
                                                       compounding[n],
                                                       frequencies[l]);

                Rate calculated = BondFunctions::yield(vars.settings, bond, price,
                ...
                if (std::fabs(yields[m]-calculated) > tolerance) {
                  ...
                  Real price2 = BondFunctions::cleanPrice(vars.settings, bond, calculated,
                                                          bondDayCount,
                                                          compounding[n],
                                                          frequencies[l]);
                ...
                price = BondFunctions::dirtyPrice(vars.settings, bond, yields[m],
                                                  bondDayCount,
                                                  compounding[n],
                                                  frequencies[l]);

                calculated = BondFunctions::yield(vars.settings, bond, price,
                                                  bondDayCount, compounding[n],
                                                  frequencies[l],
                                                  Date(),
                                                  tolerance, maxEvaluations,
                ...
                if (std::fabs(yields[m]-calculated) > tolerance) {
                  // the difference might not matter
                  Real price2 = BondFunctions::dirtyPrice(vars.settings, bond, calculated,
                                                          bondDayCount,
                                                          compounding[n],
                                                          frequencies[l]);
                  if (std::fabs(price-price2)/price > tolerance) {
...
void BondTest::testAtmRate() {
    ...
    BusinessDayConvention paymentConvention = ModifiedFollowing;
    Real redemption = 100.0;
    Handle<YieldTermStructure> disc(flatRate(vars.settings, vars.today,0.03,Actual360()));
    ext::shared_ptr<PricingEngine> bondEngine(new DiscountingBondEngine(vars.settings, disc));
      ...
      for (Size j=0; j<LENGTH(lengths); j++) {
        for (Size k=0; k<LENGTH(coupons); k++) {
          for (Size l=0; l<LENGTH(frequencies); l++) {
            ...
            Rate calculated = BondFunctions::atmRate(vars.settings, bond,
                                                     **disc,
                                                     bond.settlementDate(),
                                                     price);
...
void BondTest::testZspread() {
    ...
    Handle<YieldTermStructure> discountCurve(
                                       flatRate(vars.settings, vars.today,0.03,Actual360()));
          ...
          for (Size l=0; l<LENGTH(frequencies); l++) {
            for (Size n=0; n<LENGTH(compounding); n++) {
              ...
              for (Size m=0; m<LENGTH(spreads); m++) {
                ...
                Real price = BondFunctions::cleanPrice(vars.settings, bond, *discountCurve,
                                                   spreads[m],
                                                   bondDayCount,
                                                   compounding[n],
                                                   frequencies[l]);
                Spread calculated = BondFunctions::zSpread(vars.settings, bond, price,
                                                           *discountCurve,
                                                           bondDayCount,
                                                           compounding[n],
                                                           frequencies[l],
                ...
                if (std::fabs(spreads[m]-calculated) > tolerance) {
                  // the difference might not matter
                  Real price2 = BondFunctions::cleanPrice(vars.settings, bond, *discountCurve,
                                                      calculated,
                                                      bondDayCount,
                                                      compounding[n],
                                                      frequencies[l]);
    ...
    for (Size j=0; j<LENGTH(lengths); j++) {
      for (Size k=0; k<LENGTH(coupons); k++) {
        for (Size l=0; l<LENGTH(frequencies); l++) {
            ...
            Handle<YieldTermStructure> discountCurve(flatRate(vars.settings, vars.today,
                                                              rate,
                                                              bondDayCount));
            ...
            ext::shared_ptr<PricingEngine> bondEngine(
                                    new DiscountingBondEngine(vars.settings, discountCurve));
            bond.setPricingEngine(bondEngine);
            ...
            for (Size m=0; m<LENGTH(yields); m++) {
                ...
                Rate calculatedYield = BondFunctions::yield(vars.settings, bond, calculatedPrice,
...
void BondTest::testCached() {
    ...
    vars.settings->evaluationDate() = today;

    Calendar bondCalendar = NullCalendar();
    
    Natural settlementDays = 1;

    Handle<YieldTermStructure> discountCurve(flatRate(vars.settings, today,0.03,Actual360()));
    ...
    Schedule sch1(vars.settings->evaluationDate(), Date(31, October, 2004),
    ...
    FixedRateBond bond1(vars.settings, settlementDays, vars.faceAmount, sch1,
                        std::vector<Rate>(1, 0.025),
                        bondDayCount1, ModifiedFollowing,
                        100.0, Date(1, November, 2004));
    FixedRateBond bond1NoSchedule(vars.settings,
    ...
    Schedule sch2(vars.settings->evaluationDate(), Date(15, November, 2004),
    ...
    FixedRateBond bond2(vars.settings, settlementDays, vars.faceAmount, sch2,
                        std::vector<Rate>(1, 0.035),
                        bondDayCount2, ModifiedFollowing,
                        100.0, Date(15, November, 2004));
    FixedRateBond bond2NoSchedule(vars.settings, settlementDays, vars.faceAmount, sch2,
        std::vector<Rate>(1, 0.035),
    ...
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond1, marketYield1, bondDayCount1, Compounded, freq),
        cachedPrice1a,
        tolerance,
        "failed to reproduce cached price with schedule for bond 1:"
    );
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond1NoSchedule, marketYield1, bondDayCount1NoSchedule, Compounded, freq),
        cachedPrice1a,
        tolerance,
    ...
    checkValue(
        BondFunctions::yield(vars.settings, bond1, marketPrice1, bondDayCount1, Compounded, freq),
        cachedYield1a,
        tolerance,
        "failed to reproduce cached compounded yield with schedule for bond 1:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond1NoSchedule, marketPrice1, bondDayCount1NoSchedule, Compounded, freq),
        cachedYield1a,
        tolerance,
        "failed to reproduce cached compounded yield with no schedule for bond 1:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond1, marketPrice1, bondDayCount1, Continuous, freq),
        cachedYield1b,
        tolerance,
        "failed to reproduce cached continuous yield with schedule for bond 1:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond1NoSchedule, marketPrice1, bondDayCount1NoSchedule, Continuous, freq),
        cachedYield1b,
        tolerance,
        "failed to reproduce cached continuous yield with no schedule for bond 1:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond1, bond1.cleanPrice(), bondDayCount1, Continuous, freq, bond1.settlementDate()),
        cachedYield1c,
        tolerance,
        "failed to reproduce cached continuous yield with schedule for bond 1:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond1NoSchedule, bond1NoSchedule.cleanPrice(), bondDayCount1NoSchedule, Continuous, freq, bond1.settlementDate()),
        cachedYield1c,
        tolerance,
        "failed to reproduce cached continuous yield with no schedule for bond 1:"
    );
    ...
    //Now bond 2
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond2, marketYield2, bondDayCount2, Compounded, freq),
        cachedPrice2a,
        tolerance,
        "failed to reproduce cached price with schedule for bond 2"
    );
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond2NoSchedule, marketYield2, bondDayCount2NoSchedule, Compounded, freq),
        cachedPrice2a,
        tolerance,
    ...
    checkValue(
        BondFunctions::yield(vars.settings, bond2, marketPrice2, bondDayCount2, Compounded, freq),
        cachedYield2a,
        tolerance,
        "failed to reproduce cached compounded yield with schedule for bond 2:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond2NoSchedule, marketPrice2, bondDayCount2NoSchedule, Compounded, freq),
        cachedYield2a,
        tolerance,
        "failed to reproduce cached compounded yield with no schedule for bond 2:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond2, marketPrice2, bondDayCount2, Continuous, freq),
        cachedYield2b,
        tolerance,
        "failed to reproduce chached continuous yield with schedule for bond 2:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond2NoSchedule, marketPrice2, bondDayCount2NoSchedule, Continuous, freq),
        cachedYield2b,
        tolerance,
        "failed to reproduce cached continuous yield with schedule for bond 2:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond2, bond2.cleanPrice(), bondDayCount2, Continuous, freq, bond2.settlementDate()),
        cachedYield2c,
        tolerance,
        "failed to reproduce cached continuous yield for bond 2 with schedule:"
    );
    checkValue(
        BondFunctions::yield(vars.settings, bond2NoSchedule, bond2NoSchedule.cleanPrice(), bondDayCount2NoSchedule, Continuous, freq, bond2NoSchedule.settlementDate()),
        cachedYield2c,
        tolerance,
    ...
    Schedule sch3(vars.settings->evaluationDate(), Date(30,November,2004),
    ...
    FixedRateBond bond3(vars.settings, settlementDays, vars.faceAmount, sch3,
                        std::vector<Rate>(1, 0.02875),
                        bondDayCount3,
                        ModifiedFollowing,
                        100.0, Date(30,November,2004));
    FixedRateBond bond3NoSchedule(vars.settings, settlementDays, vars.faceAmount, sch3,
        std::vector<Rate>(1, 0.02875),
    ...
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond3, marketYield3, bondDayCount3, Compounded, freq, settlementDate),
        cachedPrice3,
        tolerance,
        "Failed to reproduce cached price for bond 3 with schedule"
    );
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond3NoSchedule, marketYield3, bondDayCount3NoSchedule, Compounded, freq, settlementDate),
        cachedPrice3,
        tolerance,
    ...
    vars.settings->evaluationDate() = Date(22,November,2004);
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond3, marketYield3, bondDayCount3, Compounded, freq),
        cachedPrice3,
        tolerance,
        "Failed to reproduce the cached price for bond 3 with schedule and the earlierst possible settlment date"
    );
    checkValue(
        BondFunctions::cleanPrice(vars.settings, bond3NoSchedule, marketYield3, bondDayCount3NoSchedule, Compounded, freq),
        cachedPrice3,
        tolerance,
...
void BondTest::testCachedZero() {
    ...
    ZeroCouponBond bond1(vars.settings, settlementDays,
                         UnitedStates(UnitedStates::GovernmentBond),
                         vars.faceAmount,
    ...
    ZeroCouponBond bond2(vars.settings, settlementDays,
                         UnitedStates(UnitedStates::GovernmentBond),
                         vars.faceAmount,
    ...
    ZeroCouponBond bond3(vars.settings, settlementDays,
                         UnitedStates(UnitedStates::GovernmentBond),
                         vars.faceAmount,
...
void BondTest::testCachedFixed() {
    ...
    Schedule sch(vars.settings->evaluationDate(), Date(30,November,2004),
                 Date(30,November,2008), Period(Semiannual),
                 UnitedStates(UnitedStates::GovernmentBond),
                 Unadjusted, Unadjusted, DateGeneration::Backward, false);

    FixedRateBond bond1(vars.settings, settlementDays, vars.faceAmount, sch,
                        std::vector<Rate>(1, 0.02875),
    ...
    FixedRateBond bond2(vars.settings, settlementDays, vars.faceAmount, sch,
                          couponRates,
                          ActualActual(ActualActual::ISMA),
                          ModifiedFollowing,
                          100.0, Date(30,November,2004));
...
void BondTest::testCachedFloating() {
    ...
    Handle<YieldTermStructure> riskFreeRate(flatRate(vars.settings, today,0.025,Actual360()));
    Handle<YieldTermStructure> discountCurve(flatRate(vars.settings, today,0.03,Actual360()));

    ext::shared_ptr<IborIndex> index(new USDLibor(vars.settings, 6*Months, riskFreeRate));
    Natural fixingDays = 1;

    Real tolerance = 1.0e-6;

    ext::shared_ptr<IborCouponPricer> pricer(new
        BlackIborCouponPricer(vars.settings, Handle<OptionletVolatilityStructure>()));
    ...
    FloatingRateBond bond1(vars.settings, settlementDays, vars.faceAmount, sch,
    ...
    ext::shared_ptr<PricingEngine> bondEngine(
                                     new DiscountingBondEngine(vars.settings, riskFreeRate));
    bond1.setPricingEngine(bondEngine);
    ...
    FloatingRateBond bond2(vars.settings, settlementDays, vars.faceAmount, sch,
                           index, ActualActual(ActualActual::ISMA),
                           ModifiedFollowing, fixingDays,
    ...
    FloatingRateBond bond3(vars.settings, settlementDays, vars.faceAmount, sch,
                           index, ActualActual(ActualActual::ISMA),
                           ModifiedFollowing, fixingDays,
    ...
    Schedule sch2(vars.settings->evaluationDate(), Date(26, November, 2003), Date(26, November, 2007), Period(Semiannual),
                 UnitedStates(UnitedStates::GovernmentBond), ModifiedFollowing, ModifiedFollowing,
                 DateGeneration::Backward, false);
    FloatingRateBond bond4(vars.settings, settlementDays, vars.faceAmount, sch2, index,
...
void BondTest::testBrazilianCached() {
    ...
    for (Size bondIndex = 0; bondIndex < maturityDates.size(); bondIndex++) {
        ...
        Schedule schedule(vars.settings->evaluationDate(), Date(1,January,2007),
                          maturityDates[bondIndex], Period(Semiannual),
                          Brazil(Brazil::Settlement),
                          Unadjusted, Unadjusted,
                          DateGeneration::Backward, false);

        FixedRateBond bond(vars.settings, settlementDays,
            faceAmount,
            schedule,
            couponRates,
            Following,
        ...
        Real price = faceAmount *
            (BondFunctions::cleanPrice(vars.settings, bond, yield.rate(), yield.dayCounter(),
...
void BondTest::testExCouponGilt() {
    ...
    */
    ...
    Schedule schedule = Schedule(vars.settings->evaluationDate(), startDate, maturityDate, tenor,
    ...
    FixedRateBond bond(vars.settings, settlementDays, 100.0,
                       schedule,
...
void BondTest::testExCouponAustralianBond() {
    ...
    */
    ...
    Schedule schedule = Schedule(settings->evaluationDate(), startDate, maturityDate, tenor,
        NullCalendar(), Unadjusted, Unadjusted,
        DateGeneration::Forward, true, firstCouponDate);
    DayCounter dc = ActualActual(ActualActual::ISMA, schedule);

    FixedRateBond bond(settings, settlementDays, 100.0,
                       schedule,
                       std::vector<Rate>(1, coupon),
                       dc, Unadjusted, 100.0,
    ...
    for (Size i=0; i<LENGTH(cases); ++i) {
        ...
        Rate yield = CashFlows::yield(settings, leg, npv, dc, comp, freq,
                                      false, cases[i].settlementDate);
        ASSERT_CLOSE("yield", cases[i].settlementDate,
                     yield, cases[i].yield, 1e-5);

        Time duration = CashFlows::duration(settings, leg, yield, dc, comp, freq,
                                            Duration::Modified, false,
                                            cases[i].settlementDate);
        ASSERT_CLOSE("duration", cases[i].settlementDate,
                     duration, cases[i].duration, 1e-5);

        Real convexity = CashFlows::convexity(settings, leg, yield, dc, comp, freq,
                                              false, cases[i].settlementDate);
        ASSERT_CLOSE("convexity", cases[i].settlementDate,
                     convexity, cases[i].convexity, 1e-4);

        Real calcnpv = CashFlows::npv(settings, leg, yield, dc, comp, freq,
                                      false, cases[i].settlementDate);
        ASSERT_CLOSE("NPV from yield", cases[i].settlementDate,
...
void BondTest::testBondFromScheduleWithDateVector()
{
    ...
    Schedule schedule(settings->evaluationDate(), issueDate, maturityDate, tenor,
        NullCalendar(), Unadjusted, Unadjusted,
    ...
    Real calculatedPrice = BondFunctions::dirtyPrice(settings, bond, yield, settlementDate);
...
void BondTest::testFixedRateBondWithArbitrarySchedule() {
    ...
    Handle<YieldTermStructure> discountCurve(flatRate(settings, today, 0.03, Actual360()));
    bond.setPricingEngine(ext::shared_ptr<PricingEngine>(new DiscountingBondEngine(settings, discountCurve)));
...
void BondTest::testThirty360BondWithSettlementOn31st(){
    ...
    settings->evaluationDate() = Date(28, July, 2017);
    ...
    Schedule fixedBondSchedule(settings->evaluationDate(), datedDate,
            maturity,
            Period(Semiannual),
            UnitedStates(UnitedStates::GovernmentBond),
            Unadjusted, Unadjusted, DateGeneration::Forward, false);

    FixedRateBond fixedRateBond(settings, 
            1,
            100,
            fixedBondSchedule,
    ...
    Real yield = BondFunctions::yield(settings, fixedRateBond, cleanPrice, dayCounter, compounding, Semiannual, settlement);
    ASSERT_CLOSE("yield", settlement, yield, 0.015, 1e-4);

    Real duration = BondFunctions::duration(settings, fixedRateBond, InterestRate(yield, dayCounter, compounding, Semiannual), Duration::Macaulay, settlement);
    ASSERT_CLOSE("duration", settlement, duration, 1.022, 1e-3);

    Real convexity = BondFunctions::convexity(settings, fixedRateBond, InterestRate(yield, dayCounter, compounding, Semiannual), settlement)/100;
    ASSERT_CLOSE("convexity", settlement, convexity, 0.015, 1e-3);

    Real accrued = BondFunctions::accruedAmount(settings, fixedRateBond, settlement);
