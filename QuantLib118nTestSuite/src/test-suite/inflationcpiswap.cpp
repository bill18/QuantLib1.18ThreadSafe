...
namespace inflation_cpi_swap_test {
    ...
    struct CommonVars {
        ...
        : nominals(1,1000000) {
            ...
            ii = ext::make_shared<UKRPI>(settings, interp, hcpi);
            ...
            ZeroInflationIndex>(settings, zciisData, zciisDataLength, ii,
                                observationLag,
...
void CPISwapTest::consistency() {
    ...
    ext::shared_ptr<IborIndex> floatIndex(new GBPLibor(common.settings, Period(6,Months),
                                                         common.nominalTS));
    ...
    Schedule floatSchedule = MakeSchedule(common.settings->evaluationDate()).from(startDate).to(endDate)
    .withTenor(Period(6,Months))
    .withCalendar(UnitedKingdom())
    .withConvention(floatPaymentConvention)
    .backwards()
    ;
    Schedule fixedSchedule = MakeSchedule(common.settings->evaluationDate()).from(startDate).to(endDate)
    .withTenor(Period(6,Months))
    .withCalendar(UnitedKingdom())
    ...
    CPISwap zisV(common.settings, type, nominal, subtractInflationNominal,
    ...
    ext::shared_ptr<DiscountingSwapEngine>
        dse(new DiscountingSwapEngine(common.settings, common.nominalTS));
...
void CPISwapTest::zciisconsistency() {
    ...
    Rate quote = 0.03714;
    ZeroCouponInflationSwap zciis(common.settings, ztype, nominal, startDate, endDate, cal,
    ...
    CPISwap cS(common.settings, stype, floatNominal, subtractInflationNominal, dummySpread, dummyDC, schOneDate,
...
void CPISwapTest::cpibondconsistency() {
    ...
    CPIBond cpiB(common.settings, settlementDays, nominal, growthOnly,
    ...
    dbe(new DiscountingBondEngine(common.settings, common.nominalTS));
