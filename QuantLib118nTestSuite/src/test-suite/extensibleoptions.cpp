...
void ExtensibleOptionsTest::testAnalyticHolderExtensibleOptionEngine() {
    ...
    HolderExtensibleOption option(settings, type, premium,
                                  exDate2, strike2,
                                  payoff, exercise);

    Handle<Quote> underlying(spot);
    Handle<YieldTermStructure> dividendTS(flatRate(settings, today, qRate, dc));
    Handle<YieldTermStructure> riskFreeTS(flatRate(settings, today, rRate, dc));
    Handle<BlackVolTermStructure> blackVolTS(flatVol(settings, today, vol, dc));

    const ext::shared_ptr<BlackScholesMertonProcess> process =
        ext::make_shared<BlackScholesMertonProcess>(settings, underlying,
                                                      dividendTS,
                                                      riskFreeTS,
                                                      blackVolTS);

    option.setPricingEngine(
           ext::make_shared<AnalyticHolderExtensibleOptionEngine>(settings, process));
...
void ExtensibleOptionsTest::testAnalyticWriterExtensibleOptionEngine() {
    ...
    ext::shared_ptr<PricingEngine> engine =
        ext::make_shared<AnalyticWriterExtensibleOptionEngine>(settings, process);
    ...
    WriterExtensibleOption option(settings, payoff1, exercise1,
                                  payoff2, exercise2);
