...
void GsrTest::testGsrProcess() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate(); // ????? init ...
    ...
    Handle<YieldTermStructure> yts0(ext::shared_ptr<YieldTermStructure>(
        new FlatForward(settings, 0, TARGET(), 0.00, Actual365Fixed())));
    ...
    do {
        ...
        ext::shared_ptr<Gsr> model(
            new Gsr(settings, yts0, stepDates0, vols0, reversions0, T));
        ext::shared_ptr<StochasticProcess1D> gsrProcess =
            model->stateProcess();
        ext::shared_ptr<Gsr> model2(
            new Gsr(settings, yts0, stepDates1, vols1, reversions1, T));
...
void GsrTest::testGsrModel() {
    ...
    ext::shared_ptr<Gsr> model(
        new Gsr(settings, yts, stepDates, vols, reversions, 50.0));
    ext::shared_ptr<Gsr> model2(
        new Gsr(settings, yts, stepDates1, vols1, reversions1, 50.0));
    ...
    ext::shared_ptr<SwapIndex> swpIdx(new EuriborSwapIsdaFixA(settings, tenor, yts));
    ...
    ext::shared_ptr<VanillaSwap> underlyingFixed =
        MakeVanillaSwap(settings, 10 * Years, swpIdx->iborIndex(), forward)
    ...
    ext::shared_ptr<Swaption> stdswaption(
        new Swaption(settings, underlyingFixed, exercise));
    ext::shared_ptr<NonstandardSwaption> nonstdswaption(
        new NonstandardSwaption(settings, *stdswaption));

    stdswaption->setPricingEngine(ext::shared_ptr<PricingEngine>(
        new JamshidianSwaptionEngine(settings, hw, yts)));
    Real HwJamNpv = stdswaption->NPV();

    nonstdswaption->setPricingEngine(ext::shared_ptr<PricingEngine>(
        new Gaussian1dNonstandardSwaptionEngine(settings, model, 64, 7.0, true, false)));
    stdswaption->setPricingEngine(ext::shared_ptr<PricingEngine>(
        new Gaussian1dSwaptionEngine(settings, model, 64, 7.0, true, false)));
    Real GsrNonStdNpv = nonstdswaption->NPV();
    Real GsrStdNpv = stdswaption->NPV();
    stdswaption->setPricingEngine(ext::shared_ptr<PricingEngine>(
        new Gaussian1dJamshidianSwaptionEngine(settings, model)));
