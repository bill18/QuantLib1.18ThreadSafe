...
namespace bermudan_swaption_test {
    ...
    struct CommonVars {
        ...
        ext::shared_ptr<VanillaSwap> makeSwap(Rate fixedRate) {
            ...
            Schedule fixedSchedule(settings->evaluationDate(), start, maturity,
                                   Period(fixedFrequency),
                                   calendar,
                                   fixedConvention,
                                   fixedConvention,
                                   DateGeneration::Forward, false);
            Schedule floatSchedule(settings->evaluationDate(), start, maturity,
...
void BermudanSwaptionTest::testCachedValues() {
    ...
    vars.termStructure.linkTo(flatRate(vars.settings, vars.settlement,
                                          0.04875825,
                                          Actual365Fixed()));
    ...
    ext::shared_ptr<PricingEngine> treeEngine(
                                            new TreeSwaptionEngine(vars.settings, model, 50));
    ext::shared_ptr<PricingEngine> fdmEngine(
                                         new FdHullWhiteSwaptionEngine(vars.settings, model));
    ...
    Swaption swaption(vars.settings, itmSwap, exercise);
    ...
    swaption = Swaption(vars.settings, atmSwap, exercise);
    swaption.setPricingEngine(treeEngine);
    ...
    swaption = Swaption(vars.settings, otmSwap, exercise);
    swaption.setPricingEngine(treeEngine);
...
void BermudanSwaptionTest::testCachedG2Values() {
    ...
    for (Real s=0.5; s<1.51; s+=0.25) {
        ...
        swaptions.push_back(ext::make_shared<Swaption>(vars.settings, swap,
    ...
    const ext::shared_ptr<PricingEngine> fdmEngine(
        ext::make_shared<FdG2SwaptionEngine>(vars.settings, g2Model, 50, 75, 75, 0, 1e-3));
    const ext::shared_ptr<PricingEngine> treeEngine(
        ext::make_shared<TreeSwaptionEngine>(vars.settings, g2Model, 50));
