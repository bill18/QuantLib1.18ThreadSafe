...
void BarrierOptionTest::testParity() {
    ...
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, 0.01, dc);
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, today, 0.20, dc);
    RelinkableHandle<BlackVolTermStructure> volHandle(volTS);

    ext::shared_ptr<BlackScholesProcess> stochProcess =
        ext::make_shared<BlackScholesProcess>(settings, 
                                      Handle<Quote>(spot),
    ...
    BarrierOption knockIn(settings, Barrier::DownIn, 90.0, 0.0,
                          payoff, exercise);
    BarrierOption knockOut(settings, Barrier::DownOut, 90.0, 0.0,
                           payoff, exercise);
    EuropeanOption european(settings, payoff, exercise);

    ext::shared_ptr<PricingEngine> barrierEngine
        = ext::make_shared<AnalyticBarrierEngine>(settings, stochProcess);

    ext::shared_ptr<PricingEngine> europeanEngine
        = ext::make_shared<AnalyticEuropeanEngine>(settings, stochProcess);
    ...
    volHandle.linkTo(flatVol(settings, today, 0.20, Business252(TARGET())));
...
void BarrierOptionTest::testHaugValues() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        if (values[i].exType == Exercise::European) {
           ...
           engine = ext::make_shared<AnalyticBarrierEngine>(settings, stochProcess);
           ...
           engine = ext::make_shared<FdBlackScholesBarrierEngine>(settings, stochProcess, 200, 400);
           barrierOption.setPricingEngine(engine);
        ...
        engine = ext::make_shared<BinomialBarrierEngine<CoxRossRubinstein,DiscretizedBarrierOption> >(settings, stochProcess, 400);
        barrierOption.setPricingEngine(engine);
        ...
        engine = ext::make_shared<BinomialBarrierEngine<CoxRossRubinstein,DiscretizedDermanKaniBarrierOption> >(settings, stochProcess, 400);
        barrierOption.setPricingEngine(engine);
        calculated = barrierOption.NPV();
        expected = values[i].result;
        error = std::fabs(calculated-expected);
...
void BarrierOptionTest::testBabsiriValues() {
    ...
    */
    ...
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, today, qH_SME, dc);

    ext::shared_ptr<SimpleQuote> rH_SME = ext::make_shared<SimpleQuote>(r);
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, rH_SME, dc);

    ext::shared_ptr<SimpleQuote> volatility =
        ext::make_shared<SimpleQuote>(0.10);
    ext::shared_ptr<BlackVolTermStructure> volTS =
        flatVol(settings, today, volatility, dc);
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        // analytic
        BarrierOption barrierCallOption(settings, 
                values[i].type,
                values[i].barrier,
                rebate,
                callPayoff,
        ...
        ext::shared_ptr<PricingEngine> mcEngine =
            MakeMCBarrierEngine<LowDiscrepancy>(settings, stochProcess)
            .withStepsPerYear(1)
            .withBrownianBridge()
...
void BarrierOptionTest::testPerturbative() {
    ...
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, today, q, dc);
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, r, dc);
    ...
    ext::shared_ptr<BlackVolTermStructure> volTS =
        ext::make_shared<BlackVarianceCurve>(settings, today, dates, vols, dc);
    ...
    BarrierOption option(settings, Barrier::UpOut, barrier, rebate, payoff, exercise);

    Natural order = 0;
    bool zeroGamma = false;
    ext::shared_ptr<PricingEngine> engine =
        ext::make_shared<PerturbativeBarrierOptionEngine>(settings, stochProcess,
                                                            order, zeroGamma);
    ...
    order = 1;
    engine = ext::make_shared<PerturbativeBarrierOptionEngine>(settings, stochProcess,
                                                                 order,
                                                                 zeroGamma);
...
void BarrierOptionTest::testLocalVolAndHestonComparison() {
    ...
    const ext::shared_ptr<BlackVarianceSurface> volTS =
        ext::make_shared<BlackVarianceSurface>(settings, 
    ...
    ext::shared_ptr<HestonProcess> hestonProcess =
        ext::make_shared<HestonProcess>(settings, rTS, qTS, s0, v0,
    ...
    ext::shared_ptr<PricingEngine> fdHestonEngine =
        ext::make_shared<FdHestonBarrierEngine>(settings, hestonModel, 100, 400, 50);
    
    ext::shared_ptr<PricingEngine> fdLocalVolEngine =
        ext::make_shared<FdBlackScholesBarrierEngine>(settings, localVolProcess,
                                                        100, 400, 0,
    ...
    BarrierOption barrierOption(settings, Barrier::DownOut, 
...
void BarrierOptionTest::testVannaVolgaSimpleBarrierValues() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        BarrierOption barrierOption(settings, values[i].barrierType,
                                    values[i].barrier,
                                    values[i].rebate,
                                    payoff,
                                    exercise);
        ...
        ext::shared_ptr<PricingEngine> vannaVolgaEngine =
            ext::make_shared<VannaVolgaBarrierEngine>(settings, 
                            volAtmQuote,
							vol25PutQuote,
							vol25CallQuote,
							Handle<Quote> (spot),
...
void BarrierOptionTest::testDividendBarrierOption() {
    ...
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, v, dc));

    const ext::shared_ptr<BlackScholesMertonProcess> bsProcess =
        ext::make_shared<BlackScholesMertonProcess>(settings, s0, qTS, rTS, volTS);
    ...
    for (Size i=0; i < LENGTH(barriers); ++i) {
        for (Size j=0; j < LENGTH(engines); ++j) {
            ...
            DividendBarrierOption barrierOption(settings, 
