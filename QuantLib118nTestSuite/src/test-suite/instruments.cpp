...
void InstrumentTest::testCompositeWhenShiftingDates() {
    ...
    ext::shared_ptr<Instrument> option(new EuropeanOption(settings, payoff, exercise));

    ext::shared_ptr<SimpleQuote> spot(new SimpleQuote(100.0));
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, 0.0, dc);
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, 0.01, dc);
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, 0.1, dc);
    ...
    ext::shared_ptr<PricingEngine> engine(new AnalyticEuropeanEngine(settings, process));
    ...
    settings->evaluationDate() = today+45;
