...
void NormalCLVModelTest::testHestonCumlativeDistributionFunction() {
    ...
    const Handle<BlackVolTermStructure> hestonVolTS(
        ext::make_shared<HestonBlackVolSurface>(settings,
            Handle<HestonModel>(ext::make_shared<HestonModel>(settings, process))));
    ...
    const HestonRNDCalculator rndCalculator(settings, process);
...
void NormalCLVModelTest::testIllustrative1DExample() {
    ...
    const Handle<BlackVolTermStructure> sabrVol(
        ext::make_shared<SABRVolTermStructure>(settings, 
...
void NormalCLVModelTest::testMonteCarloBSOptionPricing() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, qRate, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, rRate, dc));
    const Handle<BlackVolTermStructure> vTS(flatVol(settings, today, vol, dc));
    ...
    fdmOption.setPricingEngine(
        ext::make_shared<FdOrnsteinUhlenbeckVanillaEngine>(settings, 
    ...
    */
    ...
    const ext::shared_ptr<PricingEngine> analyticEngine(
        ext::make_shared<AnalyticDoubleBarrierBinaryEngine>(settings, 
            ext::make_shared<GeneralizedBlackScholesProcess>(settings, 
                spot, qTS, rTS,
                Handle<BlackVolTermStructure>(flatVol(settings, atmVol, dc)))));
