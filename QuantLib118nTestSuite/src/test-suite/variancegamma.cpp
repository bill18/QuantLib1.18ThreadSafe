...
void VarianceGammaTest::testVarianceGamma() {
    ...
    for (Size i=0; i<LENGTH(processes); i++) {
        ...
        ext::shared_ptr<PricingEngine> analyticEngine(
            new VarianceGammaEngine(settings, stochProcess));

        // FFT engine
        ext::shared_ptr<FFTVarianceGammaEngine> fftEngine(
            new FFTVarianceGammaEngine(settings, stochProcess));
        ...
        for (Size j=0; j<LENGTH(options); j++)
        {
            ...
            ext::shared_ptr<EuropeanOption> option(new EuropeanOption(settings, payoff, exercise));
...
void VarianceGammaTest::testSingularityAtZero() {
    ...
    settings->evaluationDate() = valuation;
    ...
    Handle<YieldTermStructure> dividend(
             ext::make_shared<FlatForward>(settings, valuation,0.0,discountCounter));
    Handle<YieldTermStructure> disc(
             ext::make_shared<FlatForward>(settings, valuation,0.05,discountCounter));
    ...
    option.setPricingEngine(ext::make_shared<VarianceGammaEngine>(settings, process));
