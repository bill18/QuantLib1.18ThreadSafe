...
void CliquetOptionTest::testValues() {
    ...
    ext::shared_ptr<PricingEngine> engine(new AnalyticCliquetEngine(settings, process));
    ...
    CliquetOption option(settings, payoff, exercise, reset);
...
namespace {
    ...
    template <class T>
    void testOptionGreeks(Settings* settings) {
          ...
          for (Size j=0; j<LENGTH(moneyness); j++) {
            for (Size k=0; k<LENGTH(lengths); k++) {
              for (Size kk=0; kk<LENGTH(frequencies); kk++) {
                ...
                ext::shared_ptr<PricingEngine> engine(new T(settings, process));

                CliquetOption option(settings, payoff, maturity, reset);
                option.setPricingEngine(engine);
...
void CliquetOptionTest::testGreeks() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testOptionGreeks<AnalyticCliquetEngine>(&env);
}
...
void CliquetOptionTest::testPerformanceGreeks() {
    ...
    Settings env;
    env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
    testOptionGreeks<AnalyticPerformanceEngine>(&env);
}
        ...
        for (Size k=0; k<LENGTH(lengths); k++) {
          for (Size kk=0; kk<LENGTH(frequencies); kk++) {
              ...
              ext::shared_ptr<PricingEngine> refEngine(
                                      new AnalyticPerformanceEngine(settings, process));

              ext::shared_ptr<PricingEngine> mcEngine =
                  MakeMCPerformanceEngine<PseudoRandom>(settings, process)
                  .withBrownianBridge()
