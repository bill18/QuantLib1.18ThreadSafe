    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            termStructure.linkTo(
                ext::shared_ptr<YieldTermStructure>(new
                    FlatForward(settings, 0, conventions.calendar,
                                0.05, Actual365Fixed())));
        }
        ...
        void makeCoherenceTest(
            ...
            ext::shared_ptr<BlackSwaptionEngine> engine(new
                BlackSwaptionEngine(settings, termStructure,
            ...
            for (Size j=0; j<atm.tenors.swaps.size(); j++) {
                ...
                ext::shared_ptr<SwapIndex> swapIndex(new
                    EuriborSwapIsdaFixA(settings, atm.tenors.swaps[j], termStructure));
                ...
                for (Size i=0; i<atm.tenors.options.size(); ++i) {
                    ...
                    // ATM swaption
                    Swaption swaption =
                        MakeSwaption(settings, swapIndex, atm.tenors.options[i])
...
void SwaptionVolatilityMatrixTest::testSwaptionVolMatrixObservability() {
    ...
    vol = ext::make_shared<SwaptionVolatilityMatrix>(
                                 vars.settings,
    ...
    vol = ext::make_shared<SwaptionVolatilityMatrix>(vars.settings,
                                 vars.settings->evaluationDate(),
                                 vars.conventions.calendar,
                                 vars.conventions.optionBdc,
                                 vars.atm.tenors.options,
