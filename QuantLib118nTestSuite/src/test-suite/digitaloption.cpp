    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new
            BlackScholesMertonProcess(settings, Handle<Quote>(spot),
                                      ...
                                      Handle<BlackVolTermStructure>(volTS)));
        ext::shared_ptr<PricingEngine> engine(
                             new AnalyticDigitalAmericanEngine(settings, stochProcess));

        VanillaOption opt(settings, payoff, amExercise);
        opt.setPricingEngine(engine);
...
void DigitalOptionTest::testCashAtExpiryOrNothingAmericanValues() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine;
        if (values[i].knockin)
           engine.reset(new AnalyticDigitalAmericanEngine(settings, stochProcess));
        else
           engine.reset(new AnalyticDigitalAmericanKOEngine(settings, stochProcess));
    ...
    for (Size j=0; j<LENGTH(engines); j++) {
      for (Size i1=0; i1<LENGTH(types); i1++) {
        for (Size i6=0; i6<LENGTH(strikes); i6++) {
          ...
          VanillaOption opt(settings, payoff, exercises[j]);
...
void DigitalOptionTest::testMCCashAtHit() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> mcldEngine =
            MakeMCDigitalEngine<LowDiscrepancy>(settings, stochProcess)
