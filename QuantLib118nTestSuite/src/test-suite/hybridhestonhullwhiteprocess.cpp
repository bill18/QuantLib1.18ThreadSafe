...
void HybridHestonHullWhiteProcessTest::testBsmHullWhiteEngine() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, qRate, dc));
    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.0525));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, rRate, dc));
    ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.25));
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, vol, dc));

    ext::shared_ptr<HullWhite> hullWhiteModel(
        new HullWhite(Handle<YieldTermStructure>(rTS), 0.00883, 0.00526));

    ext::shared_ptr<BlackScholesMertonProcess> stochProcess(
                      new BlackScholesMertonProcess(settings, spot, qTS, rTS, volTS));
    ...
    EuropeanOption option(settings, payoff, exercise);
    ...
    for (Size i=0; i < LENGTH(corr); ++i) {
        ext::shared_ptr<PricingEngine> bsmhwEngine(
                         new AnalyticBSMHullWhiteEngine(settings, corr[i], stochProcess,
                                                        hullWhiteModel));
        ...
        const Handle<BlackVolTermStructure> compVolTS(
                                        flatVol(settings, today, expectedVol[i], dc));

        ext::shared_ptr<BlackScholesMertonProcess> bsProcess(
                    new BlackScholesMertonProcess(settings, spot, qTS, rTS, compVolTS));
        ext::shared_ptr<PricingEngine> bsEngine(
                                       new AnalyticEuropeanEngine(settings, bsProcess));

        EuropeanOption comp(settings, payoff, exercise);
...
void HybridHestonHullWhiteProcessTest::testCompareBsmHWandHestonHW() {
    ...
    const Handle<YieldTermStructure> rTS(
       ext::shared_ptr<YieldTermStructure>(new ZeroCurve(settings, dates, rates, dc)));
    const Handle<YieldTermStructure> qTS(
       ext::shared_ptr<YieldTermStructure>(
                                          new ZeroCurve(settings, dates, divRates, dc)));
    ...
    ext::shared_ptr<HestonProcess> hestonProcess( 
                   new HestonProcess(settings, rTS, qTS, spot,
                                     vol->value()*vol->value(), 1.0,
                                     vol->value()*vol->value(), 1e-4, 0.0));

    ext::shared_ptr<HestonModel> hestonModel(new HestonModel(settings, hestonProcess));

    ext::shared_ptr<HullWhite> hullWhiteModel(
        new HullWhite(Handle<YieldTermStructure>(rTS), 0.01, 0.01));

    ext::shared_ptr<PricingEngine> bsmhwEngine(
             new AnalyticBSMHullWhiteEngine(settings, 0.0, bsmProcess, hullWhiteModel));

    ext::shared_ptr<PricingEngine> hestonHwEngine(
          new AnalyticHestonHullWhiteEngine(settings, hestonModel, hullWhiteModel, 128));
...
void HybridHestonHullWhiteProcessTest::testZeroBondPricing() {
    ...
    const Handle<YieldTermStructure> ts(
       ext::shared_ptr<YieldTermStructure>(new ZeroCurve(settings, dates, rates, dc)));
    const Handle<YieldTermStructure> ds(flatRate(settings, today, 0.0, dc));

    const ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, ts, ds, s0, 0.02, 1.0, 0.2, 0.5, -0.8));
...
void HybridHestonHullWhiteProcessTest::testMcVanillaPricing() {
    ...
    const ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
              new BlackScholesMertonProcess(settings, s0, qTS, rTS, volTS));
    const ext::shared_ptr<HestonProcess> hestonProcess(
              new HestonProcess(settings, rTS, qTS, s0, 0.0625, 0.5, 0.0625, 1e-5, 0.3));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(
    ...
    for (Size i=0; i < LENGTH(corr); ++i) {
        for (Size j=0; j < LENGTH(strike); ++j) {
            ...
            VanillaOption optionHestonHW(settings, payoff, exercise);
            ext::shared_ptr<PricingEngine> engine =
                MakeMCHestonHullWhiteEngine<PseudoRandom>(settings, jointProcess)
                .withSteps(1)
            ...
            VanillaOption optionBsmHW(settings, payoff, exercise);
            optionBsmHW.setPricingEngine(
                ext::shared_ptr<PricingEngine>(
                           new AnalyticBSMHullWhiteEngine(settings, corr[i], bsmProcess,
                                                          hwModel)));
...
void HybridHestonHullWhiteProcessTest::testMcPureHestonPricing() {
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(
              new HestonProcess(settings, rTS, qTS, s0, 0.08, 1.5, 0.0625, 0.5, -0.8));
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(
    ...
    for (Size i=0; i < LENGTH(corr); ++i) {
        for (Size j=0; j < LENGTH(strike); ++j) {
            ...
            VanillaOption optionHestonHW(settings, payoff, exercise);
            VanillaOption optionPureHeston(settings, payoff, exercise);
...
void HybridHestonHullWhiteProcessTest::testAnalyticHestonHullWhitePricing() {
    ...
    for (Size i=0; i < LENGTH(types); ++i) {
        for (Size j=0; j < LENGTH(strike); ++j) {
            ...
            optionPureHeston.setPricingEngine(
                ext::shared_ptr<PricingEngine>(
                    new AnalyticHestonHullWhiteEngine(settings, hestonModel,
                                                      hullWhiteModel, 128)));
...
void HybridHestonHullWhiteProcessTest::testCallableEquityPricing() {
    ...
    */
    ...
    ext::shared_ptr<SimpleQuote> qRate(new SimpleQuote(0.04));
    Handle<YieldTermStructure> qTS(flatRate(settings, today, qRate, dc));
    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.04));
    Handle<YieldTermStructure> rTS(flatRate(settings, today, rRate, dc));

    const ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, rTS, qTS, spot, 0.0625, 1.0,
                              0.24*0.24, 1e-4, 0.0));
    // FLOATING_POINT_EXCEPTION
    const ext::shared_ptr<HullWhiteForwardProcess> hwProcess(
    ...
    Schedule schedule(settings->evaluationDate(), today, today + Period(maturity, Years),
...
void HybridHestonHullWhiteProcessTest::testDiscretizationError() {
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(
           new HestonProcess(settings, rTS, qTS, s0, v*v, 1, v*v, 1e-6, -0.4));
...
void HybridHestonHullWhiteProcessTest::testFdmHestonHullWhiteEngine() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, 0.02, dc));

    const Volatility vol = 0.30;
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, vol, dc));

    const Real v0 = vol*vol;
    ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(settings, rTS, qTS, s0, v0, 1.0, v0, 0.000001, 0.0));
    ...
    for (Size i=0; i < LENGTH(corr); ++i) {
        for (Size j=0; j < LENGTH(strike); ++j) {
            ...
            option.setPricingEngine(ext::shared_ptr<PricingEngine>(
                new FdHestonHullWhiteVanillaEngine(settings, 
                    ext::make_shared<HestonModel>(settings, 
                        hestonProcess),
            ...
            option.setPricingEngine(ext::shared_ptr<PricingEngine>(
                new AnalyticBSMHullWhiteEngine(settings, corr[i],
...
namespace hybrid_heston_hullwhite_process_test {
    ...
    ext::shared_ptr<HestonProcess> makeHestonProcess(
        ...
        Handle<YieldTermStructure> rTS(flatRate(settings, params.r, dayCounter));
        Handle<YieldTermStructure> qTS(flatRate(settings, params.q, dayCounter));

        return ext::make_shared<HestonProcess>(settings, 
                   rTS, qTS, spot, params.v0, params.kappa,
                   params.theta, params.sigma, params.rho);
    }

    ext::shared_ptr<VanillaOption> makeVanillaOption(Settings* settings,
                                            const VanillaOptionData& params) {

        Date maturity = Date(settings->evaluationDate())
                                          + Period(Size(params.maturity*365), Days);
        ext::shared_ptr<Exercise> exercise(new EuropeanExercise(maturity));
        ext::shared_ptr<StrikedTypePayoff> payoff(
                    new PlainVanillaPayoff(params.optionType, params.strike));

        return ext::make_shared<VanillaOption>(settings, 
                                          payoff, exercise);
    }
}
...
void HybridHestonHullWhiteProcessTest::testBsmHullWhitePricing() {
    ...
    ext::shared_ptr<HestonProcess> hp(makeHestonProcess(settings, hestonModelData));
    ext::shared_ptr<HestonModel> hestonModel(new HestonModel(settings, hp));
    ...
    ext::shared_ptr<BlackScholesMertonProcess> bsmProcess(
        new BlackScholesMertonProcess(settings, 
            hp->s0(), hp->dividendYield(), hp->riskFreeRate(),
            Handle<BlackVolTermStructure>( 
                flatVol(settings, today, std::sqrt(hestonModelData.theta),
                        hp->riskFreeRate()->dayCounter()))));

    ext::shared_ptr<PricingEngine> bsmhwEngine(
                     new AnalyticBSMHullWhiteEngine(settings, equityIrCorr, bsmProcess,
                                                    hullWhiteModel));
            ...
            for (Size u=0; u < LENGTH(listOfTimeStepsPerYear); ++u) {
                ...
                for (Size k=0; k < LENGTH(strikes); ++k) {
                    ...
                    ext::shared_ptr<VanillaOption> option
                                        = makeVanillaOption(settings, optionData);
                    option->setPricingEngine(bsmhwEngine);
...
void HybridHestonHullWhiteProcessTest::testHestonHullWhiteCalibration() {
    ...
    const ext::shared_ptr<HestonProcess> hestonProcess(
        new HestonProcess(settings, rTS, qTS, s0, start_v0, start_kappa,
    ...
    const ext::shared_ptr<PricingEngine> analyticHestonEngine(
                         new AnalyticHestonEngine(settings, analyticHestonModel, 164));
    ...
    for (Size i=0; i < LENGTH(maturities); ++i) {
        ...
        for (Size j=0; j < LENGTH(strikes); ++j) {
            ...
            ext::shared_ptr<BlackCalibrationHelper> helper(
                new HestonModelHelper(settings, maturity, calendar, s0,
                                      strikes[j], v, rTS, qTS,
                ...
                QuantLib::detail::ImpliedVolatilityHelper::clone(
                    ...
                    ext::make_shared<GeneralizedBlackScholesProcess>(
                            ...
                            s0, qTS, rTS, Handle<BlackVolTermStructure>(
                                                    flatVol(settings, v->value(), dc))),
                        volQuote);

            VanillaOption dummyOption(settings, payoff, exercise);

            ext::shared_ptr<PricingEngine> bshwEngine(
                new AnalyticBSMHullWhiteEngine(settings, equityShortRateCorr,
    ...
    for (Size i=0; i < LENGTH(maturities); ++i) {
        ...
        ext::shared_ptr<FdHestonHullWhiteVanillaEngine> engine(
            new FdHestonHullWhiteVanillaEngine(settings, fdmHestonModel, hwProcess,
                                               equityShortRateCorr,
...
void HybridHestonHullWhiteProcessTest::testH1HWPricingEngine() {
    ...
    */
    ...
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, r, dc));
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, q, dc));

    const Handle<BlackVolTermStructure> flatVolTS(flatVol(settings, today, 0.20, dc));
    const ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
        new GeneralizedBlackScholesProcess(settings, s0, qTS, rTS, flatVolTS));
    ...
    for (Size j=0; j < LENGTH(sigma_v); ++j) {
        const ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, rTS, qTS, s0, v0, kappa_v, theta,
                              sigma_v[j], rho_sv));
        const ext::shared_ptr<HestonModel> hestonModel(
            new HestonModel(settings, hestonProcess));
        ...
        for (Size i=0; i < LENGTH(strikes); ++i) {
            ...
            const ext::shared_ptr<PricingEngine> analyticH1HWEngine(
                new AnalyticH1HWEngine(settings, hestonModel, hullWhiteModel,
                                       rho_sr, 144));
