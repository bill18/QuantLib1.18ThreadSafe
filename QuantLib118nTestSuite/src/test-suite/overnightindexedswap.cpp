...
namespace overnight_indexed_swap_test {
    ...
    struct CommonVars {
        ...
        ext::shared_ptr<OvernightIndexedSwap> makeSwap(Period length,
            ...
            return MakeOIS(settings, length, eoniaIndex, fixedRate)
        ...
        CommonVars() {
            ...
            eoniaIndex = ext::make_shared<Eonia>(settings, eoniaTermStructure);
            fixedSwapConvention = ModifiedFollowing;
            fixedSwapFrequency = Annual;
            fixedSwapDayCount = Thirty360();
            swapIndex = ext::shared_ptr<IborIndex>(new Euribor3M(settings, swapTermStructure));
            ...
            eoniaTermStructure.linkTo(flatRate(settings, today, 0.05,
                                               Actual365Fixed()));
        }
    };
...
void OvernightIndexedSwapTest::testCachedValue() {
    ...
    Real flat = 0.05;
    vars.eoniaTermStructure.linkTo(flatRate(vars.settings, vars.settlement,flat,Actual360()));
...
void testBootstrap(bool telescopicValueDates) {
    ...
    ext::shared_ptr<IborIndex> euribor3m(new Euribor3M(vars.settings));
    ext::shared_ptr<Eonia> eonia(new Eonia(vars.settings));
    ...
    for (Size i = 0; i < LENGTH(depositData); i++) {
        ...
        ext::shared_ptr<RateHelper> helper(new
                    DepositRateHelper(vars.settings, Handle<Quote>(quote),
                                      term,
    ...
    for (Size i = 0; i < LENGTH(eoniaSwapData); i++) {
        ...
        ext::shared_ptr<RateHelper> helper(new
                     OISRateHelper(vars.settings, eoniaSwapData[i].settlementDays,
                                   term,
                                   Handle<Quote>(quote),
                                   eonia,
    ...
    ext::shared_ptr<PiecewiseFlatForward> eoniaTS(
        new PiecewiseFlatForward (vars.settings, vars.today, eoniaHelpers, Actual365Fixed()));
...
void OvernightIndexedSwapTest::testBootstrapRegression() {
    ...
    settings->evaluationDate() = Date(21, February, 2017);

    std::vector<ext::shared_ptr<RateHelper> > helpers;
    ext::shared_ptr<FedFunds> index(new FedFunds(settings));

    helpers.push_back(
        ext::make_shared<DepositRateHelper>(settings, data[0].rate,
    ...
    for (Size i=1; i<LENGTH(data); ++i) {
        helpers.push_back(
            ext::shared_ptr<RateHelper>(
                new OISRateHelper(settings, data[i].settlementDays,
    ...
    PiecewiseYieldCurve<Discount,LogCubic> curve(settings, 0, UnitedStates(), helpers, Actual365Fixed(),
