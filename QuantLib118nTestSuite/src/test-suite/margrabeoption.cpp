    ...
    */
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                             new AnalyticEuropeanMargrabeEngine(settings, stochProcess1,
                                                                stochProcess2,
                                                                values[i].rho));

        MargrabeOption margrabeOption(settings, values[i].Q1, values[i].Q2, exercise);
...
void MargrabeOptionTest::testGreeks() {
    ...
    ext::shared_ptr<SimpleQuote> qRate1(new SimpleQuote(0.0));
    ext::shared_ptr<YieldTermStructure> qTS1 = flatRate(settings, qRate1, dc);
    ext::shared_ptr<SimpleQuote> qRate2(new SimpleQuote(0.0));
    ext::shared_ptr<YieldTermStructure> qTS2 = flatRate(settings, qRate2, dc);

    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.0));
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, rRate, dc);

    ext::shared_ptr<SimpleQuote> vol1(new SimpleQuote(0.0));
    ext::shared_ptr<BlackVolTermStructure> volTS1 = flatVol(settings, vol1, dc);
    ext::shared_ptr<SimpleQuote> vol2(new SimpleQuote(0.0));
    ext::shared_ptr<BlackVolTermStructure> volTS2 = flatVol(settings, vol2, dc);
    ...
    for (Size k=0; k<LENGTH(residualTimes); k++) {
          ...
          MargrabeOption margrabeOption(settings, 1, 1, exercise);
...
void MargrabeOptionTest::testAmericanExchangeTwoAssets() {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        ext::shared_ptr<PricingEngine> engine(
                             new AnalyticAmericanMargrabeEngine(settings, stochProcess1,
                                                                stochProcess2,
                                                                values[i].rho));
