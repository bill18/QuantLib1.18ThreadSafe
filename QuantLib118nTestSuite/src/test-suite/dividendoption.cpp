      ...
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
          ...
          ext::shared_ptr<PricingEngine> engine(
                            new AnalyticDividendEuropeanEngine(settings, stochProcess));

          DividendVanillaOption option(settings, payoff, exercise,
                                       dividendDates, dividends);
          option.setPricingEngine(engine);

          VanillaOption ref_option(settings, payoff, exercise);
    ...
    for (Size i=0; i<LENGTH(types); i++) {
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
          ...
          VanillaOption ref_option(settings, refPayoff, exercise);
          ref_option.setPricingEngine(ref_engine);
    ...
    for (Size i=0; i<LENGTH(types); i++) {
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
          ...
          ext::shared_ptr<PricingEngine> engine =
              MakeFdBlackScholesVanillaEngine(settings, stochProcess)
              .withTGrid(timeSteps)
          ...
          DividendVanillaOption ref_option(settings, payoff, exercise,
                                           dividendDates, dividends);
          ref_option.setPricingEngine(ref_engine);
...
namespace {
    ...
    void testFdGreeks(Settings* settings, const Date& today,
...
void DividendOptionTest::testFdEuropeanGreeks() {
    ...
    for (Size i=0; i<LENGTH(lengths); i++) {
        ...
        ext::shared_ptr<Exercise> exercise(new EuropeanExercise(exDate));
        testFdGreeks(settings, today,exercise,FdBlackScholesVanillaEngine::Spot);
        testFdGreeks(settings, today,exercise,FdBlackScholesVanillaEngine::Escrowed);
    }
}
...
namespace {
    ...
    void testFdDegenerate(Settings* settings, const Date& today,
                          const ext::shared_ptr<Exercise>& exercise,
                          FdBlackScholesVanillaEngine::CashDividendModel model) {

        DayCounter dc = Actual360();
        ext::shared_ptr<SimpleQuote> spot(new SimpleQuote(54.625));
        Handle<YieldTermStructure> rTS(flatRate(settings, 0.052706, dc));
        Handle<YieldTermStructure> qTS(flatRate(settings, 0.0, dc));
        Handle<BlackVolTermStructure> volTS(flatVol(settings, 0.282922, dc));
        ...
        ext::shared_ptr<PricingEngine> engine =
              MakeFdBlackScholesVanillaEngine(settings, process)
              .withTGrid(timeSteps)
              .withXGrid(gridPoints)
              .withCashDividendModel(model);
        ...
        DividendVanillaOption option1(settings, payoff, exercise,
                                      dividendDates, dividends);
...
void DividendOptionTest::testFdEuropeanDegenerate() {
    ...
    testFdDegenerate(settings, today,exercise,FdBlackScholesVanillaEngine::Spot);
    testFdDegenerate(settings, today,exercise,FdBlackScholesVanillaEngine::Escrowed);
}
...
namespace {
    ...
    void testFdDividendAtTZero(Settings* settings, const Date& today,
                               const ext::shared_ptr<Exercise>& exercise,
                               FdBlackScholesVanillaEngine::CashDividendModel model) {

        DayCounter dc = Actual360();
        ext::shared_ptr<SimpleQuote> spot(new SimpleQuote(54.625));
        Handle<YieldTermStructure> rTS(flatRate(settings, 0.0, dc));
        Handle<BlackVolTermStructure> volTS(flatVol(settings, 0.282922, dc));
        ...
        DividendVanillaOption europeanOption(settings, 
            payoff, europeanExercise, dividendDates, dividends);

        europeanOption.setPricingEngine(
            ext::make_shared<AnalyticDividendEuropeanEngine>(settings, process));
...
void DividendOptionTest::testFdEuropeanWithDividendToday() {
    ...
    testFdDividendAtTZero(settings, today,exercise,FdBlackScholesVanillaEngine::Spot);
    testFdDividendAtTZero(settings, today,exercise,FdBlackScholesVanillaEngine::Escrowed);
}
...
void DividendOptionTest::testEscrowedDividendModel() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.063, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.094, dc));
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, 0.3, dc));
