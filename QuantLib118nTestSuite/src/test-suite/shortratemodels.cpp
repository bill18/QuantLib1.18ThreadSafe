...
void ShortRateModelTest::testCachedHullWhite() {
    ...
    Handle<YieldTermStructure> termStructure(flatRate(settings, settlement,0.04875825,
                                                      Actual365Fixed()));
    ...
    ext::shared_ptr<PricingEngine> engine(
                                         new JamshidianSwaptionEngine(settings, model));
    ...
    for (Size i=0; i<LENGTH(data); i++) {
        ...
        ext::shared_ptr<BlackCalibrationHelper> helper(
                             new SwaptionHelper(settings, Period(data[i].start, Years),
...
void ShortRateModelTest::testCachedHullWhite2() {
    ...
    CalibrationData data[] = {{ 1, 5, 0.1148 },
                              ...
                              { 5, 1, 0.1000 }};
    ext::shared_ptr<IborIndex> index(new Euribor6M(settings, termStructure));
    ext::shared_ptr<IborIndex> index0(new IborIndex(settings, 
...
void ShortRateModelTest::testSwaps() {
    ...
    ext::shared_ptr<IborIndex> euribor(new Euribor6M(settings, termStructure));

    ext::shared_ptr<PricingEngine> engine(
                                        new TreeVanillaSwapEngine(settings, model,120));
    ...
    for (Size i=0; i<LENGTH(start); i++) {
        ...
        for (Size j=0; j<LENGTH(length); j++) {
            ...
            Schedule fixedSchedule(settings->evaluationDate(), startDate, maturity, Period(Annual),
                                   calendar, Unadjusted, Unadjusted,
                                   DateGeneration::Forward, false);
            Schedule floatSchedule(settings->evaluationDate(), startDate, maturity, Period(Semiannual),
                                   calendar, Following, Following,
                                   DateGeneration::Forward, false);
            for (Size k=0; k<LENGTH(rates); k++) {

                VanillaSwap swap(settings, VanillaSwap::Payer, 1000000.0,
...
void ShortRateModelTest::testExtendedCoxIngersollRossDiscountFactor() {
    ...
    const Handle<YieldTermStructure> rTS(
        flatRate(settings, today, rate, Actual365Fixed()));
