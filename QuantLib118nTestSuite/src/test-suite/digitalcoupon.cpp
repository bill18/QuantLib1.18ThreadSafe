...
namespace digital_coupon_test {
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            fixingDays = 2;
            nominal = 1000000.0;
            index = ext::shared_ptr<IborIndex>(new Euribor6M(settings, termStructure));
            ...
            termStructure.linkTo(flatRate(settings, settlement,0.05,Actual365Fixed()));
    ...
    */
    ...
    for (Size i = 0; i< LENGTH(vols); i++) {
            ...
            vol.linkTo(ext::shared_ptr<OptionletVolatilityStructure>(new
                ConstantOptionletVolatility(vars.settings, vars.today,
            ...
            for (Size k=9; k<10; k++) {
                ...
                for (Size h=0; h<LENGTH(gearings); h++) {
                    ...
                    ext::shared_ptr<FloatingRateCoupon> underlying(new
                        IborCoupon(vars.settings, paymentDate, vars.nominal,
                                   startDate, endDate,
                                   vars.fixingDays, vars.index,
                                   gearing, spread));
                    // Floating Rate Coupon - Call Digital option
                    DigitalCoupon digitalCappedCoupon(vars.settings, underlying,
                    ...
                    ext::shared_ptr<IborCouponPricer> pricer(new
                                                            BlackIborCouponPricer(vars.settings, vol));
                    ...
                    if (spread==0.0) {
                        ...
                        ext::shared_ptr<YieldTermStructure>
                            qTS = flatRate(vars.settings, vars.today, qRate, Actual360());
                        ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.0));
                        ext::shared_ptr<BlackVolTermStructure>
                            volTS = flatVol(vars.settings, vars.today, capletVol, Actual360());
                        ext::shared_ptr<StrikedTypePayoff>
                            callPayoff(new AssetOrNothingPayoff(Option::Call,effStrike));
                        ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new
                            BlackScholesMertonProcess(vars.settings, Handle<Quote>(fwd),
                                              Handle<YieldTermStructure>(qTS),
                                              Handle<YieldTermStructure>(vars.termStructure),
                                              Handle<BlackVolTermStructure>(volTS)));
                        ext::shared_ptr<PricingEngine>
                            engine(new AnalyticEuropeanEngine(vars.settings, stochProcess));
                        VanillaOption callOpt(vars.settings, callPayoff, exercise);
                    ...
                    DigitalCoupon digitalFlooredCoupon(vars.settings, underlying,
                    ...
                    if (spread==0.0) {
                        ...
                        ext::shared_ptr<PricingEngine> engine(new AnalyticEuropeanEngine(vars.settings, stochProcess));
                        VanillaOption putOpt(vars.settings, putPayoff, exercise);
...
void DigitalCouponTest::testAssetOrNothingDeepInTheMoney() {
    ...
    volatility.linkTo(ext::shared_ptr<OptionletVolatilityStructure>(new
        ConstantOptionletVolatility(vars.settings, vars.today, vars.calendar, Following,
    ...
    for (Size k = 0; k<10; k++) {   // Loop on start and end dates
        ...
        DigitalCoupon digitalCappedCoupon(vars.settings, underlying,
                                          ...
                                          replication);
        ext::shared_ptr<IborCouponPricer> pricer(new
            BlackIborCouponPricer(vars.settings, volatility));
        digitalCappedCoupon.setPricer(pricer);
...
void DigitalCouponTest::testAssetOrNothingDeepOutTheMoney() {
    ...
    for (Size k = 0; k<10; k++) { // loop on start and end dates
        ...
        ext::shared_ptr<IborCouponPricer> pricer(new BlackIborCouponPricer(vars.settings, volatility));
        digitalCappedCoupon.setPricer(pricer);
        ...
        for (Size j = 0; j< LENGTH(strikes); j++) {
            ...
            for (Size k = 0; k<10; k++) {
                ...
                DigitalCoupon digitalCappedCoupon(vars.settings, underlying,
                                          ...
                                          nullstrike, Position::Short, false, nullstrike,
                                          replication);
                ext::shared_ptr<IborCouponPricer> pricer(new BlackIborCouponPricer(vars.settings, vol));
                digitalCappedCoupon.setPricer(pricer);
                ...
                ext::shared_ptr<SimpleQuote> qRate(new SimpleQuote(0.0));
                ext::shared_ptr<YieldTermStructure> qTS = flatRate(vars.settings, vars.today, qRate, Actual360());
                ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.0));
                ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(vars.settings, vars.today, capletVol,
                                                                         Actual360());
    ...
    for (Size i = 0; i< LENGTH(vols); i++) {
        ...
        for (Size j = 0; j< LENGTH(strikes); j++) {
            ...
            for (Size k = 0; k<10; k++) {
                ...
                Rate cashRate = 0.01;
                // Floating Rate Coupon + Call Digital option
                DigitalCoupon cash_digitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Long, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          replication);
                ext::shared_ptr<IborCouponPricer> pricer(new
                ...
                // Floating Rate Coupon - Put Digital option
                DigitalCoupon cash_digitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Short, false, cashRate,
                                          replication);
                ...
                // Asset-or-Nothing
                // Floating Rate Coupon + Call Digital option
                DigitalCoupon asset_digitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Long, false, nullstrike,
                                          nullstrike, Position::Long, false, nullstrike,
                                          replication);
                asset_digitalCallCoupon.setPricer(pricer);
                // Floating Rate Coupon - Put Digital option
                DigitalCoupon asset_digitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Short, false, nullstrike,
                                          replication);
    ...
    for (Size i = 0; i< LENGTH(vols); i++) {
        ...
        for (Size j = 0; j< LENGTH(strikes); j++) {
            ...
            for (Size k = 0; k<10; k++) {
                ...
                Rate cashRate = 0.005;
                // Floating Rate Coupon + Call Digital option
                DigitalCoupon sub_cash_longDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Long, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          subReplication);
                DigitalCoupon central_cash_longDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Long, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          centralReplication);
                DigitalCoupon over_cash_longDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Long, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          superReplication);
                ext::shared_ptr<IborCouponPricer> pricer(new
                ...
                // Floating Rate Coupon - Call Digital option
                DigitalCoupon sub_cash_shortDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Short, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          subReplication);
                DigitalCoupon central_cash_shortDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Short, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          centralReplication);
                DigitalCoupon over_cash_shortDigitalCallCoupon(vars.settings, underlying,
                                          strike, Position::Short, false, cashRate,
                                          nullstrike, Position::Long, false, nullstrike,
                                          superReplication);
                ...
                if ( ( (sub_digitalPrice > central_digitalPrice) &&
                    ...
                    BOOST_ERROR("\nCash-or-nothing: Floating Rate Coupon - Call Digital option" <<
                                ...
                                "\nOver-Replication Price = "  << over_digitalPrice);
                // Floating Rate Coupon + Put Digital option
                DigitalCoupon sub_cash_longDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Long, false, cashRate,
                                          subReplication);
                DigitalCoupon central_cash_longDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Long, false, cashRate,
                                          centralReplication);
                DigitalCoupon over_cash_longDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Long, false, cashRate,
                                          superReplication);
                ...
                // Floating Rate Coupon - Put Digital option
                DigitalCoupon sub_cash_shortDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Short, false, cashRate,
                                          subReplication);
                DigitalCoupon central_cash_shortDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Short, false, cashRate,
                                          centralReplication);
                DigitalCoupon over_cash_shortDigitalPutCoupon(vars.settings, underlying,
                                          nullstrike, Position::Long, false, nullstrike,
                                          strike, Position::Short, false, cashRate,
                                          superReplication);
