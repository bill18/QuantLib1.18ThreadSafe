...
void FdmLinearOpTest::testFdmHestonExpress() {
    ...
    Handle<HestonProcess> hestonProcess(ext::make_shared<HestonProcess> (settings,
    ...
    DividendSchedule dividendSchedule(1, ext::shared_ptr<Dividend>(
        new FixedDividend(settings, 2.5, evaluationDate + Period(6, Months))));
...
namespace {
    ...
    ext::shared_ptr<HybridHestonHullWhiteProcess> createHestonHullWhite(
        ...
        const Date today = settings->evaluationDate();
        Handle<Quote> s0(ext::shared_ptr<Quote>(new SimpleQuote(100.0)));
        ...
        const Real v0 = 0.04;
        ext::shared_ptr<HestonProcess> hestonProcess(
            new HestonProcess(settings, rTS, qTS, s0, v0, 1.0, v0*0.75, 0.4, -0.7));
...
void FdmLinearOpTest::testFdmHestonHullWhiteOp() {
    ...
    ext::shared_ptr<HybridHestonHullWhiteProcess> jointProcess
                                            = createHestonHullWhite(settings, maturity);
...
void FdmLinearOpTest::testCrankNicolsonWithDamping() {
    ...
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, today, 0.06, dc);
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, today, 0.06, dc);
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, today, 0.35, dc);
    ...
    VanillaOption opt(settings, payoff, exercise);
...
void FdmLinearOpTest::testHighInterestRateBlackScholesMesher() {
    ...
    const ext::shared_ptr<GeneralizedBlackScholesProcess> process =
        ext::make_shared<GeneralizedBlackScholesProcess>(settings,
            ...
            Handle<YieldTermStructure>(flatRate(settings, today, q, dc)),
            Handle<YieldTermStructure>(flatRate(settings, today, r, dc)),
            Handle<BlackVolTermStructure>(flatVol(settings, today, v, dc)));
    ...
    const std::vector<Real> loc = FdmBlackScholesMesher(settings,
...
void FdmLinearOpTest::testLowVolatilityHighDiscreteDividendBlackScholesMesher() {
    ...
    const Handle<YieldTermStructure> qTS(flatRate(settings, today, 0.07, dc));
    const Handle<YieldTermStructure> rTS(flatRate(settings, today, 0.16, dc));
    const Handle<BlackVolTermStructure> volTS(flatVol(settings, today, 0.0, dc));
    ...
    divSchedule.push_back(
        ext::make_shared<FixedDividend>(settings, firstDivAmount, firstDivDate));
    divSchedule.push_back(
        ext::make_shared<FixedDividend>(settings, secondDivAmount, secondDivDate));
