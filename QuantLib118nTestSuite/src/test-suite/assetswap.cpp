...
namespace asset_swap_test {
    ...
    struct CommonVars {
        ...
        CommonVars() {
            settings = &env;
            env.evaluationDate() = EvaluationDateSingleton::instance().getDate();
            ...
            iborIndex = ext::shared_ptr<IborIndex>(
                     new Euribor(settings, Period(floatingFrequency), termStructure));
            Calendar calendar = iborIndex->fixingCalendar();
            swapIndex= ext::make_shared<SwapIndex>(settings, 
...
void AssetSwapTest::testConsistency() {
    ...
    Schedule bondSchedule(vars.settings->evaluationDate(), Date(4,January,2005),
                          Date(4,January,2037),
                          Period(Annual), bondCalendar,
                          Unadjusted, Unadjusted,
                          DateGeneration::Backward, false);
    ext::shared_ptr<Bond> bond(new
        FixedRateBond(vars.settings, settlementDays, vars.faceAmount,
                      bondSchedule,
    ...
    bool isPar = true;
    AssetSwap parAssetSwap(vars.settings, payFixedRate,
                         bond, bondPrice,
    ...
    ext::shared_ptr<PricingEngine> swapEngine(new
        DiscountingSwapEngine(vars.settings, vars.termStructure,
                              true,
                              bond->settlementDate(),
    ...
    AssetSwap assetSwap2(vars.settings, payFixedRate,
                         bond, fairCleanPrice,
                         vars.iborIndex, vars.spread,
                         Schedule(),
                         vars.iborIndex->dayCounter(),
    ...
    AssetSwap assetSwap3(vars.settings, payFixedRate,
                         bond, bondPrice,
    ...
    assetSwap2 = AssetSwap(vars.settings, payFixedRate,
                           bond, fairCleanPrice,
                           vars.iborIndex, vars.spread,
                           Schedule(),
                           vars.iborIndex->dayCounter(),
    ...
    assetSwap3 = AssetSwap(vars.settings, payFixedRate,
                           bond, bondPrice,
                           vars.iborIndex, fairSpread,
                           Schedule(),
                           vars.iborIndex->dayCounter(),
    ...
    isPar = false;
    AssetSwap mktAssetSwap(vars.settings, payFixedRate,
                           bond, bondPrice,
                           vars.iborIndex, vars.spread,
                           Schedule(),
                           vars.iborIndex->dayCounter(),
    ...
    AssetSwap assetSwap4(vars.settings, payFixedRate,
                         bond, fairCleanPrice,
                         vars.iborIndex, vars.spread,
                         Schedule(),
                         vars.iborIndex->dayCounter(),
    ...
    AssetSwap assetSwap5(vars.settings, payFixedRate,
                         bond, bondPrice,
                         vars.iborIndex, fairSpread,
                         Schedule(),
                         vars.iborIndex->dayCounter(),
    ...
    assetSwap4 = AssetSwap(vars.settings, payFixedRate,
                           bond, fairCleanPrice,
                           vars.iborIndex, vars.spread,
                           Schedule(),
                           vars.iborIndex->dayCounter(),
    ...
    if (std::fabs(assetSwap4.fairSpread() - vars.spread)>tolerance) {
     ...
     assetSwap5 = AssetSwap(vars.settings, payFixedRate,
                            bond, bondPrice,
                            vars.iborIndex, fairSpread,
                            Schedule(),
                            vars.iborIndex->dayCounter(),
...
void AssetSwapTest::testImpliedValue() {
    ...
    Schedule fixedBondSchedule1(vars.settings->evaluationDate(), Date(4,January,2005),
                                Date(4,January,2037),
                                Period(Annual), bondCalendar,
                                Unadjusted, Unadjusted,
                                DateGeneration::Backward, false);
    ext::shared_ptr<Bond> fixedBond1(
                         new FixedRateBond(vars.settings, settlementDays, vars.faceAmount,
                                           fixedBondSchedule1,
                                           std::vector<Rate>(1, 0.04),
                                           ActualActual(ActualActual::ISDA),
                                           Following,
    ...
    ext::shared_ptr<PricingEngine> bondEngine(
                            new DiscountingBondEngine(vars.settings, vars.termStructure));
    ext::shared_ptr<PricingEngine> swapEngine(
                            new DiscountingSwapEngine(vars.settings, vars.termStructure));
    fixedBond1->setPricingEngine(bondEngine);

    Real fixedBondPrice1 = fixedBond1->cleanPrice();
    AssetSwap fixedBondAssetSwap1(vars.settings, payFixedRate,
    ...
    Schedule fixedBondSchedule2(vars.settings->evaluationDate(), Date(5,February,2005),
    ...
    AssetSwap fixedBondAssetSwap2(vars.settings, payFixedRate,
    ...
    Schedule floatingBondSchedule1(vars.settings->evaluationDate(), Date(29,September,2003),
    ...
    ext::shared_ptr<Bond> floatingBond1(
                      new FloatingRateBond(vars.settings, settlementDays, vars.faceAmount,
    ...
    AssetSwap floatingBondAssetSwap1(vars.settings, payFixedRate,
    ...
    Schedule floatingBondSchedule2(vars.settings->evaluationDate(), Date(24,September,2004),
    ...
    AssetSwap floatingBondAssetSwap2(vars.settings, payFixedRate,
    ...
    Schedule cmsBondSchedule1(vars.settings->evaluationDate(), Date(22,August,2005),
                              Date(22,August,2020),
                              Period(Annual), bondCalendar,
                              Unadjusted, Unadjusted,
                              DateGeneration::Backward, false);
    ext::shared_ptr<Bond> cmsBond1(
                          new CmsRateBond(vars.settings, settlementDays, vars.faceAmount,
                                          cmsBondSchedule1,
    ...
    AssetSwap cmsBondAssetSwap1(vars.settings, payFixedRate,
    ...
    Schedule cmsBondSchedule2(vars.settings->evaluationDate(), Date(06,May,2005),
                              Date(06,May,2015),
                              Period(Annual), bondCalendar,
                              Unadjusted, Unadjusted,
                              DateGeneration::Backward, false);
    ext::shared_ptr<Bond> cmsBond2(new
        CmsRateBond(vars.settings, settlementDays, vars.faceAmount, cmsBondSchedule2,
                    vars.swapIndex, Thirty360(),
                    Following, fixingDays,
    ...
    AssetSwap cmsBondAssetSwap2(vars.settings, payFixedRate,
    ...
    ext::shared_ptr<Bond> zeroCpnBond1(new
        ZeroCouponBond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    AssetSwap zeroCpnAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap zeroCpnAssetSwap2(vars.settings, payFixedRate,
...
void AssetSwapTest::testMarketASWSpread() {
    ...
    ext::shared_ptr<Bond> fixedBond1(new
        FixedRateBond(vars.settings, settlementDays, vars.faceAmount, fixedBondSchedule1,
                      std::vector<Rate>(1, 0.04),
    ...
    AssetSwap fixedBondParAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap fixedBondMktAssetSwap1(vars.settings, payFixedRate,
                                     fixedBond1, fixedBondMktPrice1,
                                     vars.iborIndex, vars.spread,
                                     Schedule(),
                                     vars.iborIndex->dayCounter(),
    ...
    ext::shared_ptr<Bond> fixedBond2(new
        FixedRateBond(vars.settings, settlementDays, vars.faceAmount, fixedBondSchedule2,
                      std::vector<Rate>(1, 0.05),
    ...
    AssetSwap fixedBondParAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap fixedBondMktAssetSwap2(vars.settings, payFixedRate,
                                     fixedBond2, fixedBondMktPrice2,
                                     vars.iborIndex, vars.spread,
                                     Schedule(),
                                     vars.iborIndex->dayCounter(),
    ...
    ext::shared_ptr<Bond> floatingBond1(new
        FloatingRateBond(vars.settings, settlementDays, vars.faceAmount,
                         floatingBondSchedule1,
                         vars.iborIndex, Actual360(),
                         Following, fixingDays,
    ...
    AssetSwap floatingBondParAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap floatingBondMktAssetSwap1(vars.settings, payFixedRate,
                                        floatingBond1, floatingBondMktPrice1,
                                        vars.iborIndex, vars.spread,
                                        Schedule(),
                                        vars.iborIndex->dayCounter(),
    ...
    AssetSwap floatingBondParAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap floatingBondMktAssetSwap2(vars.settings, payFixedRate,
                                        floatingBond2, floatingBondMktPrice2,
                                        vars.iborIndex, vars.spread,
                                        Schedule(),
                                        vars.iborIndex->dayCounter(),
    ...
    ext::shared_ptr<Bond> cmsBond1(new
        CmsRateBond(vars.settings, settlementDays, vars.faceAmount, cmsBondSchedule1,
                    vars.swapIndex, Thirty360(),
                    Following, fixingDays,
    ...
    AssetSwap cmsBondParAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap cmsBondMktAssetSwap1(vars.settings, payFixedRate,
                                   cmsBond1, cmsBondMktPrice1,
                                   vars.iborIndex, vars.spread,
                                   Schedule(),
                                   vars.iborIndex->dayCounter(),
    ...
    AssetSwap cmsBondParAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap cmsBondMktAssetSwap2(vars.settings, payFixedRate,
                                   cmsBond2, cmsBondMktPrice2,
                                   vars.iborIndex, vars.spread,
                                   Schedule(),
                                   vars.iborIndex->dayCounter(),
    ...
    AssetSwap zeroCpnBondParAssetSwap1(vars.settings, payFixedRate,zeroCpnBond1,
    ...
    AssetSwap zeroCpnBondMktAssetSwap1(vars.settings, payFixedRate,zeroCpnBond1,
                                       zeroCpnBondMktPrice1,
                                       vars.iborIndex, vars.spread,
                                       Schedule(),
                                       vars.iborIndex->dayCounter(),
    ...
    AssetSwap zeroCpnBondParAssetSwap2(vars.settings, payFixedRate,zeroCpnBond2,
    ...
    AssetSwap zeroCpnBondMktAssetSwap2(vars.settings, payFixedRate,zeroCpnBond2,
                                       zeroCpnBondMktPrice2,
                                       vars.iborIndex, vars.spread,
                                       Schedule(),
                                       vars.iborIndex->dayCounter(),
...
void AssetSwapTest::testZSpread() {
    ...
    Real fixedBondCleanPrice1 = BondFunctions::cleanPrice(vars.settings, 
    ...
    Real fixedBondCleanPrice2 = BondFunctions::cleanPrice(vars.settings, 
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real floatingBondCleanPrice1 = BondFunctions::cleanPrice(vars.settings, 
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real floatingBondCleanPrice2 = BondFunctions::cleanPrice(vars.settings, 
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real cmsBondCleanPrice1 = BondFunctions::cleanPrice(vars.settings, 
    ...
    // standard market conventions:
    // bond's frequency + coumpounding and daycounter of the YieldCurve
    Real cmsBondCleanPrice2 = BondFunctions::cleanPrice(vars.settings, 
    ...
    Real zeroCpnBondCleanPrice1 =
        BondFunctions::cleanPrice(vars.settings, *zeroCpnBond1,
                              *vars.termStructure,
                              vars.spread,
                              Actual365Fixed(),
    ...
    Real zeroCpnBondCleanPrice2 =
        BondFunctions::cleanPrice(vars.settings, *zeroCpnBond2,
                              *vars.termStructure,
                              vars.spread,
                              Actual365Fixed(),
                              vars.compounding, Annual,
...
void AssetSwapTest::testGenericBondImplied() {
    ...
    Schedule fixedBondSchedule1(vars.settings->evaluationDate(), fixedBondStartDate1,
                                fixedBondMaturityDate1,
                                Period(Annual), bondCalendar,
                                Unadjusted, Unadjusted,
                                DateGeneration::Backward, false);
    Leg fixedBondLeg1 = FixedRateLeg(vars.settings, fixedBondSchedule1)
    ...
    fixedBondLeg1.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, fixedbondRedemption1)));
    ext::shared_ptr<Bond> fixedBond1(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Schedule fixedBondSchedule2(vars.settings->evaluationDate(), fixedBondStartDate2,
                                fixedBondMaturityDate2,
                                Period(Annual), bondCalendar,
                                Unadjusted, Unadjusted,
                                DateGeneration::Backward, false);
    Leg fixedBondLeg2 = FixedRateLeg(vars.settings, fixedBondSchedule2)
        .withNotionals(vars.faceAmount)
    ...
    fixedBondLeg2.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, fixedbondRedemption2)));
    ext::shared_ptr<Bond> fixedBond2(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Schedule floatingBondSchedule1(vars.settings->evaluationDate(), floatingBondStartDate1,
                                   floatingBondMaturityDate1,
                                   Period(Semiannual), bondCalendar,
                                   Unadjusted, Unadjusted,
                                   DateGeneration::Backward, false);
    Leg floatingBondLeg1 = IborLeg(vars.settings, floatingBondSchedule1, vars.iborIndex)
        .withNotionals(vars.faceAmount)
    ...
    floatingBondLeg1.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, floatingbondRedemption1)));
    ext::shared_ptr<Bond> floatingBond1(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Schedule floatingBondSchedule2(vars.settings->evaluationDate(), floatingBondStartDate2,
                                   floatingBondMaturityDate2,
                                   Period(Semiannual), bondCalendar,
                                   ModifiedFollowing, ModifiedFollowing,
                                   DateGeneration::Backward, false);
    Leg floatingBondLeg2 = IborLeg(vars.settings, floatingBondSchedule2, vars.iborIndex)
        .withNotionals(vars.faceAmount)
        .withPaymentDayCounter(Actual360())
    ...
    floatingBondLeg2.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, floatingbondRedemption2)));
    ext::shared_ptr<Bond> floatingBond2(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Schedule cmsBondSchedule1(vars.settings->evaluationDate(), cmsBondStartDate1,
                              cmsBondMaturityDate1,
                              Period(Annual), bondCalendar,
                              Unadjusted, Unadjusted,
                              DateGeneration::Backward, false);
    Leg cmsBondLeg1 = CmsLeg(vars.settings, cmsBondSchedule1, vars.swapIndex)
        .withNotionals(vars.faceAmount)
    ...
    cmsBondLeg1.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, cmsbondRedemption1)));
    ext::shared_ptr<Bond> cmsBond1(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Schedule cmsBondSchedule2(vars.settings->evaluationDate(), cmsBondStartDate2,
                              cmsBondMaturityDate2,
                              Period(Annual), bondCalendar,
                              Unadjusted, Unadjusted,
                              DateGeneration::Backward, false);
    Leg cmsBondLeg2 = CmsLeg(vars.settings, cmsBondSchedule2, vars.swapIndex)
        .withNotionals(vars.faceAmount)
        .withPaymentDayCounter(Thirty360())
        .withFixingDays(fixingDays)
        .withGearings(0.84)
    ...
    cmsBondLeg2.push_back(ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, cmsbondRedemption2)));
    ext::shared_ptr<Bond> cmsBond2(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Leg zeroCpnBondLeg1 = Leg(1, ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, zeroCpnBondRedemption1)));
    ext::shared_ptr<Bond> zeroCpnBond1(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
    ...
    Leg zeroCpnBondLeg2 = Leg(1, ext::shared_ptr<CashFlow>(new
        SimpleCashFlow(vars.settings, 100.0, zerocpbondRedemption2)));
    ext::shared_ptr<Bond> zeroCpnBond2(new
        Bond(vars.settings, settlementDays, bondCalendar, vars.faceAmount,
...
void AssetSwapTest::testZSpreadWithGenericBond() {
    ...
    ext::shared_ptr<PricingEngine> bondEngine(new
        DiscountingBondEngine(vars.settings, vars.termStructure));
    fixedBond1->setPricingEngine(bondEngine);
...
void AssetSwapTest::testSpecializedBondVsGenericBondUsingAsw() {
    ...
    AssetSwap fixedBondAssetSwap1(vars.settings, payFixedRate,
                                  ...
                                  parAssetSwap);
    fixedBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap fixedBondASW1(vars.settings, payFixedRate,
                            fixedBond1, fixedBondMktPrice1,
                            vars.iborIndex, vars.spread,
                            Schedule(),
                            vars.iborIndex->dayCounter(),
    ...
    AssetSwap fixedSpecializedBondASW1(vars.settings, payFixedRate,
                                       fixedSpecializedBond1,
                                       fixedBondMktPrice1,
                                       vars.iborIndex, vars.spread,
                                       Schedule(),
    ...
    AssetSwap fixedBondAssetSwap2(vars.settings, payFixedRate,
                                  ...
                                  parAssetSwap);
    fixedBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap fixedSpecializedBondAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap fixedBondASW2(vars.settings, payFixedRate,
                            fixedBond2, fixedBondMktPrice2,
                            vars.iborIndex, vars.spread,
                            Schedule(),
                            vars.iborIndex->dayCounter(),
    ...
    AssetSwap fixedSpecializedBondASW2(vars.settings, payFixedRate,
                                       fixedSpecializedBond2,
                                       fixedBondMktPrice2,
                                       vars.iborIndex, vars.spread,
                                       Schedule(),
    ...
    AssetSwap floatingBondAssetSwap1(vars.settings, payFixedRate,
                                     ...
                                     parAssetSwap);
    floatingBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap floatingBondASW1(vars.settings, payFixedRate,
                               floatingBond1, floatingBondMktPrice1,
                               vars.iborIndex, vars.spread,
                               Schedule(),
                               vars.iborIndex->dayCounter(),
    ...
    AssetSwap floatingSpecializedBondASW1(vars.settings, payFixedRate,
                                          floatingSpecializedBond1,
    ...
    AssetSwap floatingBondAssetSwap2(vars.settings, payFixedRate,
                                     ...
                                     parAssetSwap);
    floatingBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap floatingSpecializedBondAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap floatingBondASW2(vars.settings, payFixedRate,
                               floatingBond2, floatingBondMktPrice2,
                               vars.iborIndex, vars.spread,
                               Schedule(),
                               vars.iborIndex->dayCounter(),
    ...
    AssetSwap floatingSpecializedBondASW2(vars.settings, payFixedRate,
                                          floatingSpecializedBond2,
    ...
    AssetSwap cmsBondAssetSwap1(vars.settings, payFixedRate,cmsBond1, cmsBondPrice1,
                                vars.iborIndex, vars.nonnullspread,
                                Schedule(),vars.iborIndex->dayCounter(),
                                parAssetSwap);
    cmsBondAssetSwap1.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondAssetSwap1(vars.settings, payFixedRate,cmsSpecializedBond1,
    ...
    AssetSwap cmsBondASW1(vars.settings, payFixedRate,
                          cmsBond1, cmsBondMktPrice1,
                          vars.iborIndex, vars.spread,
                          Schedule(),
                          vars.iborIndex->dayCounter(),
    ...
    AssetSwap cmsSpecializedBondASW1(vars.settings, payFixedRate,
                                     cmsSpecializedBond1,
                                     cmsBondMktPrice1,
                                     vars.iborIndex, vars.spread,
                                     Schedule(),
    ...
    AssetSwap cmsBondAssetSwap2(vars.settings, payFixedRate,cmsBond2, cmsBondPrice2,
                                vars.iborIndex, vars.nonnullspread,
                                Schedule(),
                                vars.iborIndex->dayCounter(),
                                parAssetSwap);
    cmsBondAssetSwap2.setPricingEngine(swapEngine);
    AssetSwap cmsSpecializedBondAssetSwap2(vars.settings, payFixedRate,cmsSpecializedBond2,
    ...
    AssetSwap cmsBondASW2(vars.settings, payFixedRate,
                          cmsBond2, cmsBondMktPrice2,
                          vars.iborIndex, vars.spread,
                          Schedule(),
                          vars.iborIndex->dayCounter(),
    ...
    AssetSwap cmsSpecializedBondASW2(vars.settings, payFixedRate,
                                     cmsSpecializedBond2,
                                     cmsBondMktPrice2,
                                     vars.iborIndex, vars.spread,
                                     Schedule(),
   ...
   //  maturity doesn't occur on a business day
    ...
    AssetSwap zeroCpnBondAssetSwap1(vars.settings, payFixedRate,zeroCpnBond1,
                                    zeroCpnBondPrice1,
                                    vars.iborIndex, vars.nonnullspread,
                                    Schedule(),
                                    vars.iborIndex->dayCounter(),
    ...
    AssetSwap zeroCpnSpecializedBondAssetSwap1(vars.settings, payFixedRate,
    ...
    AssetSwap zeroCpnBondASW1(vars.settings, payFixedRate,
    ...
    AssetSwap zeroCpnSpecializedBondASW1(vars.settings, payFixedRate,
                                         zeroCpnSpecializedBond1,
                                         zeroCpnBondMktPrice1,
                                         vars.iborIndex, vars.spread,
                                         Schedule(),
   ...
   //  maturity doesn't occur on a business day
    ...
    AssetSwap zeroCpnBondAssetSwap2(vars.settings, payFixedRate,zeroCpnBond2,
                                    zeroCpnBondPrice2,
                                    vars.iborIndex, vars.nonnullspread,
                                    Schedule(),
                                    vars.iborIndex->dayCounter(),
    ...
    AssetSwap zeroCpnSpecializedBondAssetSwap2(vars.settings, payFixedRate,
    ...
    AssetSwap zeroCpnBondASW2(vars.settings, payFixedRate,
    ...
    AssetSwap zeroCpnSpecializedBondASW2(vars.settings, payFixedRate,
                                         zeroCpnSpecializedBond2,
                                         zeroCpnBondMktPrice2,
                                         vars.iborIndex, vars.spread,
                                         Schedule(),
