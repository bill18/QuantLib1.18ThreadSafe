...
void AsianOptionTest::testAnalyticContinuousGeometricAveragePrice() {
    ...
    ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new
        BlackScholesMertonProcess(settings,
        						  Handle<Quote>(spot),
    ...
    ext::shared_ptr<PricingEngine> engine(new
            AnalyticContinuousGeometricAveragePriceAsianEngine(settings, stochProcess));
    ...
    ContinuousAveragingAsianOption option(settings, averageType, payoff, exercise);
    ...
    ext::shared_ptr<PricingEngine> engine2(new
              AnalyticDiscreteGeometricAveragePriceAsianEngine(settings, stochProcess));
    DiscreteAveragingAsianOption option2(settings, averageType,
                                         runningAccumulator, pastFixings,
                                         fixingDates,
                                         payoff,
                                         exercise);
...
void AsianOptionTest::testAnalyticContinuousGeometricAveragePriceGreeks() {
    ...
    ext::shared_ptr<BlackScholesMertonProcess> process(
         new BlackScholesMertonProcess(settings, Handle<Quote>(spot), qTS, rTS, volTS));
    ...
    for (Size i=0; i<LENGTH(types); i++) {
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
            ...
            ext::shared_ptr<PricingEngine> engine(new
                 AnalyticContinuousGeometricAveragePriceAsianEngine(settings, process));

            ContinuousAveragingAsianOption option(settings, Average::Geometric,
                                                  payoff, maturity);
            option.setPricingEngine(engine);
...
void AsianOptionTest::testAnalyticDiscreteGeometricAveragePrice() {
    ...
    ext::shared_ptr<PricingEngine> engine(
          new AnalyticDiscreteGeometricAveragePriceAsianEngine(settings, stochProcess));
    ...
    DiscreteAveragingAsianOption option(settings, averageType, runningAccumulator,
...
void AsianOptionTest::testAnalyticDiscreteGeometricAverageStrike() {
    ...
    ext::shared_ptr<PricingEngine> engine(
          new AnalyticDiscreteGeometricAverageStrikeAsianEngine(settings, stochProcess));
...
void AsianOptionTest::testMCDiscreteGeometricAveragePrice() {
    ...
    ext::shared_ptr<PricingEngine> engine =
        MakeMCDiscreteGeometricAPEngine<LowDiscrepancy>(settings, stochProcess)
        .withSamples(8191);
...
void AsianOptionTest::testMCDiscreteArithmeticAveragePrice() {
    ...
    for (Size l=0; l<LENGTH(cases4); l++) {
        ...
        ext::shared_ptr<PricingEngine> engine =
            MakeMCDiscreteArithmeticAPEngine<LowDiscrepancy>(settings, stochProcess)
            .withSamples(2047)
            .withControlVariate(cases4[l].controlVariate);

        DiscreteAveragingAsianOption option(settings, averageType, runningSum,
                                            pastFixings, fixingDates,
                                            payoff, exercise);
        option.setPricingEngine(engine);
        ...
        if(cases4[l].fixings < 100) {
            engine = ext::shared_ptr<PricingEngine>(
                    new FdBlackScholesAsianEngine(settings, stochProcess, 100, 100, 100));
            option.setPricingEngine(engine);
...
void AsianOptionTest::testMCDiscreteArithmeticAverageStrike() {
    ...
    for (Size l=0; l<LENGTH(cases5); l++) {
        ...
        ext::shared_ptr<PricingEngine> engine =
            MakeMCDiscreteArithmeticASEngine<LowDiscrepancy>(settings, stochProcess)
            .withSeed(3456789)
            .withSamples(1023);
    ...
    for (Size i=0; i<LENGTH(types); i++) {
      for (Size j=0; j<LENGTH(strikes); j++) {
        for (Size k=0; k<LENGTH(lengths); k++) {
            ...
            ext::shared_ptr<PricingEngine> engine(
               new AnalyticDiscreteGeometricAveragePriceAsianEngine(settings, process));

            DiscreteAveragingAsianOption option(settings, Average::Geometric,
                                                runningAverage, pastFixings,
...
void AsianOptionTest::testPastFixings() {
    ...
    DiscreteAveragingAsianOption option1(settings, Average::Arithmetic, runningSum,
    ...
    DiscreteAveragingAsianOption option2(settings, Average::Arithmetic, runningSum,
    ...
    DiscreteAveragingAsianOption option3(settings, Average::Geometric, runningProduct,
                                         pastFixings, fixingDates1,
                                         payoff, exercise);
    ...
    DiscreteAveragingAsianOption option4(settings, Average::Geometric, runningProduct,
                                         pastFixings, fixingDates2,
                                         payoff, exercise);
...
void AsianOptionTest::testAllFixingsInThePast() {
    ...
    ext::shared_ptr<YieldTermStructure> qTS = flatRate(settings, qRate, dc);
    ext::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.01));
    ext::shared_ptr<YieldTermStructure> rTS = flatRate(settings, rRate, dc);
    ext::shared_ptr<SimpleQuote> vol(new SimpleQuote(0.20));
    ext::shared_ptr<BlackVolTermStructure> volTS = flatVol(settings, vol, dc);
...
//    //SavedSettings backup;
    ...
    settings->evaluationDate() = fixingDates.back();
...
void AsianOptionTest::testLevyEngine() {
    ...
    for (Size l=0; l<LENGTH(cases); l++) {
        ...
        ext::shared_ptr<YieldTermStructure> qTS =
            flatRate(settings, today, cases[l].dividendYield, dc);
        ext::shared_ptr<YieldTermStructure> rTS =
            flatRate(settings, today, cases[l].riskFreeRate, dc);
        ext::shared_ptr<BlackVolTermStructure> volTS =
            flatVol(settings, today, cases[l].volatility, dc);
        ...
        ext::shared_ptr<PricingEngine> engine(
            new ContinuousArithmeticAsianLevyEngine(settings,
                     stochProcess, Handle<Quote>(average), startDate));

        ContinuousAveragingAsianOption option(settings, averageType,
                                              payoff, exercise);
        option.setPricingEngine(engine);
...
void AsianOptionTest::testVecerEngine() {
    ...
    Option::Type type = Option::Call;
    Handle<YieldTermStructure> q(flatRate(settings, today, 0.0, dayCounter));
    ...
    for (Size i=0; i<LENGTH(cases); ++i) {
        ...
        Handle<YieldTermStructure> r(flatRate(settings, today,
                                              cases[i].riskFreeRate,
                                              dayCounter));
        Handle<BlackVolTermStructure> sigma(flatVol(settings, today,
                                                    cases[i].volatility,
                                                    dayCounter));
        ext::shared_ptr<BlackScholesMertonProcess> process =
            ext::make_shared<BlackScholesMertonProcess>(settings, u, q, r, sigma);
        ...
        ContinuousAveragingAsianOption option(settings, Average::Arithmetic,
                                              payoff, exercise);
        option.setPricingEngine(
            ext::make_shared<ContinuousArithmeticAsianVecerEngine>(settings,
