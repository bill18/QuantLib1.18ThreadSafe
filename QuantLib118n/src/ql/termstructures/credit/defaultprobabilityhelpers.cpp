...
namespace QuantLib {
    ...
    CdsHelper::CdsHelper(Settings* settings, const Handle<Quote> &quote, const Period &tenor,
        ...
        : RelativeDateDefaultProbabilityHelper(settings, quote), tenor_(tenor),
    ...
    CdsHelper::CdsHelper(Settings* settings,Rate quote, const Period &tenor,
                         Integer settlementDays, const Calendar &calendar,
                         Frequency frequency,
                         BusinessDayConvention paymentConvention,
                         DateGeneration::Rule rule,
    ...
    void CdsHelper::initializeDates() {
        ...
        schedule_ =
            MakeSchedule(env_->evaluationDate()).from(startDate)
                          .to(endDate)
    ...
    SpreadCdsHelper::SpreadCdsHelper(
    		Settings* settings,
                              ...
                              const bool rebatesAccrual,
                              const CreditDefaultSwap::PricingModel model)
    : CdsHelper(settings, runningSpread, tenor, settlementDays, calendar,
    ...
    void SpreadCdsHelper::resetEngine() {
        swap_ = ext::shared_ptr<CreditDefaultSwap>(new CreditDefaultSwap(env_,
    ...
    UpfrontCdsHelper::UpfrontCdsHelper(
    		Settings* settings,
                              ...
                              const bool rebatesAccrual,
                              const CreditDefaultSwap::PricingModel model)
    : CdsHelper(settings, upfront, tenor, settlementDays, calendar,
                frequency, paymentConvention, rule, dayCounter,
                recoveryRate, discountCurve, settlesAccrual, paysAtDefaultTime,
                startDate, lastPeriodDayCounter, rebatesAccrual, model),
    ...
    UpfrontCdsHelper::UpfrontCdsHelper(
    		Settings* settings,
                              ...
                              const bool rebatesAccrual,
                              const CreditDefaultSwap::PricingModel model)
    : CdsHelper(settings, upfrontSpread, tenor, settlementDays, calendar,
                frequency, paymentConvention, rule, dayCounter,
                recoveryRate, discountCurve, settlesAccrual, paysAtDefaultTime,
                startDate, lastPeriodDayCounter, rebatesAccrual, model),
      upfrontSettlementDays_(upfrontSettlementDays),
...
//        SavedSettings backup;
        env_->includeTodaysCashFlows() = true;
        swap_->recalculate();
