...
namespace QuantLib {
    ...
    void SmileSection::update() {
        if (isFloating_) {
            referenceDate_ = env_->evaluationDate();
    ...
    SmileSection::SmileSection(Settings* settings, const Date& d,
                               const DayCounter& dc,
                               const Date& referenceDate,
                               const VolatilityType type,
                               const Rate shift)
        : exerciseDate_(d), dc_(dc), volatilityType_(type), shift_(shift), env_(settings) {
    ...
    SmileSection::SmileSection(Settings* settings, Time exerciseTime,
                               const DayCounter& dc,
                               const VolatilityType type,
                               const Rate shift)
    : isFloating_(false), referenceDate_(Date()),
      dc_(dc), exerciseTime_(exerciseTime), volatilityType_(type), shift_(shift), env_(settings) {
