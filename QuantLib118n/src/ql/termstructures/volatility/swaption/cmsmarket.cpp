	  ...
	  env_(settings)
    {
        ...
        for (Size i=0; i<nExercise_; ++i) {
            ...
            for (Size j=0; j<nSwapIndexes_; ++j) {
                ...
                spotSwaps_[i][j] = MakeCms(settings, swapLengths_[i],
                                           swapIndexes_[j],
                                           iborIndex_, 0.0,
                                           Period())
                                   .operator ext::shared_ptr<Swap>();
                fwdSwaps_[i][j]  = MakeCms(settings, swapLengths_[i]-start,
                                           swapIndexes_[j],
                                           iborIndex_, 0.0,
                                           start)
    ...
    void CmsMarket::performCalculations() const {
        for (Size j=0; j<nSwapIndexes_; ++j) {
          ...
          for (Size i=0; i<nExercise_; ++i) {
            ...
            spotFloatLegNPV_[i][j] = CashFlows::npv(env_, spotFloatLeg,
                                                    **discTS_,
                                                    false, discTS_->referenceDate());
            spotFloatLegBPS_[i][j] = CashFlows::bps(env_, spotFloatLeg,
                                                    **discTS_,
                                                    false, discTS_->referenceDate());
