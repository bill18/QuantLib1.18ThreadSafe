...
Gaussian1dSmileSection::Gaussian1dSmileSection(
    ...
    : SmileSection(settings, fixingDate, dc, model->termStructure()->referenceDate()),
      fixingDate_(fixingDate), swapIndex_(swapIndex),
      iborIndex_(ext::shared_ptr<IborIndex>()), model_(model),
      engine_(swaptionEngine), env_(settings) {
...
Gaussian1dSmileSection::Gaussian1dSmileSection(
    ...
    : SmileSection(settings, fixingDate, dc, model->termStructure()->referenceDate()),
      ...
      iborIndex_(iborIndex), model_(model), engine_(capEngine), env_(settings) {

    atm_ = model_->forwardRate(fixingDate_, Null<Date>(), 0.0, iborIndex_);
    CapFloor c = MakeCapFloor(env_, CapFloor::Cap, iborIndex_->tenor(), iborIndex_,
                              Null<Real>(),
...
Real Gaussian1dSmileSection::optionPrice(Rate strike, Option::Type type,
    ...
    if (swapIndex_ != NULL) {
        Swaption s = MakeSwaption(env_, swapIndex_, fixingDate_, strike)
        ...
        CapFloor c =
            MakeCapFloor(env_, type == Option::Call ? CapFloor::Cap : CapFloor::Floor,
