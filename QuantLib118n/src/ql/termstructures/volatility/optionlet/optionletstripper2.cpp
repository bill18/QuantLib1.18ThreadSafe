...
namespace QuantLib {
    ...
    : OptionletStripper(settings, optionletStripper1->termVolSurface(),
    ...
    void OptionletStripper2::performCalculations() const {
        ...
        for (Size j=0; j<nOptionExpiries_; ++j) {
            ...
            ext::shared_ptr<BlackCapFloorEngine> engine(new
                    BlackCapFloorEngine(env_, iborIndex_->forwardingTermStructure(),
                                        atmOptionVol, dc_));
            caps_[j] = MakeCapFloor(env_, CapFloor::Cap,
        ...
        StrippedOptionletAdapter adapter(env_, stripper1_);
    ...
    std::vector<Volatility> OptionletStripper2::spreadsVolImplied() const {
        ...
        for (Size j=0; j<nOptionExpiries_; ++j) {
            ObjectiveFunction f(env_, stripper1_, caps_[j], atmCapFloorPrices_[j]);
...
//==========================================================================//
      ...
      targetValue_(targetValue)
    {
        ext::shared_ptr<OptionletVolatilityStructure> adapter(new
            StrippedOptionletAdapter(settings, optionletStripper1));
        adapter->enableExtrapolation();
        ...
        ext::shared_ptr<OptionletVolatilityStructure> spreadedAdapter(new
            SpreadedOptionletVolatility(settings, Handle<OptionletVolatilityStructure>(
                adapter), Handle<Quote>(spreadQuote_)));

        ext::shared_ptr<BlackCapFloorEngine> engine(new
            BlackCapFloorEngine(settings,
