...
OptionletStripper1::OptionletStripper1(
    ...
    : OptionletStripper(settings, termVolSurface, index, discount, type, displacement),
      floatingSwitchStrike_(switchStrike == Null< Rate >() ? true : false),
      switchStrike_(switchStrike),
      accuracy_(accuracy), maxIter_(maxIter), dontThrow_(dontThrow), env_(settings) {
    ...
    void OptionletStripper1::performCalculations() const {
        ...
        for (Size i=0; i<nOptionletTenors_; ++i) {
            CapFloor temp = MakeCapFloor(env_, CapFloor::Cap,
                                         capFloorLengths_[i],
                                         iborIndex_,
                                         0.04, // dummy strike
                                         0*Days)
        ...
        for (Size j=0; j<nStrikes_; ++j) {
            ...
            for (Size i=0; i<nOptionletTenors_; ++i) {
                ...
                ext::shared_ptr<CapFloor> capFloor =
                    MakeCapFloor(env_, capFloorType, capFloorLengths_[i],
