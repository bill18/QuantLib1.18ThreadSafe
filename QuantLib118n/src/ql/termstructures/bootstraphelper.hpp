...
namespace QuantLib {
    ...
    */
    ...
    class RelativeDateBootstrapHelper : public BootstrapHelper<TS> {
      public:
        explicit RelativeDateBootstrapHelper(Settings* settings, const Handle<Quote>& quote);
        explicit RelativeDateBootstrapHelper(Settings* settings, Real quote);
        //! \name Observer interface
        //@{
        void update() {
            if (evaluationDate_ != env_->evaluationDate()) {
                evaluationDate_ = env_->evaluationDate();
                initializeDates();
            }
    ...
    : BootstrapHelper<TS>(quote), env_(settings) {
        this->registerWith(env_->evaluationDate());
        evaluationDate_ = env_->evaluationDate();
    }

    template <class TS>
    RelativeDateBootstrapHelper<TS>::RelativeDateBootstrapHelper(Settings* settings, Real quote)
    : BootstrapHelper<TS>(quote), env_(settings) {
        this->registerWith(env_->evaluationDate());
        evaluationDate_ = env_->evaluationDate();
    }
