...
namespace QuantLib {
    ...
    : BootstrapHelper<ZeroInflationTermStructure>(quote),
      ...
      zii_(zii), nominalTermStructure_(nominalTermStructure), env_(settings) {
    ...
    void ZeroCouponInflationSwapHelper::setTermStructure(
        ...
        zciis_->setPricingEngine(ext::shared_ptr<PricingEngine>(
                new DiscountingSwapEngine(env_, nominalTS)));
    }
    ...
    : BootstrapHelper<YoYInflationTermStructure>(quote),
      ...
      dayCounter_(dayCounter), yii_(yii), nominalTermStructure_(nominalTermStructure), env_(settings) {
    ...
    : BootstrapHelper<YoYInflationTermStructure>(quote),
      swapObsLag_(swapObsLag), maturity_(maturity),
      calendar_(calendar), paymentConvention_(paymentConvention),
      dayCounter_(dayCounter), yii_(yii), env_(settings) {
    ...
    void YearOnYearInflationSwapHelper::setTermStructure(
        ...
        Date from = env_->evaluationDate();
        Date to = maturity_;
        Schedule fixedSchedule = MakeSchedule(env_->evaluationDate()).from(from).to(to)
                                    .withTenor(1*Years)
        ...
        Real nominal = 1000000.0;   // has to be something but doesn't matter what
        yyiis_.reset(new YearOnYearInflationSwap(env_, YearOnYearInflationSwap::Payer,
                                                    nominal,
                                                    fixedSchedule,
                                                    fixedRate,
                                                    dayCounter_,
        ...
        yyiis_->setPricingEngine(ext::shared_ptr<PricingEngine>(
                    new DiscountingSwapEngine(env_, !nominalTermStructure_.empty() ?
