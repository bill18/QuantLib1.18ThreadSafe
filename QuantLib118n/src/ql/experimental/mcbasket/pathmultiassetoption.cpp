...
namespace QuantLib {
    ...
    PathMultiAssetOption::PathMultiAssetOption(
    		Settings* settings,
                            const ext::shared_ptr<PricingEngine>& engine):env_(settings) {
        if (engine)
            setPricingEngine(engine);
    }

    bool PathMultiAssetOption::isExpired() const {
        return detail::simple_event(fixingDates().back()).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
