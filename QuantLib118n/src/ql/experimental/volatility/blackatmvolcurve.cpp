...
namespace QuantLib {
    ...
    BlackAtmVolCurve::BlackAtmVolCurve(Settings* settings,BusinessDayConvention bdc,
                                       const DayCounter& dc)
    : VolatilityTermStructure(settings, bdc, dc) {}

    BlackAtmVolCurve::BlackAtmVolCurve(Settings* settings, const Date& refDate,
                                       const Calendar& cal,
                                       BusinessDayConvention bdc,
                                       const DayCounter& dc)
    : VolatilityTermStructure(settings, refDate, cal, bdc, dc) {}

    BlackAtmVolCurve::BlackAtmVolCurve(Settings* settings, Natural settlDays,
                                       const Calendar& cal,
                                       BusinessDayConvention bdc,
                                       const DayCounter& dc)
