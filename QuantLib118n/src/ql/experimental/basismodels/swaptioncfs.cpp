...
namespace QuantLib {
    ...
    IborLegCashFlows::IborLegCashFlows(Settings* settings, const Leg& iborLeg,
                                       const Handle<YieldTermStructure>& discountCurve,
                                       bool contTenorSpread)
    : refDate_(discountCurve->referenceDate()), env_(settings) {
        ...
        if (refDate_ <= (ext::dynamic_pointer_cast<Coupon>(iborLeg[floatIdx]))
            ...
            floatLeg_.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(settings,
            ...
            for (Size k = floatIdx; k < iborLeg.size(); ++k) {
                ...
                floatLeg_.push_back(ext::shared_ptr<CashFlow>(new FixedRateCoupon(env_,
            ...
            floatLeg_.push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(env_,
    ...
    SwapCashFlows::SwapCashFlows(Settings* settings, const ext::shared_ptr<VanillaSwap>& swap,
                                 const Handle<YieldTermStructure>& discountCurve,
                                 bool contTenorSpread)
    : IborLegCashFlows(settings, swap->floatingLeg(), discountCurve, contTenorSpread) {
    ...
    SwaptionCashFlows::SwaptionCashFlows(Settings* settings, const ext::shared_ptr<Swaption>& swaption,
                                         const Handle<YieldTermStructure>& discountCurve,
                                         bool contTenorSpread)
    : SwapCashFlows(settings, swaption->underlyingSwap(), discountCurve, contTenorSpread),
      swaption_(swaption) {
        // assemble raw cash flow data...
        Actual365Fixed dc;
        // ... exercise times
