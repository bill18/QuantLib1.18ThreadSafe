...
namespace QuantLib {
    ...
    class IborLegCashFlows {
      ...
      public:
        ...
        IborLegCashFlows(Settings* settings, const Leg& iborLeg,
                         const Handle<YieldTermStructure>& discountCurve,
                         bool contTenorSpread = true);
        IborLegCashFlows(Settings* settings):env_(settings) {} // allow default constructor which does nothing
      protected:
        Settings* env_;
    };
    ...
    class SwapCashFlows : public IborLegCashFlows {
      ...
      public:
        SwapCashFlows(Settings* settings, const ext::shared_ptr<VanillaSwap>& swap,
                      const Handle<YieldTermStructure>& discountCurve,
                      bool contTenorSpread = true);
        SwapCashFlows(Settings* settings):IborLegCashFlows(settings) {} // allow default constructor which does nothing
                           // inspectors
    ...
    class SwaptionCashFlows : public SwapCashFlows {
      ...
      public:
        SwaptionCashFlows(Settings* settings, const ext::shared_ptr<Swaption>& swaption,
                          const Handle<YieldTermStructure>& discountCurve,
                          bool contTenorSpread = true);
        SwaptionCashFlows(Settings* settings):SwapCashFlows(settings) {} // allow default constructor which does nothing
        // inspectors
