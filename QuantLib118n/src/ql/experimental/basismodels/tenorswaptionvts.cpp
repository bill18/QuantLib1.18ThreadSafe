...
namespace QuantLib {
    ...
    : SmileSection(settings, optionTime, volTS.baseVTS_->dayCounter(), Normal, 0.0) {
        ...
        Schedule baseFixedSchedule(settings->evaluationDate(), effectiveDate, maturityDate, volTS.baseFixedFreq_,
                                   volTS.baseIndex_->fixingCalendar(), ModifiedFollowing,
                                   Unadjusted, DateGeneration::Backward, false);
        Schedule finlFixedSchedule(settings->evaluationDate(), effectiveDate, maturityDate, volTS.targFixedFreq_,
                                   volTS.targIndex_->fixingCalendar(), ModifiedFollowing,
                                   Unadjusted, DateGeneration::Backward, false);
        Schedule baseFloatSchedule(settings->evaluationDate(), effectiveDate, maturityDate, volTS.baseIndex_->tenor(),
                                   volTS.baseIndex_->fixingCalendar(), ModifiedFollowing,
                                   Unadjusted, DateGeneration::Backward, false);
        Schedule targFloatSchedule(settings->evaluationDate(), effectiveDate, maturityDate, volTS.targIndex_->tenor(),
                                   volTS.baseIndex_->fixingCalendar(), ModifiedFollowing,
                                   Unadjusted, DateGeneration::Backward, false);
        // and swaps
        ext::shared_ptr<VanillaSwap> baseSwap(new VanillaSwap(settings,
        ...
        ext::shared_ptr<VanillaSwap> targSwap(new VanillaSwap(settings,
        ...
        ext::shared_ptr<VanillaSwap> finlSwap(new VanillaSwap(settings,
            VanillaSwap::Payer, 1.0, finlFixedSchedule, 1.0, volTS.targFixedDC_, targFloatSchedule,
            volTS.targIndex_, 0.0, volTS.targIndex_->dayCounter()));
        // adding engines
        baseSwap->setPricingEngine(
            ext::shared_ptr<PricingEngine>(new DiscountingSwapEngine(settings, volTS.discountCurve_)));
        ...
        SwaptionCashFlows cfs(settings,
            ext::shared_ptr<Swaption>(new Swaption(settings,
                baseSwap, ext::shared_ptr<Exercise>(new EuropeanExercise(exerciseDate)))),
            volTS.discountCurve_);
        SwaptionCashFlows cf2(settings,
            ext::shared_ptr<Swaption>(new Swaption(settings,
