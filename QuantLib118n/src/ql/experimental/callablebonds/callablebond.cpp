...
namespace QuantLib {
    ...
    CallableBond::CallableBond(Settings* settings, Natural settlementDays,
                               const Schedule& schedule,
                               const DayCounter& paymentDayCounter,
                               const Date& issueDate,
                               const CallabilitySchedule& putCallSchedule)
    : Bond(settings, settlementDays, schedule.calendar(), issueDate),
   ...
   Real CallableBond::NPVSpreadHelper::operator()(Real x) const
   {
    ...
    CallableFixedRateBond::CallableFixedRateBond(
    		Settings* settings,
                              ...
                              const Date& issueDate,
                              const CallabilitySchedule& putCallSchedule)
    : CallableBond(settings, settlementDays, schedule, accrualDayCounter,
        ...
        if (!isZeroCouponBond) {
            cashflows_ =
                FixedRateLeg(settings, schedule)
        ...
        blackEngine_ = ext::shared_ptr<PricingEngine>(
                   new BlackCallableFixedRateBondEngine(settings, blackVolQuote_,
                                                        blackDiscountCurve_));
    }
    ...
    CallableZeroCouponBond::CallableZeroCouponBond(
    		Settings* settings,
                              ...
                              const Date& issueDate,
                              const CallabilitySchedule& putCallSchedule)
    : CallableFixedRateBond(settings, settlementDays,faceAmount,
                            Schedule(settings->evaluationDate(), issueDate, maturityDate,
                                     Period(Once),
                                     calendar,
                                     paymentConvention,
                                     paymentConvention,
