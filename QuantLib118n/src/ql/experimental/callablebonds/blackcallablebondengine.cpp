...
namespace QuantLib {
    ...
    : CallableFixedRateBond::engine(settings), volatility_(ext::shared_ptr<CallableBondVolatilityStructure>(
                      new CallableBondConstantVolatility(settings, 0, NullCalendar(),
                                                         fwdYieldVol,
                                                         Actual365Fixed()))),
    ...
    BlackCallableFixedRateBondEngine::BlackCallableFixedRateBondEngine(
    		Settings* settings,
             ...
             const Handle<YieldTermStructure>& discountCurve)
    : CallableFixedRateBond::engine(settings), volatility_(yieldVolStructure), discountCurve_(discountCurve) {
        registerWith(volatility_);
        registerWith(discountCurve_);
    }
    ...
    Volatility BlackCallableFixedRateBondEngine::forwardPriceVolatility()
        ...
        Real fwdNpv = CashFlows::npv(env_, fixedLeg,
                                     **discountCurve_,
                                     false, exerciseDate);
        ...
        Rate fwdYtm = CashFlows::yield(env_, fixedLeg,
                                       fwdNpv,
                                       dayCounter,
                                       Compounded,
                                       frequency,
        ...
        Time fwdDur = CashFlows::duration(env_, fixedLeg,
                                          fwdRate,
                                          Duration::Modified,
                                          false, exerciseDate);
    ...
    void BlackCallableFixedRateBondEngine::calculate() const {
        ...
        Real value = CashFlows::npv(env_, fixedLeg,
                                    **discountCurve_,
                                    false, settle);

        Real npv = CashFlows::npv(env_, fixedLeg,
                                  **discountCurve_,
