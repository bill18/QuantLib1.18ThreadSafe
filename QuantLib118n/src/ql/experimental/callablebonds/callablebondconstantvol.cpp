...
namespace QuantLib {
    ...
    : CallableBondVolatilityStructure(settings, referenceDate),
    ...
    CallableBondConstantVolatility::CallableBondConstantVolatility(
    		Settings* settings,
                                              ...
                                              const DayCounter& dayCounter)
    : CallableBondVolatilityStructure(settings, referenceDate), volatility_(volatility),
      dayCounter_(dayCounter), maxBondTenor_(100*Years),
	  env_(settings) {
    ...
    CallableBondConstantVolatility::CallableBondConstantVolatility(
    		Settings* settings,
                                                 ...
                                                 Volatility volatility,
                                                 const DayCounter& dayCounter)
    : CallableBondVolatilityStructure(settings, settlementDays, calendar),
      volatility_(ext::shared_ptr<Quote>(new SimpleQuote(volatility))),
      dayCounter_(dayCounter), maxBondTenor_(100*Years),
	  env_(settings) {
    ...
    CallableBondConstantVolatility::smileSectionImpl(Time optionTime,
        ...
        return ext::shared_ptr<SmileSection>(
                                    new FlatSmileSection(env_, optionTime,
                                                         atmVol,
                                                         dayCounter_));
    }
