...
namespace QuantLib {
    ...
    */
    ...
    class InterpolatedYoYOptionletStripper : public YoYOptionletStripper {
      public:
        InterpolatedYoYOptionletStripper(Settings* settings):env_(settings) {}
    ...
    : slope_(slope), K_(K), frequency_(anIndex->frequency()),
      ...
      priceToMatch_(priceToMatch), surf_(surf), p_(p), env_(settings) {
        ...
        capfloor_ =
            MakeYoYInflationCapFloor(settings, type, anIndex,
    ...
    ObjectiveFunction::operator()(Volatility guess) const {
        ...
        vCurve(
            new InterpolatedYoYOptionletVolatilityCurve<Linear>(env_,
    ...
    initialize(const ext::shared_ptr<YoYCapFloorTermPriceSurface> &s,
        ...
        ext::shared_ptr<YoYInflationIndex> anIndex(
                                           new YYGenericCPI(env_, frequency_, false,
                                                            false, lag_,
                                                            Currency(), hYoY));
        ...
        for (Size i=0; i<YoYCapFloorTermPriceSurface_->strikes().size(); i++) {
            ...
            try{
                found = solver.solve(
                      ObjectiveFunction(env_, useType, slope, K, lag_, fixingDays_,
            ...
            for (Size j = 0; j < YoYCapFloorTermPriceSurface_->maturities().size(); j++){
                ...
                helpers.push_back(ext::shared_ptr<YoYOptionletHelper>(
                          new YoYOptionletHelper(env_, quote1, notional, useType,
                                                 lag_,
                                                 dc, cal,
                                                 fixingDays_,
                                                 anIndex, K, nT, p_)));
                ...
                ext::shared_ptr<ConstantYoYOptionletVolatility> yoyVolBLACK(
                          new ConstantYoYOptionletVolatility(env_, found, settlementDays,
                                                             cal, bdc, dc,
                                                             lag_, frequency_,
                                                             false,
                                                             // -100% to +300%
            ...
            ext::shared_ptr<
                ...
                new PiecewiseYoYOptionletVolatilityCurve<Interpolator1D>(env_,
