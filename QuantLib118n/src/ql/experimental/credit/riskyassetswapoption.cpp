...
namespace QuantLib {
    ...
    RiskyAssetSwapOption::RiskyAssetSwapOption(Settings* settings,
    ...
    : asw_(asw), expiry_(expiry),
      ...
      spreadVolatility_(spreadVolatility), env_(settings) {

    }

    bool RiskyAssetSwapOption::isExpired() const {
        return detail::simple_event(expiry_).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
