...
namespace QuantLib {
	  ...
	  env_(settings)
    {
        ...
        premiumLeg_ = FixedRateLeg(settings, premiumSchedule)
...
// SOME OF THESE ARE INLINES---------------------------------
    ...
    bool NthToDefault::isExpired() const {
        return detail::simple_event(premiumLeg_.back()->date()).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
