...
namespace QuantLib {
    ...
    bool RiskyBond::isExpired() const {
        return detail::simple_event(maturityDate()).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
    ...
    void RiskyBond::performCalculations() const {
        NPV_ = 0;
        Date today = env_->evaluationDate();
        ...
        for (Size i = 0; i < cf.size(); i++) {
            ...
            if (d2 > npvDate) {
                ...
                ext::shared_ptr<CashFlow>
                    flow1(new SimpleCashFlow(env_, coupon, d2));
                expected.push_back(flow1);

                ext::shared_ptr<CashFlow>
                    flow2(new SimpleCashFlow(env_, recovery, defaultDate));
    ...
    : RiskyBond(settings, name, ccy, recoveryRate, defaultTS, yieldTS,
        ...
        for (Size i = 1; i < dates.size(); i++) {
            ...
            ext::shared_ptr<CashFlow> interest (new
                   FixedRateCoupon(settings, dates[i], previousNotional,
                                   rate_, dayCounter_, dates[i-1], dates[i]));
            ext::shared_ptr<CashFlow> amortization(new
                 AmortizingPayment(settings, previousNotional - currentNotional, dates[i]));
        ...
        ext::shared_ptr<CashFlow> redemption(new
                 Redemption(env_, previousNotional, schedule_.dates().back()));
    ...
    : RiskyBond(settings, name, ccy, recoveryRate, defaultTS, yieldTS,
        ...
        for (Size i = 1; i < dates.size(); i++) {
            Real currentNotional = (i < notionals_.size() ?
                             ...
                             notionals_.back());
            ext::shared_ptr<CashFlow> interest (new
                   IborCoupon(settings, dates[i], previousNotional, dates[i-1], dates[i],
        ...
        ext::shared_ptr<CashFlow> redemption(new
                 Redemption(settings, previousNotional, schedule_.dates().back()));
        leg_.push_back(redemption);
        redemptionLeg_.push_back(redemption);

        ext::shared_ptr<IborCouponPricer>
            fictitiousPricer(new
                BlackIborCouponPricer(settings, Handle<OptionletVolatilityStructure>()));
