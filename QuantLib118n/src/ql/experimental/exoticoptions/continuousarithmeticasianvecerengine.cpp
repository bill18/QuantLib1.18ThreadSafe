...
namespace QuantLib {
    ...
    ContinuousArithmeticAsianVecerEngine::ContinuousArithmeticAsianVecerEngine(
         ...
         Real z_min,
         Real z_max )
        : ContinuousAveragingAsianOption::engine(settings), process_(process), currentAverage_(currentAverage),
    ...
    void ContinuousArithmeticAsianVecerEngine::calculate() const {
        ...
        if ((T2 - T1) < 0.001) {
            ...
            VanillaOption europeanOption(env_, payoff, arguments_.exercise);
            europeanOption.setPricingEngine(
                        ext::make_shared<AnalyticEuropeanEngine>(env_, process_));
