...
namespace QuantLib {
    ...
    : OneAssetOption(settings, payoff1, exercise1),
    ...
    bool WriterExtensibleOption::isExpired() const {
        return detail::simple_event(exercise2_->lastDate()).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
