...
namespace QuantLib {
    ...
    AnalyticAmericanMargrabeEngine::AnalyticAmericanMargrabeEngine(
            ...
            Real correlation)
    : MargrabeOption::engine(settings), process1_(process1), process2_(process2), rho_(correlation) {
    ...
    void AnalyticAmericanMargrabeEngine::calculate() const {
        ...
        ext::shared_ptr<YieldTermStructure> qTS(
                                            new FlatForward(env_, today, q1, rfdc));

        ext::shared_ptr<YieldTermStructure> rTS(
                                            new FlatForward(env_, today, q2, rfdc));
        ...
        ext::shared_ptr<BlackVolTermStructure> volTS(
               new BlackConstantVol(env_, today, NullCalendar(), volatility, rfdc));

        ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new
            BlackScholesMertonProcess(env_, Handle<Quote>(spot),
                                      Handle<YieldTermStructure>(qTS),
                                      Handle<YieldTermStructure>(rTS),
                                      Handle<BlackVolTermStructure>(volTS)));

        ext::shared_ptr<PricingEngine> engine(
                     new BjerksundStenslandApproximationEngine(env_, stochProcess));

        VanillaOption option(env_, payoff, exercise);
