    ...
    class BinomialConvertibleEngine : public ConvertibleBond::option::engine {
      public:
        BinomialConvertibleEngine(
             ...
             Size timeSteps)
        : ConvertibleBond::option::engine(settings), process_(process), timeSteps_(timeSteps),
		  env_(settings) {
    ...
    void BinomialConvertibleEngine<T>::calculate() const {
        ...
        Handle<YieldTermStructure> flatRiskFree(
            ext::shared_ptr<YieldTermStructure>(
                new FlatForward(env_, referenceDate, riskFreeRate, rfdc)));
        Handle<YieldTermStructure> flatDividends(
            ext::shared_ptr<YieldTermStructure>(
                new FlatForward(env_, referenceDate, q, divdc)));
        Handle<BlackVolTermStructure> flatVol(
            ext::shared_ptr<BlackVolTermStructure>(
                new BlackConstantVol(env_, referenceDate, volcal, v, voldc)));
        ...
        ext::shared_ptr<GeneralizedBlackScholesProcess> bs(
                 new GeneralizedBlackScholesProcess(env_, underlying, flatDividends,
