...
namespace QuantLib {
    ...
    : EnergySwap(settings, calendar, payCurrency, receiveCurrency,
    ...
    void EnergyBasisSwap::performCalculations() const {
        ...
        try {
            ...
            Date evaluationDate = env_->evaluationDate();
            ...
            Real quantityUomConversionFactor =
                calculateUomConversionFactor(env_->evaluationDate(),
                               pricingPeriods_[0]->quantity().commodityType(),
                               baseUnitOfMeasure,
                               pricingPeriods_[0]->quantity().unitOfMeasure());
            Real payIndexUomConversionFactor =
                calculateUomConversionFactor(env_->evaluationDate(), payIndex_->commodityType(),
                                             payIndex_->unitOfMeasure(),
                                             baseUnitOfMeasure);
            Real receiveIndexUomConversionFactor =
                calculateUomConversionFactor(env_->evaluationDate(), receiveIndex_->commodityType(),
                                             receiveIndex_->unitOfMeasure(),
                                             baseUnitOfMeasure);

            Real payIndexFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), payIndex_->currency(),
                                            baseCurrency, evaluationDate);
            Real receiveIndexFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), receiveIndex_->currency(),
                                            baseCurrency, evaluationDate);
            Real payLegFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), baseCurrency, payCurrency_,
                                            evaluationDate);
            Real receiveLegFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), baseCurrency, receiveCurrency_,
                                            evaluationDate);
            ...
            Real basisFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), baseCurrency,
            ...
            for (PricingPeriods::const_iterator pi = pricingPeriods_.begin();
                ...
                paymentCashFlows_[pricingPeriod->paymentDate()] =
                    ext::make_shared<CommodityCashFlow> (
                    		env_,
                           pricingPeriod->paymentDate(),
                                                 Money(env_->evaluationDate(), baseCurrency,
                                                       uDelta * discountFactor),
                                                 Money(env_->evaluationDate(), baseCurrency, uDelta),
                                                 Money(env_->evaluationDate(), pmtCurrency,
