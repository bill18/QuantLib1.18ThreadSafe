...
namespace QuantLib {
    ...
    : EnergySwap(settings, calendar, payCurrency, receiveCurrency, pricingPeriods,
    ...
    bool EnergyVanillaSwap::isExpired() const {
        return detail::simple_event(pricingPeriods_.back()->endDate())
               .hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
    ...
    void EnergyVanillaSwap::performCalculations() const {
        ...
        try {
            ...
            Real indexUomConversionFactor =
                calculateUomConversionFactor(env_->evaluationDate(), index_->commodityType(),
                                             index_->unitOfMeasure(),
                                             baseUnitOfMeasure);

            Real fixedPriceFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), fixedPrice_.currency(),
                                            baseCurrency, evaluationDate);
            Real indexPriceFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(), index_->currency(),
                                            baseCurrency, evaluationDate);
            Real payLegFxConversionFactor =
                calculateFxConversionFactor(env_->evaluationDate(),
                            baseCurrency,
