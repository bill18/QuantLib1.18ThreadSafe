...
namespace QuantLib {
    ...
    class CommodityPricingHelper {
      public:
        ...
        static Real calculateFxConversionFactor(const Date& evalDate, const Currency& fromCurrency,
        ...
        static Real calculateUnitCost(const Date& evalDate, const CommodityType& commodityType,
    ...
    inline Real CommodityPricingHelper::calculateUnitCost(
        ...
        if (unitCost.amount().value() != 0) {
            Real unitCostUomConversionFactor =
                calculateUomConversionFactor(evalDate, commodityType,
                                             unitCost.unitOfMeasure(),
                                             baseUnitOfMeasure);
            Real unitCostFxConversionFactor =
                calculateFxConversionFactor(evalDate, unitCost.amount().currency(),
