...
namespace QuantLib {
    ...
    : EnergyCommodity(settings, commodityType, secondaryCosts),
    ...
    void EnergyFuture::performCalculations() const {
        ...
        Real tradePriceUomConversionFactor =
            calculateUomConversionFactor(env_->evaluationDate(), quantity_.commodityType(),
                                         tradePrice_.unitOfMeasure(),
                                         baseUnitOfMeasure);

        Real tradePriceFxConversionFactor =
            calculateFxConversionFactor(env_->evaluationDate(), tradePrice_.amount().currency(),
                                        baseCurrency, evaluationDate);
        Real indexPriceFxConversionFactor =
            calculateFxConversionFactor(env_->evaluationDate(),index_->currency(), baseCurrency,
                                        evaluationDate);
