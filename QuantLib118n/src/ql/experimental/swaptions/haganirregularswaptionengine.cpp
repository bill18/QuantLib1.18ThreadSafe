...
namespace QuantLib {
    ...
    HaganIrregularSwaptionEngine::Basket::Basket(Settings* settings,ext::shared_ptr<IrregularSwap> swap,
        ...
        env_(settings) {
            ...
            engine_ = ext::shared_ptr<PricingEngine>(new DiscountingSwapEngine(settings, termStructure_));
            ...
            for(Size i = 0; i < fixedLeg.size(); ++i)  
            {
                ...
                annuities_.push_back(10000*CashFlows::bps(settings, fixedCFS,**termStructure_,true));
                ...
                for(Size j = 0; j < floatLeg.size(); ++j){
                    ...
                    if( coupon->date() <= expiries_[i] ){
                        ext::shared_ptr<IborCoupon> newCpn = ext::shared_ptr<IborCoupon> (
                            new  IborCoupon(settings, coupon->date(),
                            1.0,
                            coupon->accrualStartDate(),
                            coupon->accrualEndDate(),
                            coupon->fixingDays(),
                        ...
                        if (!newCpn->isInArrears())
                            newCpn->setPricer(
                                         ext::shared_ptr<FloatingRateCouponPricer>(
                                                  new BlackIborCouponPricer(settings)));

                        floatCFS.push_back(newCpn);
                    }
                }

                Real floatLegNPV = CashFlows::npv(settings, floatCFS,**termStructure_,true);
    ...
    ext::shared_ptr<VanillaSwap> HaganIrregularSwaptionEngine::Basket::component(Size i) const {
        ...
        ext::shared_ptr<VanillaSwap> memberSwap_ = MakeVanillaSwap(env_, dummySwapLength,iborIndex)
        ...
        Real stdAnnuity = 10000*CashFlows::bps(env_, memberSwap_->fixedLeg(),**termStructure_,true);

        //compute annuity transformed rate
        Rate transformedRate = (fairRates_[i]+lambda_)*annuities_[i]/stdAnnuity;

        memberSwap_ = MakeVanillaSwap(env_, dummySwapLength,iborIndex,transformedRate)
                                                     .withType(VanillaSwap::Type(swap_->type()))
                                                     .withEffectiveDate(swap_->startDate())
                                                     .withTerminationDate(expiries_[i])
                                                     .withRule(DateGeneration::Backward)
    ...
    HaganIrregularSwaptionEngine::HaganIrregularSwaptionEngine(Settings* settings, const Handle<SwaptionVolatilityStructure>& volatilityStructure,
        const Handle<YieldTermStructure>& termStructure)
        : GenericEngine<IrregularSwaption::arguments,IrregularSwaption::results>(settings),
    ...
    void HaganIrregularSwaptionEngine::calculate() const {
        ...
        Real fxdLgBPS = CashFlows::bps(env_, fixedLeg,**termStructure_,true);

        Leg  floatLeg = swap_->floatingLeg();
        Real fltLgNPV = CashFlows::npv(env_, floatLeg,**termStructure_,true);
        Real fltLgBPS = CashFlows::bps(env_, floatLeg,**termStructure_,true);
        ...
        for(Size j = 0; j < floatLeg.size(); ++j){
            ...
            ext::shared_ptr<IborCoupon> newCpn = ext::shared_ptr<IborCoupon> (
                new  IborCoupon(env_, coupon->date(),
                coupon->nominal(),
                coupon->accrualStartDate(),
                coupon->accrualEndDate(),
                coupon->fixingDays(),
            ...
            if (!newCpn->isInArrears())
                newCpn->setPricer(
                             ext::shared_ptr<FloatingRateCouponPricer>(
                                      new BlackIborCouponPricer(env_)));

            floatCFS.push_back(newCpn);
        }


        Real sprdLgNPV = fltLgNPV - CashFlows::npv(env_, floatCFS,**termStructure_,true);
        ...
        swap_  =  ext::make_shared<IrregularSwap>(env_, arguments_.swap->type(),fixedCFS,floatCFS);
        ...
        Basket basket(env_, swap_,termStructure_,volatilityStructure_);
    ...
    Real  HaganIrregularSwaptionEngine::HKPrice(Basket& basket,ext::shared_ptr<Exercise>& exercise) const {
        ...
        ext::shared_ptr<PricingEngine> blackSwaptionEngine = 
             ext::shared_ptr<PricingEngine>(new BlackSwaptionEngine(env_, termStructure_,volatilityStructure_));
        ...
        for(Size i=0; i<weights.size(); ++i)
        {
            ...
            Swaption swaption = Swaption(env_, pvSwap_,exercise);
