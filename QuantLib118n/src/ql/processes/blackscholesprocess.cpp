...
namespace QuantLib {
    ...
    GeneralizedBlackScholesProcess::localVolatility() const {
        ...
        if (!updated_) {
            ...
            localVolatility_.linkTo(
                ext::make_shared<LocalVolSurface>(env_, blackVolatility_, riskFreeRate_,
    ...
    : GeneralizedBlackScholesProcess(settings,
             ...
             Handle<YieldTermStructure>(ext::shared_ptr<YieldTermStructure>(
                  new FlatForward(settings, 0, NullCalendar(), 0.0, Actual365Fixed()))),
             riskFreeTS,
             blackVolTS,
    ...
    BlackScholesMertonProcess::BlackScholesMertonProcess(
    		Settings* settings,
                              ...
                              const ext::shared_ptr<discretization>& d,
                              bool forceDiscretization)
    : GeneralizedBlackScholesProcess(settings, x0,dividendTS,riskFreeTS,blackVolTS,d,
                                     forceDiscretization) {}


    BlackProcess::BlackProcess(Settings* settings, const Handle<Quote>& x0,
                               const Handle<YieldTermStructure>& riskFreeTS,
                               const Handle<BlackVolTermStructure>& blackVolTS,
                               const ext::shared_ptr<discretization>& d,
                               bool forceDiscretization)
    : GeneralizedBlackScholesProcess(settings, x0,riskFreeTS,riskFreeTS,blackVolTS,d,
                                     forceDiscretization) {}
    ...
    GarmanKohlagenProcess::GarmanKohlagenProcess(
    		Settings* settings,
                          ...
                          const ext::shared_ptr<discretization>& d,
                          bool forceDiscretization)
    : GeneralizedBlackScholesProcess(settings, x0,foreignRiskFreeTS,domesticRiskFreeTS,
