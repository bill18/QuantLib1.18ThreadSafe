...
template <typename Model> class XABRCoeffHolder {
  public:
    XABRCoeffHolder(Settings* settings, const Time t, const Real &forward,
        ...
        updateModelInstance(settings);
    }
    virtual ~XABRCoeffHolder() {}

    void updateModelInstance(Settings* settings) {
        modelInstance_ = Model().instance(settings, t_, forward_, params_, addParams_);
    }
...
class XABRInterpolationImpl : public Interpolation::templateImpl<I1, I2>,
  ...
  public:
    XABRInterpolationImpl(
        ...
        : Interpolation::templateImpl<I1, I2>(xBegin, xEnd, yBegin, 1),
          XABRCoeffHolder<Model>(settings, t, forward, params, paramIsFixed, addParams),
    ...
    void update() {
        ...
        if (std::accumulate(this->paramIsFixed_.begin(),
            ...
            XABRError costFunction(env_, this);
  ...
  private:
    class XABRError : public CostFunction {
      public:
        explicit XABRError(Settings* settings, XABRInterpolationImpl *xabr) : xabr_(xabr), env_(settings) {}
        ...
        Real value(const Array &x) const {
            ...
            for (Size i = 0; i < xabr_->params_.size(); ++i)
                xabr_->params_[i] = y[i];
            xabr_->updateModelInstance(env_);
