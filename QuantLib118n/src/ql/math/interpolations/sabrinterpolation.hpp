...
class SABRWrapper {
  public:
    SABRWrapper(Settings* settings, const Time t, const Real &forward,
...
struct SABRSpecs {
    ...
    ext::shared_ptr<type> instance(Settings* settings, const Time t, const Real &forward,
                                     const std::vector<Real> &params,
                                     const std::vector<Real> &addParams) {
        return ext::make_shared<type>(settings, t, forward, params, addParams);
    }
};
}
...
class SABRInterpolation : public Interpolation {
  public:
    ...
    SABRInterpolation(Settings* settings,
                      ...
                      const Size maxGuesses = 50, const Real shift = 0.0): env_(settings) {
