...
namespace QuantLib {
    ...
    class Dividend : public CashFlow {
      public:
        Dividend(Settings* settings, const Date& date)
        :CashFlow(settings), date_(date) {}
        //! \name Event interface
        //@{
    ...
    class FixedDividend : public Dividend {
      public:
        FixedDividend(Settings* settings, Real amount, const Date& date)
        : Dividend(settings, date), amount_(amount) {}
    ...
    class FractionalDividend : public Dividend {
      public:
        FractionalDividend(Settings* settings, Real rate, const Date& date)
        : Dividend(settings, date), rate_(rate), nominal_(Null<Real>()) {}

        FractionalDividend(Settings* settings, Real rate, Real nominal, const Date& date)
        : Dividend(settings, date), rate_(rate), nominal_(nominal) {}
        //! \name Dividend interface
        //@{
    ...
    DividendVector(Settings* settings, const std::vector<Date>& dividendDates,
