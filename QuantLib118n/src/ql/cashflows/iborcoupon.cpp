...
namespace QuantLib {
    ...
    IborCoupon::IborCoupon(Settings* settings,
    ...
    IborLeg::IborLeg(Settings* settings, const Schedule& schedule,
    ...
    IborLeg::operator Leg() const {
        ...
        if (caps_.empty() && floors_.empty() && !inArrears_) {
            ext::shared_ptr<IborCouponPricer> pricer(new BlackIborCouponPricer(env_));
