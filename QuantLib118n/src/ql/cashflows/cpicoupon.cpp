...
namespace QuantLib {
    ...
    : InflationCoupon(settings, paymentDate, nominal, startDate, endDate,
    ...
    CPILeg::CPILeg(Settings* settings, const Schedule& schedule, const ext::shared_ptr<ZeroInflationIndex>& index,
    ...
    CPILeg::operator Leg() const {
            ...
            for (Size i=0; i<n; ++i) {
                ...
                if (detail::get(fixedRates_, i, 1.0) == 0.0) { // fixed coupon
                    leg.push_back(ext::shared_ptr<CashFlow>
                                  (new FixedRateCoupon
                                   (env_, paymentDate, detail::get(notionals_, i, 0.0),
                    ...
                    if (detail::noOption(caps_, floors_, i)) { // just swaplet
                        ...
                        coup = ext::shared_ptr<CPICoupon>
                            (new CPICoupon(env_, baseCPI_,    // all have same base for ratio
                                     paymentDate,
        ...
        ext::shared_ptr<CashFlow> xnl(new CPICashFlow
                          (env_, detail::get(notionals_, n, 0.0), index_,
