...
namespace QuantLib {
    ...
    namespace {
        ...
        class OvernightIndexedCouponPricer : public FloatingRateCouponPricer {
          public:
        	OvernightIndexedCouponPricer(Settings* settings):env_(settings) {
    ...
    : FloatingRateCoupon(settings, paymentDate, nominal, startDate, endDate,
        ...
        if (telescopicValueDates) {
            ...
            Date evalDate = env_->evaluationDate();
        ...
        Schedule sch =
            MakeSchedule(env_->evaluationDate())
                .from(startDate)
                // .to(endDate)
                .to(tmpEndDate)
                .withTenor(1 * Days)
        ...
        setPricer(ext::shared_ptr<FloatingRateCouponPricer>(new
                                            OvernightIndexedCouponPricer(settings)));
    }
    ...
    OvernightLeg::OvernightLeg(Settings* settings, const Schedule& schedule,
    ...
    OvernightLeg::operator Leg() const {
        ...
        for (Size i=0; i<n; ++i) {
            ...
            cashflows.push_back(ext::shared_ptr<CashFlow>(new
                OvernightIndexedCoupon(env_, paymentDate,
