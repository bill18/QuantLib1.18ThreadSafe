...
namespace QuantLib {
    ...
    Rate DigitalCoupon::callOptionRate() const {
        ...
        if(hasCallStrike_) {
            ...
            CappedFlooredCoupon next(env_, underlying_, callStrike_ + callRightEps_);
            CappedFlooredCoupon previous(env_, underlying_, callStrike_ - callLeftEps_);
            callOptionRate *= (next.rate() - previous.rate())
                            / (callLeftEps_ + callRightEps_);
            if (!isCallCashOrNothing_) {
                // Call
                CappedFlooredCoupon atStrike(env_, underlying_, callStrike_);
    ...
    Rate DigitalCoupon::putOptionRate() const {
        ...
        if(hasPutStrike_) {
            ...
            CappedFlooredCoupon next(env_, underlying_, Null<Rate>(), putStrike_ + putRightEps_);
            CappedFlooredCoupon previous(env_, underlying_, Null<Rate>(), putStrike_ - putLeftEps_);
            putOptionRate *= (next.rate() - previous.rate())
                           / (putLeftEps_ + putRightEps_);
            if (!isPutCashOrNothing_) {
                // Put
                CappedFlooredCoupon atStrike(env_, underlying_, Null<Rate>(), putStrike_);
    ...
    Rate DigitalCoupon::rate() const {
        ...
        bool enforceTodaysHistoricFixings =
            env_->enforcesTodaysHistoricFixings();
