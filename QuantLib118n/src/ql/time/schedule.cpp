...
//	  env_(settings)
    ...
    Schedule::Schedule(Date evalDate, Date effectiveDate,
...
//            Date evalDate = settings->evaluationDate();
    ...
    Schedule::lower_bound(const Date& d) const {
//        Date d = (refDate==Date() ?
//                  env_->evaluationDate() :
//                  refDate);
    ...
    MakeSchedule::MakeSchedule(const Date& evalDate)
    : rule_(DateGeneration::Backward), endOfMonth_(false), evalDate_(evalDate) {}
    ...
    MakeSchedule::operator Schedule() const {
        ...
        return Schedule(evalDate_, effectiveDate_, terminationDate_, *tenor_, calendar,
