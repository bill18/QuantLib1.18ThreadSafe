...
namespace QuantLib {
    ...
    BlackCapFloorEngine::BlackCapFloorEngine(
    		Settings* settings,
                              ...
                              const DayCounter& dc,
                              Real displacement)
    : CapFloor::engine(settings), discountCurve_(discountCurve),
      vol_(ext::shared_ptr<OptionletVolatilityStructure>(new
          ConstantOptionletVolatility(settings, 0, NullCalendar(), Following, v, dc))),
    ...
    BlackCapFloorEngine::BlackCapFloorEngine(
        ...
        Real displacement)
        : CapFloor::engine(settings), discountCurve_(discountCurve), vol_(volatility) {
        QL_REQUIRE(
