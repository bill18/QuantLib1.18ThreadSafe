...
namespace QuantLib {
    ...
    */
    ...
    class GenericModelEngine
      ...
      public:
        explicit GenericModelEngine(Settings* settings,
                         const Handle<ModelType>& model = Handle<ModelType>())
        : GenericEngine<ArgumentsType, ResultsType>(settings), model_(model) {
            this->registerWith(model_);
        }
        explicit GenericModelEngine(Settings* settings, const ext::shared_ptr<ModelType>& model)
        :  GenericEngine<ArgumentsType, ResultsType>(settings), model_(model) {
            this->registerWith(model_);
        }
      protected:
