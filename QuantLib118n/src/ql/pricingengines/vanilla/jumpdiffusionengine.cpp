...
namespace QuantLib {
    ...
    JumpDiffusionEngine::JumpDiffusionEngine(
        ...
        Size maxIterations)
    : VanillaOption::engine(settings), process_(process), relativeAccuracy_(relativeAccuracy),
      maxIterations_(maxIterations), env_(settings) {
    ...
    void JumpDiffusionEngine::calculate() const {
        ...
        ext::shared_ptr<GeneralizedBlackScholesProcess> bsProcess(
                 new GeneralizedBlackScholesProcess(env_, stateVariable, dividendTS,
                                                    riskFreeTS, volTS));

        AnalyticEuropeanEngine baseEngine(env_, bsProcess);
        ...
        for (i=0;  (lastContribution>relativeAccuracy_ && i<maxIterations_) 
            ...
            riskFreeTS.linkTo(ext::shared_ptr<YieldTermStructure>(new
                FlatForward(env_, rateRefDate, r, voldc)));
            volTS.linkTo(ext::shared_ptr<BlackVolTermStructure>(new
                BlackConstantVol(env_, rateRefDate, volcal, v, voldc)));
