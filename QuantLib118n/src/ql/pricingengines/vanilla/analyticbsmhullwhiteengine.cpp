...
namespace QuantLib {
        ...
        class ShiftedBlackVolTermStructure : public BlackVolTermStructure {
          public:
            ShiftedBlackVolTermStructure(
                ...
                : BlackVolTermStructure(settings, volTS->referenceDate(),
                                        volTS->calendar(),
                                        Following,
                                        volTS->dayCounter()),
    ...
    : GenericModelEngine<HullWhite,
                         ...
                         VanillaOption::results>(settings, model),
      rho_(equityShortRateCorrelation), process_(process), env_(settings) {
    ...
    void AnalyticBSMHullWhiteEngine::calculate() const {
        ...
        Handle<BlackVolTermStructure> volTS(
             ext::shared_ptr<BlackVolTermStructure>(
              new ShiftedBlackVolTermStructure(env_, varianceOffset,
                                               process_->blackVolatility())));

        ext::shared_ptr<GeneralizedBlackScholesProcess> adjProcess(
                new GeneralizedBlackScholesProcess(env_, process_->stateVariable(),
                                                   process_->dividendYield(),
                                                   process_->riskFreeRate(),
                                                   volTS));

        ext::shared_ptr<AnalyticEuropeanEngine> bsmEngine(
                                      new AnalyticEuropeanEngine(env_, adjProcess));

        VanillaOption(env_, payoff, exercise).setupArguments(
