...
namespace QuantLib {
  ...
  : VanillaSwap::engine(settings), baseSwapEngine_(Handle<PricingEngine>(
      ext::make_shared<DiscountingSwapEngine>(settings, discountCurve))),
    ...
    invstDTS_(invstDTS.empty() ? Handle<DefaultProbabilityTermStructure>(
        ext::make_shared<FlatHazardRate>(settings, 0, NullCalendar(), 1.e-12,
  ...
  : VanillaSwap::engine(settings), baseSwapEngine_(Handle<PricingEngine>(
    ...
    swaptionletEngine_(Handle<PricingEngine>(
      ext::make_shared<BlackSwaptionEngine>(settings, discountCurve,
        blackVol))),
    discountCurve_(discountCurve),
    defaultTS_(ctptyDTS), 
    ctptyRecoveryRate_(ctptyRecoveryRate),
  ...
  void CounterpartyAdjSwapEngine::calculate() const {
    ...
    while(nextFD != arguments_.fixedPayDates.end()) {
      ...
      Swaption swaptionlet(env_, swaplet,
        ext::make_shared<EuropeanExercise>(swapletStart));
      Swaption putSwaplet(env_, revSwaplet,
        ext::make_shared<EuropeanExercise>(swapletStart));
