...
namespace QuantLib {
    ...
    DiscountingSwapEngine::DiscountingSwapEngine(
    		Settings* settings,
                            ...
                            Date settlementDate,
                            Date npvDate)
    : Swap::engine(settings),  discountCurve_(discountCurve),
      includeSettlementDateFlows_(includeSettlementDateFlows),
      settlementDate_(settlementDate), npvDate_(npvDate), env_(settings) {
    ...
    void DiscountingSwapEngine::calculate() const {
        ...
        bool includeRefDateFlows =
            ...
            *includeSettlementDateFlows_ :
            env_->includeReferenceDateEvents();

        for (Size i=0; i<n; ++i) {
            try {
                const YieldTermStructure& discount_ref = **discountCurve_;
                CashFlows::npvbps(env_, arguments_.legs[i],
                                  discount_ref,
                                  includeRefDateFlows,
                                  settlementDate,
