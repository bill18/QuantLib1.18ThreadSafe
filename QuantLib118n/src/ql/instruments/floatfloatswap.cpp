...
namespace QuantLib {
    ...
    FloatFloatSwap::FloatFloatSwap(
        ...
        boost::optional<BusinessDayConvention> paymentConvention1,
        boost::optional<BusinessDayConvention> paymentConvention2)
        : Swap(settings, 2), type_(type), nominal1_(nominal1), nominal2_(nominal2),
          schedule1_(schedule1), schedule2_(schedule2), index1_(index1),
    ...
    void FloatFloatSwap::init(
        ...
        if (ibor1) {
            IborLeg leg(env_, schedule1_, ibor1);
            leg.withNotionals(nominal1_) // leg = ?????
        ...
        if (ibor2) {
            IborLeg leg(env_, schedule2_, ibor2);
        ...
        if (cms1) {
            CmsLeg leg(env_, schedule1_, cms1);
        ...
        if (cms2) {
            CmsLeg leg(env_, schedule2_, cms2);
            leg.withNotionals(nominal2_) // leg = ?????
                      .withPaymentDayCounter(dayCount2_)
                      .withPaymentAdjustment(paymentConvention2_)
                      .withSpreads(spread2_)
                      .withGearings(gearing2_);
            if (cappedRate2_[0] != Null<Real>())
                leg.withCaps(cappedRate2_); // leg = ?????
            if (flooredRate2_[0] != Null<Real>())
                leg.withFloors(flooredRate2_);  // leg = ?????
            legs_[1] = leg;
        }

        if (cmsspread1) {
            CmsSpreadLeg leg(env_, schedule1_, cmsspread1);
            leg.withNotionals(nominal1_) // leg = ?????
                      .withPaymentDayCounter(dayCount1_)
                      .withPaymentAdjustment(paymentConvention1_)
                      .withSpreads(spread1_)
                      .withGearings(gearing1_);
            if (cappedRate1_[0] != Null<Real>())
                leg.withCaps(cappedRate1_); // leg = ?????
            if (flooredRate1_[0] != Null<Real>())
                leg.withFloors(flooredRate1_); // leg = ?????
            legs_[0] = leg;
        }

        if (cmsspread2) {
            CmsSpreadLeg leg(env_, schedule2_, cmsspread2);
            leg.withNotionals(nominal2_) // leg = ?????
                      .withPaymentDayCounter(dayCount2_)
                      .withPaymentAdjustment(paymentConvention2_)
                      .withSpreads(spread2_)
        ...
        if (intermediateCapitalExchange_) {
                ...
                if (!close(cap, 0.0)) {
                    ...
                    legs_[0].insert(
                        it1, ext::shared_ptr<CashFlow>(
                                 new Redemption(env_, cap, legs_[0][i]->date())));
            ...
            for (Size i = 0; i < legs_[1].size() - 1; i++) {
                ...
                if (!close(cap, 0.0)) {
                    ...
                    legs_[1].insert(
                        it1, ext::shared_ptr<CashFlow>(
                                 new Redemption(env_, cap, legs_[1][i]->date())));
        ...
        if (finalCapitalExchange_) {
            legs_[0].push_back(ext::shared_ptr<CashFlow>(
                new Redemption(env_, nominal1_.back(), legs_[0].back()->date())));
            nominal1_.push_back(nominal1_.back());
            legs_[1].push_back(ext::shared_ptr<CashFlow>(
                new Redemption(env_, nominal2_.back(), legs_[1].back()->date())));
