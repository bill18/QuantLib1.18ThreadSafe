...
namespace QuantLib {
    ...
    : OneAssetOption(settings, payoff, exercise),
      cashFlow_(DividendVector(settings, dividendDates, dividends)), env_(settings) {
    ...
    Volatility DividendVanillaOption::impliedVolatility(
        ...
        ext::shared_ptr<GeneralizedBlackScholesProcess> newProcess =
            detail::ImpliedVolatilityHelper::clone(env_, process, volQuote);

        // engines are built-in for the time being
        boost::scoped_ptr<PricingEngine> engine;
        switch (exercise_->type()) {
          case Exercise::European:
            engine.reset(new AnalyticDividendEuropeanEngine(env_, newProcess));
            break;
          case Exercise::American:
            engine.reset(new FdBlackScholesVanillaEngine(env_, newProcess));
            break;
