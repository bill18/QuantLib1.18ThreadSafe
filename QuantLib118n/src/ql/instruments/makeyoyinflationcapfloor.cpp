...
namespace QuantLib {
    ...
    MakeYoYInflationCapFloor::operator ext::shared_ptr<YoYInflationCapFloor>() const {
        ...
        if (effectiveDate_ != Date()) {
            ...
            Date referenceDate = env_->evaluationDate();
        ...
        Schedule schedule(env_->evaluationDate(), startDate, endDate, Period(Annual), calendar_,
                          Unadjusted, Unadjusted, // ref periods & acc periods
                          DateGeneration::Forward, false);
        Leg leg = yoyInflationLeg(env_, schedule, calendar_, index_,
                                  observationLag_)
        ...
        if (strike_ == Null<Rate>()) {
            ...
            strikeVector[0] = CashFlows::atmRate(env_, leg,**fc,
                                                 false, fc->referenceDate());
        }

        ext::shared_ptr<YoYInflationCapFloor> capFloor(new
                    YoYInflationCapFloor(env_, capFloorType_, leg, strikeVector));
