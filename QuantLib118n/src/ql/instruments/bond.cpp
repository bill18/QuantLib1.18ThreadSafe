...
namespace QuantLib {
    ...
    Bond::Bond(Settings* settings, Natural settlementDays,
               const Calendar& calendar,
               const Date& issueDate,
               const Leg& coupons)
    : settlementDays_(settlementDays), calendar_(calendar),
      cashflows_(coupons), issueDate_(issueDate), env_(settings) {
        ...
        registerWith(env_->evaluationDate());
    ...
    : settlementDays_(settlementDays), calendar_(calendar),
      ...
      issueDate_(issueDate),env_(settings) {
    ...
    bool Bond::isExpired() const {
        ...
        return CashFlows::isExpired(env_, cashflows_,
                                    true,
                                    env_->evaluationDate());
    }
    ...
    Rate Bond::yield(const DayCounter& dc,
        ...
        return BondFunctions::yield(env_, *this, cleanPrice(), dc, comp, freq,
                                    settlementDate(),
    ...
    Real Bond::cleanPrice(Rate y,
        ...
        return BondFunctions::cleanPrice(env_, *this, y, dc, comp, freq, settlement);
    }
    ...
    Real Bond::accruedAmount(Date settlement) const {
        ...
        return BondFunctions::accruedAmount(env_, *this, settlement);
    }

    Rate Bond::nextCouponRate(Date settlement) const {
        return BondFunctions::nextCouponRate(env_, *this, settlement);
    }

    Rate Bond::previousCouponRate(Date settlement) const {
        return BondFunctions::previousCouponRate(env_, *this, settlement);
    }

    Date Bond::nextCashFlowDate(Date settlement) const {
        return BondFunctions::nextCashFlowDate(env_, *this, settlement);
    }

    Date Bond::previousCashFlowDate(Date settlement) const {
        return BondFunctions::previousCashFlowDate(env_, *this, settlement);
    }
    ...
    void Bond::addRedemptionsToCashflows(const std::vector<Real>& redemptions) {
        ...
        for (Size i=1; i<notionalSchedule_.size(); ++i) {
            ...
            if (i < notionalSchedule_.size()-1)
                payment.reset(new AmortizingPayment(env_, amount,
                                                    notionalSchedule_[i]));
            else
                payment.reset(new Redemption(env_, amount, notionalSchedule_[i]));
    ...
    void Bond::setSingleRedemption(Real notional,
        ...
        ext::shared_ptr<CashFlow> redemptionCashflow(
                         new Redemption(env_, notional*redemption/100.0, date));
