...
namespace QuantLib {
    ...
    : Forward(settings, index->dayCounter(), index->fixingCalendar(),
    ...
    Date ForwardRateAgreement::settlementDate() const {
        return calendar_.advance(env_->evaluationDate(),
    ...
    bool ForwardRateAgreement::isExpired() const {
        return detail::simple_event(valueDate_).hasOccurred(settlementDate(), env_->includeReferenceDateEvents());
    }
