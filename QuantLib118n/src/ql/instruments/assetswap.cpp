...
namespace QuantLib {
    ...
    AssetSwap::AssetSwap(Settings* settings, bool parSwap,
                         ...
                         Date dealMaturity,
                         bool payBondCoupon)
    : Swap(settings, 2), bond_(bond), bondCleanPrice_(bondCleanPrice),
      nonParRepayment_(nonParRepayment), spread_(spread), parSwap_(parSwap),
	  env_(settings)
    {
        Schedule tempSch(env_->evaluationDate(), bond_->settlementDate(),
                         bond_->maturityDate(),
                         iborIndex->tenor(),
        ...
        if (floatingDayCounter==DayCounter())
            legs_[1] = IborLeg(settings, schedule, iborIndex)
        ...
        if (i<bondLeg.end()-1) {
            ...
            if (c) {
                ext::shared_ptr<CashFlow> accruedCoupon(new
                    SimpleCashFlow(settings, c->accruedAmount(dealMaturity), finalDate));
                legs_[0].push_back(accruedCoupon);
            }
        }
        // add the nonParRepayment_
        ext::shared_ptr<CashFlow> nonParRepaymentFlow(new
            SimpleCashFlow(settings, nonParRepayment_, finalDate));
        ...
        if (parSwap_) {
            ...
            ext::shared_ptr<CashFlow> upfrontCashFlow(new
                SimpleCashFlow(settings, upfront, upfrontDate_));
            ...
            ext::shared_ptr<CashFlow> backPaymentCashFlow(new
                SimpleCashFlow(settings, backPayment, finalDate));
            legs_[1].push_back(backPaymentCashFlow);
        } else {
            // final notional exchange
            ext::shared_ptr<CashFlow> finalCashFlow (new
                SimpleCashFlow(settings, notional, finalDate));
    ...
    AssetSwap::AssetSwap(Settings* settings, bool payBondCoupon,
                         const ext::shared_ptr<Bond>& bond,
                         Real bondCleanPrice,
                         const ext::shared_ptr<IborIndex>& iborIndex,
                         Spread spread,
	  ...
	  env_(settings)
    {
        ...
        if (floatSchedule.empty())
            schedule = Schedule(env_->evaluationDate(), bond_->settlementDate(),
                                bond_->maturityDate(),
                                iborIndex->tenor(),
                                iborIndex->fixingCalendar(),
                                iborIndex->businessDayConvention(),
