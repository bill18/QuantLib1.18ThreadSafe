...
namespace QuantLib {
    ...
    Swap::Swap(Settings* settings, const Leg& firstLeg,
               const Leg& secondLeg)
    : legs_(2), payer_(2),
    ...
    Swap::Swap(Settings* settings, const std::vector<Leg>& legs,
    ...
    Swap::Swap(Settings* settings, Size legs)
    ...
    bool Swap::isExpired() const {
        for (Size j=0; j<legs_.size(); ++j) {
            ...
            for (i = legs_[j].begin(); i!= legs_[j].end(); ++i)
                if (!(*i)->hasOccurred(env_->evaluationDate()))
                    return false;
        }
        return true;
    }
