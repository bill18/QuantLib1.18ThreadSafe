...
namespace QuantLib {
    ...
    namespace {
        ...
        class ImpliedSwaptionVolHelper {
          public:
            ImpliedSwaptionVolHelper(Settings* settings, const Swaption&,
    ...
    Swaption::Swaption(Settings* settings, const ext::shared_ptr<VanillaSwap>& swap,
                       const ext::shared_ptr<Exercise>& exercise,
                       Settlement::Type delivery,
                       Settlement::Method settlementMethod)
        : Option(ext::shared_ptr<Payoff>(), exercise), swap_(swap),
          settlementType_(delivery), settlementMethod_(settlementMethod), env_(settings) {
        registerWith(swap_);
        registerWithObservables(swap_);
    }

    bool Swaption::isExpired() const {
        return detail::simple_event(exercise_->dates().back()).hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents());
    }
    ...
    Volatility Swaption::impliedVolatility(Real targetValue,
        ...
        ImpliedSwaptionVolHelper f(env_, *this, d, targetValue, displacement, type);
        //Brent solver;
        NewtonSafe solver;
