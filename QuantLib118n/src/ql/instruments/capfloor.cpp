...
namespace QuantLib {
    ...
    namespace {
        ...
        class ImpliedCapVolHelper {
          public:
            ImpliedCapVolHelper(Settings* settings,
            					const CapFloor&,
        ...
        ImpliedCapVolHelper::ImpliedCapVolHelper(
        		Settings* settings,
                              ...
                              Real displacement,
                              VolatilityType type)
        : discountCurve_(discountCurve), targetValue_(targetValue), env_(settings) {
            ...
            case ShiftedLognormal:
                engine_ = ext::shared_ptr<PricingEngine>(new
                    BlackCapFloorEngine(settings, discountCurve_, h, Actual365Fixed(),
                                                                displacement));
                break;
            case Normal:
                engine_ = ext::shared_ptr<PricingEngine>(new
                    BachelierCapFloorEngine(settings, discountCurve_, h,
                                                            Actual365Fixed()));
                break;
            default:
    ...
    CapFloor::CapFloor(Settings* settings,
    				   CapFloor::Type type,
                       const Leg& floatingLeg,
                       const std::vector<Rate>& capRates,
                       const std::vector<Rate>& floorRates)
    : env_(settings), type_(type), floatingLeg_(floatingLeg),
    ...
    bool CapFloor::isExpired() const {
        for (Size i=floatingLeg_.size(); i>0; --i)
            if (!floatingLeg_[i-1]->hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents()))
                return false;
        return true;
    }
    ...
    ext::shared_ptr<CapFloor> CapFloor::optionlet(const Size i) const {
        ...
        return ext::make_shared<CapFloor>(env_, type(), cf, cap, floor);
    }
    ...
    Rate CapFloor::atmRate(const YieldTermStructure& discountCurve) const {
        ...
        return CashFlows::atmRate(env_, floatingLeg_, discountCurve,
    ...
    Volatility CapFloor::impliedVolatility(Real targetValue,
        ...
        ImpliedCapVolHelper f(env_, *this, d, targetValue, displacement, type);
        //Brent solver;
        NewtonSafe solver;
