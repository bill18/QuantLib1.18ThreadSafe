...
namespace QuantLib {
    ...
    MakeCms::MakeCms(Settings* settings, const Period& swapTenor,
    ...
    : swapTenor_(swapTenor), swapIndex_(swapIndex),
      ...
      engine_(new DiscountingSwapEngine(settings, swapIndex->forwardingTermStructure())),
	  env_(settings) {
    ...
    MakeCms::operator ext::shared_ptr<Swap>() const {
        ...
        if (effectiveDate_ != Date())
            ...
            Date refDate = env_->evaluationDate();
        ...
        Schedule cmsSchedule(env_->evaluationDate(), startDate, terminationDate,
        ...
        Schedule floatSchedule(env_->evaluationDate(), startDate, terminationDate,
        ...
        Leg cmsLeg = CmsLeg(env_, cmsSchedule, swapIndex_)
        ...
        if (payCms_)
            swap = ext::make_shared<Swap>(env_, cmsLeg, floatLeg);
        else
            swap = ext::make_shared<Swap>(env_, floatLeg, cmsLeg);
    ...
    MakeCms& MakeCms::withDiscountingTermStructure(
        ...
        engine_ = ext::make_shared<DiscountingSwapEngine>(env_, discountingTermStructure);
        return *this;
    }
