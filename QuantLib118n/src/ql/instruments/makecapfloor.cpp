...
namespace QuantLib {
    ...
    MakeCapFloor::MakeCapFloor(Settings* settings, CapFloor::Type capFloorType,
                               const Period& tenor,
    ...
    : capFloorType_(capFloorType), strike_(strike),
      ...
      makeVanillaSwap_(MakeVanillaSwap(settings, tenor, iborIndex, 0.0, forwardStart)
    ...
    MakeCapFloor::operator ext::shared_ptr<CapFloor>() const {
        ...
        if (strike_ == Null<Rate>()) {
            ...
            strikeVector[0] = CashFlows::atmRate(env_, leg,
                                                 **discountCurve,
                                                 false,
        ...
        ext::shared_ptr<CapFloor> capFloor(new
            CapFloor(env_, capFloorType_, leg, strikeVector));
