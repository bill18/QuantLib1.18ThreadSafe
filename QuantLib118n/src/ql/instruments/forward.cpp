...
namespace QuantLib {
    ...
    Forward::Forward(Settings* settings, const DayCounter& dayCounter,
    ...
    Date Forward::settlementDate() const {
        Date d = calendar_.advance(env_->evaluationDate(),
    ...
    bool Forward::isExpired() const {
        return detail::simple_event(maturityDate_)
               .hasOccurred(settlementDate(), env_->includeReferenceDateEvents());
    }
