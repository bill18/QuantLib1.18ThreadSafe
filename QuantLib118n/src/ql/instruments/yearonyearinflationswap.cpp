...
namespace QuantLib {
    ...
    YearOnYearInflationSwap(Settings* settings,
                            Type type,
                            Real nominal,
    ...
    : Swap(settings, 2), type_(type), nominal_(nominal),
    ...
    paymentConvention_(paymentConvention)
    {
        ...
        Leg fixedLeg = FixedRateLeg(settings, fixedSchedule_)
        .withNotionals(nominal_)
        .withCouponRates(fixedRate_, fixedDayCount_) // Simple compounding by default
        .withPaymentAdjustment(paymentConvention_);

        Leg yoyLeg = yoyInflationLeg(settings, yoySchedule_, paymentCalendar_, yoyIndex_, observationLag_)
        .withNotionals(nominal_)
