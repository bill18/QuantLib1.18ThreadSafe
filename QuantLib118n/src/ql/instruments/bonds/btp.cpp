...
namespace QuantLib {
    ...
    CCTEU::CCTEU(Settings* settings, const Date& maturityDate,
                 Spread spread,
                 const Handle<YieldTermStructure>& fwdCurve,
                 const Date& startDate,
                 const Date& issueDate)
    : FloatingRateBond(settings, 2, 100.0,
                       Schedule(env_->evaluationDate(), startDate,
                                maturityDate, 6*Months,
                                NullCalendar(), Unadjusted, Unadjusted,
                                DateGeneration::Backward, true),
                       ext::make_shared<Euribor6M>(settings, fwdCurve),
                       Actual360(),
                       Following,
                       Euribor6M(settings).fixingDays(),
    ...
    BTP::BTP(Settings* settings, const Date& maturityDate,
             Rate fixedRate,
             const Date& startDate,
             const Date& issueDate)
    : FixedRateBond(settings, 2, 100.0,
                    Schedule(env_->evaluationDate(), startDate,
                             maturityDate, 6*Months,
                             NullCalendar(), Unadjusted, Unadjusted,
                             DateGeneration::Backward, true),
    ...
    void RendistatoCalculator::performCalculations() const {
        ...
        FixedRateBond swapBond(env_, settlDays,
                               100.0,      // faceAmount
                               ...
                               100.0);    // redemption
        swapBondYields_[0] = BondFunctions::yield(env_, swapBond,
            100.0, // floating leg NPV including end payment
            ActualActual(ActualActual::ISMA), Compounded, Annual,
            bondSettlementDate,
            // accuracy, maxIterations, guess
            1.0e-10, 100, swapBondYields_[0]);
        swapBondDurations_[0] = BondFunctions::duration(env_,
        ...
        for (Size i=1; i<nSwaps_; ++i) {
            ...
            FixedRateBond swapBond(env_, settlDays,
                                   ...
                                   Following, // paymentConvention
                                   100.0);    // redemption
            swapBondYields_[i] = BondFunctions::yield(env_, swapBond,
                100.0, // floating leg NPV including end payment
                ActualActual(ActualActual::ISMA), Compounded, Annual,
                bondSettlementDate,
                // accuracy, maxIterations, guess
