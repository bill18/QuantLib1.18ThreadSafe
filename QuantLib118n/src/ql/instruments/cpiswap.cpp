...
namespace QuantLib {
    ...
    CPISwap::
    CPISwap(Settings* settings, Type type,
            Real nominal,
    ...
    : Swap(settings, 2), type_(type), nominal_(nominal), subtractInflationNominal_(subtractInflationNominal),
	...
	env_(settings) {
        ...
        if (floatSchedule_.size() > 1) {
            floatingLeg = IborLeg(settings, floatSchedule_, floatIndex_)
            ...
            )
        {
            ...
            ext::shared_ptr<CashFlow> nf(new SimpleCashFlow(settings, floatAmount, payNotional));
            floatingLeg.push_back(nf);
        }

        // a CPIleg know about zero legs and inclusion of base inflation notional
        Leg cpiLeg = CPILeg(env_, fixedSchedule_, fixedIndex_,
