...
namespace QuantLib {
    ...
    MakeCreditDefaultSwap::MakeCreditDefaultSwap(Settings* settings, const Period &tenor,
        ...
        : side_(Protection::Buyer), nominal_(1.0), tenor_(tenor),
          ...
          dayCounter_(Actual360()), lastPeriodDayCounter_(Actual360(true)), env_(settings) {}
    MakeCreditDefaultSwap::MakeCreditDefaultSwap(Settings* settings, const Date &termDate,
                                                 const Real couponRate)
    ...
    operator ext::shared_ptr<CreditDefaultSwap>() const {
        ...
        Date evaluation = env_->evaluationDate();
        ...
        Schedule schedule(env_->evaluationDate(), start, end, couponTenor_, WeekendsOnly(), Following,
                          Unadjusted, DateGeneration::CDS, false, Date(),
                          Date());

        ext::shared_ptr<CreditDefaultSwap> cds =
            ext::shared_ptr<CreditDefaultSwap>(new CreditDefaultSwap(env_,
