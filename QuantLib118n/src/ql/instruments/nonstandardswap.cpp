...
namespace QuantLib {
    ...
    NonstandardSwap::NonstandardSwap(Settings* settings, const VanillaSwap &fromVanilla)
        : Swap(settings, 2), type_((VanillaSwap::Type)fromVanilla.type()),
    ...
    NonstandardSwap::NonstandardSwap(
        ...
        : Swap(settings, 2), type_(type), fixedNominal_(fixedNominal),
    ...
    void NonstandardSwap::init() {
        ...
        legs_[0] = FixedRateLeg(env_, fixedSchedule_)
                       .withNotionals(fixedNominal_)
                       .withCouponRates(fixedRate_, fixedDayCount_)
                       .withPaymentAdjustment(paymentConvention_);

        legs_[1] = IborLeg(env_, floatingSchedule_, iborIndex_)
        ...
        if (finalCapitalExchange_) {
            legs_[0].push_back(ext::shared_ptr<CashFlow>(
                new Redemption(env_, fixedNominal_.back(), legs_[0].back()->date())));
            fixedNominal_.push_back(fixedNominal_.back());
            fixedRate_.push_back(0.0);
            legs_[1].push_back(ext::shared_ptr<CashFlow>(new Redemption(env_,
