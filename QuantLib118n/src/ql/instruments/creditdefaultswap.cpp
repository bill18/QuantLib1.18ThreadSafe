...
namespace QuantLib {
    ...
    CreditDefaultSwap::CreditDefaultSwap(Settings* settings, Protection::Side side,
                                         Real notional,
                                         Rate spread,
    ...
    : side_(side), notional_(notional), upfront_(boost::none),
      ...
      protectionStart_(protectionStart == Null<Date>() ? schedule[0] :
        ...
        leg_ = FixedRateLeg(settings, schedule)
        ...
        upfrontPayment_ = ext::make_shared<SimpleCashFlow>(env_, 0.0, effectiveUpfrontDate);
        if (!claim_)
    ...
    : side_(side), notional_(notional), upfront_(upfront),
      ...
      protectionStart_(protectionStart == Null<Date>() ? schedule[0] :
        ...
        upfrontPayment_ = ext::make_shared<SimpleCashFlow>(env_, notional*upfront,
    ...
    bool CreditDefaultSwap::isExpired() const {
        for (Leg::const_reverse_iterator i = leg_.rbegin();
                                         i != leg_.rend(); ++i) {
            if (!(*i)->hasOccurred(env_->evaluationDate(), env_->includeReferenceDateEvents()))
                return false;
        }
        return true;
    }
    ...
    Rate CreditDefaultSwap::impliedHazardRate(
        ...
        Handle<DefaultProbabilityTermStructure> probability =
            Handle<DefaultProbabilityTermStructure>(
                ext::make_shared<FlatHazardRate>(env_, 0, WeekendsOnly(),
