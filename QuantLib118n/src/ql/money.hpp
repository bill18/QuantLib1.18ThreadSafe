    ...
    */
    class Money {
      public:
        ...
        //@{
    	Money() {}
        Money(const Date& evalDate);
        Money(const Date& evalDate, const Currency& currency, Decimal value);
        Money(const Date& evalDate, Decimal value, const Currency& currency);
    ...
    inline Money::Money(const Date& evalDate)
    : value_(0.0), evalDate_(evalDate) {}

    inline Money::Money(const Date& evalDate, const Currency& currency, Decimal value)
    : value_(value), currency_(currency), evalDate_(evalDate) {}

    inline Money::Money(const Date& evalDate, Decimal value, const Currency& currency)
    : value_(value), currency_(currency), evalDate_(evalDate) {}
    ...
    inline Money Money::rounded() const {
        return Money(evalDate_, currency_.rounding()(value_), currency_);
    }
    ...
    inline Money Money::operator-() const {
        return Money(evalDate_, -value_, currency_);
    }
