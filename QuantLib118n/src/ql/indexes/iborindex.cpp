...
namespace QuantLib {
    ...
    IborIndex::IborIndex(Settings* settings, const std::string& familyName,
                         const Period& tenor,
    ...
    : InterestRateIndex(settings, familyName, tenor, settlementDays, currency,
    ...
    ext::shared_ptr<IborIndex> IborIndex::clone(
        ...
        return ext::make_shared<IborIndex>(env_,
                                        familyName(),
                                                      tenor(),
                                                      fixingDays(),
                                                      currency(),
    ...
    OvernightIndex::OvernightIndex(Settings* settings, const std::string& familyName,
                                   Natural settlementDays,
                                   const Currency& curr,
                                   const Calendar& fixCal,
                                   const DayCounter& dc,
                                   const Handle<YieldTermStructure>& h)
   : IborIndex(settings, familyName, 1*Days, settlementDays, curr,
               fixCal, Following, false, dc, h) {}

    ext::shared_ptr<IborIndex> OvernightIndex::clone(
                               const Handle<YieldTermStructure>& h) const {
        return ext::shared_ptr<IborIndex>(
                                        new OvernightIndex(env_, familyName(),
                                                           fixingDays(),
                                                           currency(),
                                                           fixingCalendar(),
                                                           dayCounter(),
