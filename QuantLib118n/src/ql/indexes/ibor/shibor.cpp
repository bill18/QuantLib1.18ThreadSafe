...
namespace QuantLib {
    ...
    Shibor::Shibor(Settings* settings, const Period& tenor,
                   const Handle<YieldTermStructure>& h)
    : IborIndex(settings, "Shibor", tenor, (tenor == 1*Days? 0 : 1), CNYCurrency(),
                China(China::IB), shiborConvention(tenor), false,
                Actual360(), h) {}
    
    ext::shared_ptr<IborIndex> Shibor::clone(
                                  const Handle<YieldTermStructure>& h) const {
        return ext::shared_ptr<IborIndex>(new Shibor(env_, tenor(), h));
    }
}
