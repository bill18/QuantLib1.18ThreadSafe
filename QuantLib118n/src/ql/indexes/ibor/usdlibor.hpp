...
namespace QuantLib {
    ...
    */
    class USDLibor : public Libor {
      public:
        USDLibor(Settings* settings, const Period& tenor,
                 const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : Libor(settings, "USDLibor", tenor,
                2,
                USDCurrency(),
    ...
    class DailyTenorUSDLibor : public DailyTenorLibor {
      public:
        DailyTenorUSDLibor(Settings* settings, Natural settlementDays,
                           const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : DailyTenorLibor(settings, "USDLibor", settlementDays,
                          USDCurrency(),
                          UnitedStates(UnitedStates::LiborImpact),
                          Actual360(), h) {}
    };
    ...
    class USDLiborON : public DailyTenorUSDLibor {
      public:
        explicit USDLiborON(Settings* settings, const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : DailyTenorUSDLibor(settings, 0, h) {}
    };
}
