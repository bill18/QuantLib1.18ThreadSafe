...
namespace QuantLib {
    ...
    */
    class GBPLibor : public Libor {
      public:
        GBPLibor(Settings* settings, const Period& tenor,
                 const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : Libor(settings, "GBPLibor", tenor,
                0,
                GBPCurrency(),
    ...
    class DailyTenorGBPLibor : public DailyTenorLibor {
      public:
        DailyTenorGBPLibor(Settings* settings, Natural settlementDays,
                           const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : DailyTenorLibor(settings, "GBPLibor", settlementDays,
                          GBPCurrency(),
                          UnitedKingdom(UnitedKingdom::Exchange),
                          Actual365Fixed(), h) {}
    };
    ...
    class GBPLiborON : public DailyTenorGBPLibor {
      public:
        explicit GBPLiborON(Settings* settings, const Handle<YieldTermStructure>& h =
                                    Handle<YieldTermStructure>())
        : DailyTenorGBPLibor(settings, 0, h) {}
    };
