...
namespace QuantLib {
    ...
    SwapIndex::SwapIndex(Settings* settings, const std::string& familyName,
                         const Period& tenor,
    ...
    : InterestRateIndex(settings, familyName, tenor, settlementDays,
    ...
    SwapIndex::underlyingSwap(const Date& fixingDate) const {
        ...
        if (lastFixingDate_!=fixingDate) {
            ...
            if (exogenousDiscount_)
                lastSwap_ = MakeVanillaSwap(env_, tenor_, iborIndex_, fixedRate)
    ...
    : SwapIndex(settings, familyName, tenor, settlementDays,
    ...
    OvernightIndexedSwapIndex::underlyingSwap(const Date& fixingDate) const {
        ...
        if (lastFixingDate_!=fixingDate) {
            Rate fixedRate = 0.0;
            lastSwap_ = MakeOIS(env_, tenor_, overnightIndex_, fixedRate)
                .withEffectiveDate(valueDate(fixingDate))
                .withFixedLegDayCount(dayCounter_)
