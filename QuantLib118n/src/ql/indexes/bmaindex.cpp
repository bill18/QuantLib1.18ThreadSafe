...
namespace QuantLib {
    ...
    BMAIndex::BMAIndex(Settings* settings, const Handle<YieldTermStructure>& h,
                       const Calendar& fixingCalendar)
    : InterestRateIndex(settings, "BMA",
                        1 * Weeks,
                        1,
                        USDCurrency(),
                        fixingCalendar,
    ...
    Schedule BMAIndex::fixingSchedule(const Date& start, const Date& end) {
        return MakeSchedule(env_->evaluationDate()).from(previousWednesday(start))
