...
{
public:
    ReplicationError(Settings* settings,
    				 Option::Type type,
                     Time maturity,
                     Real strike,
                     Real s0,
...
class ReplicationPathPricer : public PathPricer<Path> {
  public:
    // real constructor
    ReplicationPathPricer(Settings* settings, Option::Type type,
                          Real strike,
                          Rate r,
                          Time maturity,
                          Volatility sigma)
    ...
    */
    ...
    Handle<YieldTermStructure> riskFreeRate(
                          ext::shared_ptr<YieldTermStructure>(
                                      new FlatForward(settings, today, r_, dayCount)));
    Handle<YieldTermStructure> dividendYield(
                          ext::shared_ptr<YieldTermStructure>(
                                      new FlatForward(settings, today, 0.0, dayCount)));
    Handle<BlackVolTermStructure> volatility(
                          ext::shared_ptr<BlackVolTermStructure>(
                               new BlackConstantVol(settings, today, calendar, sigma_, dayCount)));
    ext::shared_ptr<StochasticProcess1D> diffusion(
                   new BlackScholesMertonProcess(settings, stateVariable, dividendYield,
    ...
    ext::shared_ptr<PathPricer<Path> > myPathPricer(new
        ReplicationPathPricer(settings, payoff_.optionType(), payoff_.strike(),
                              r_, maturity_, sigma_));
...
int main(int, char* []) {
    ...
    try {
        ...
        settings->evaluationDate() = Date::todaysDate();
        ReplicationError rp(settings, Option::Call, maturity, strike, underlying,
