...
int main(int, char* []) {
    ...
    try {
        ...
        ext::shared_ptr<Basket> theBskt = ext::make_shared<Basket>(settings,
        ...
        ext::shared_ptr<DefaultLossModel> rdlmG(
            ext::make_shared<RandomDefaultLM<GaussianCopulaPolicy> >(settings, lmG,
        ...
        ext::shared_ptr<DefaultLossModel> rdlmT(
            ext::make_shared<RandomDefaultLM<TCopulaPolicy> >(settings, lmT,
            std::vector<Real>(), numSimulations, 1.e-6, 2863311530UL));
        ...
        */
        ...
        Date correlDate = TARGET().advance(
            settings->evaluationDate(), Period(12, Months));
