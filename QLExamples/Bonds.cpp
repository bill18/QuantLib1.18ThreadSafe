...
int main(int, char* []) {
    ...
    try {
        ...
        // ZC rates for the short end
         ...
         ext::shared_ptr<RateHelper> zc3m(new DepositRateHelper(settings,
                 Handle<Quote>(zc3mRate),
                 3*Months, fixingDays,
                 calendar, ModifiedFollowing,
                 true, zcBondsDayCounter));
         ext::shared_ptr<RateHelper> zc6m(new DepositRateHelper(settings,
                 Handle<Quote>(zc6mRate),
                 6*Months, fixingDays,
                 calendar, ModifiedFollowing,
                 true, zcBondsDayCounter));
         ext::shared_ptr<RateHelper> zc1y(new DepositRateHelper(settings,
        ...
        for (Size i=0; i<numberOfBonds; i++) {
            ...
            Schedule schedule(settings->evaluationDate(), issueDates[i], maturities[i], Period(Semiannual), UnitedStates(UnitedStates::GovernmentBond),
         ...
         *********************/
         ...
         ext::shared_ptr<RateHelper> s2y(new SwapRateHelper(settings,
         ...
         ext::shared_ptr<PricingEngine> bondEngine(
                 new DiscountingBondEngine(settings, discountingTermStructure));
         ...
         Schedule fixedBondSchedule(settings->evaluationDate(), Date(15, May, 2007),
         ...
         const ext::shared_ptr<IborIndex> libor3m(
                 new USDLibor(settings, Period(3,Months),liborTermStructure));
         libor3m->addFixing(Date(17, July, 2008),0.0278625);

         Schedule floatingBondSchedule(settings->evaluationDate(), Date(21, October, 2005),
         ...
         // Coupon pricers
         ext::shared_ptr<IborCouponPricer> pricer(new BlackIborCouponPricer(settings));
