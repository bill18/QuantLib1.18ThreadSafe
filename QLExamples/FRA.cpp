...
int main(int, char* []) {
    ...
    try {
        ...
        ext::shared_ptr<IborIndex> euribor3m(
                                       new Euribor3M(settings, euriborTermStructure));
        ...
        ext::shared_ptr<RateHelper> fra1x4(
                           new FraRateHelper(settings, h1x4, 1, 4,
                                             fixingDays, calendar, convention,
                                             endOfMonth, fraDayCounter));

        ext::shared_ptr<RateHelper> fra2x5(
                           new FraRateHelper(settings, h2x5, 2, 5,
                                             fixingDays, calendar, convention,
                                             endOfMonth, fraDayCounter));

        ext::shared_ptr<RateHelper> fra3x6(
                           new FraRateHelper(settings, h3x6, 3, 6,
                                             fixingDays, calendar, convention,
                                             endOfMonth, fraDayCounter));

        ext::shared_ptr<RateHelper> fra6x9(
                           new FraRateHelper(settings, h6x9, 6, 9,
                                             fixingDays, calendar, convention,
                                             endOfMonth, fraDayCounter));

        ext::shared_ptr<RateHelper> fra9x12(
                           new FraRateHelper(settings, h9x12, 9, 12,
                                             fixingDays, calendar, convention,
                                             endOfMonth, fraDayCounter));
        ...
        ext::shared_ptr<YieldTermStructure> fraTermStructure(
                     new PiecewiseYieldCurve<Discount,LogLinear>(settings,
        ...
        for (i=0; i<LENGTH(monthsToStart); i++) {
            ...
            ForwardRateAgreement myFRA(settings, fraValueDate, fraMaturityDate,
