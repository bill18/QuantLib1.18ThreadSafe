...
int main(int, char* []) {
    ...
    try {
        ...
        ext::shared_ptr<IborIndex>  yieldIndx(new Euribor3M(settings));
        ...
        ext::shared_ptr<PricingEngine> riskFreeEngine(
            ext::make_shared<DiscountingSwapEngine>(settings,
        ...
        ext::shared_ptr<IborIndex> yieldIndxS(
             new Euribor3M(settings, Handle<YieldTermStructure>(swapTS)));
        std::vector<VanillaSwap> riskySwaps;
        for(Size i=0; i<sizeof(tenorsSwapMkt)/sizeof(Size); i++) 
            riskySwaps.push_back(MakeVanillaSwap(settings, tenorsSwapMkt[i]*Years,
                yieldIndxS,
                ratesSwapmkt[i], 
                0*Days)
