...
int main(int, char* []) {
    ...
    try {
        ...
        for (Size j=0; j<LENGTH(lengths); j++) {
            ...
            Schedule schedule(settings->evaluationDate(), bondSettlementDate, maturity, Period(frequency),
                              calendar, accrualConvention, accrualConvention,
                              DateGeneration::Backward, false);

            ext::shared_ptr<BondHelper> helperA(
                     new FixedRateBondHelper(settings, quoteHandle[j],
                                             bondSettlementDays,
                                             100.0,
                                             schedule,
        ...
        ext::shared_ptr<YieldTermStructure> ts0 (
              new PiecewiseYieldCurve<Discount,LogLinear>(settings, curveSettlementDays,
                                                          calendar,
                                                          instrumentsB,
                                                          dc));

        ExponentialSplinesFitting exponentialSplines(settings, constrainAtZero);

        ext::shared_ptr<FittedBondDiscountCurve> ts1 (
                  new FittedBondDiscountCurve(settings, curveSettlementDays,
                                              calendar,
                                              instrumentsA,
                                              dc,
                                              exponentialSplines,
        ...
        SimplePolynomialFitting simplePolynomial(settings, 3, constrainAtZero);
        ...
        NelsonSiegelFitting nelsonSiegel(settings);
        ...
        CubicBSplinesFitting cubicBSplines(settings, knotVector, constrainAtZero);
        ...
        SvenssonFitting svensson(settings);
        ...
        SpreadFittingMethod nelsonSiegelSpread(settings,
                                    ext::make_shared<NelsonSiegelFitting>(settings),
                                    discountCurve);
        ...
        for (Size k=0; k<LENGTH(lengths)-1; k++) {
            ...
            Rate ytm = BondFunctions::yield(settings, b, P,
                                            dc, Compounded, frequency,
                                            today);
            Time dur = BondFunctions::duration(settings, b, ytm,
                                               dc, Compounded, frequency,
                                               Duration::Modified,
                                               today);
