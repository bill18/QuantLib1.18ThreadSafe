...
int main(int, char* []) {
    ...
    try {
        ...
        Handle<YieldTermStructure> rhTermStructure(
            ext::make_shared<FlatForward>(settings,
        ...
        ext::shared_ptr<IborIndex> indexSixMonths(new
            Euribor6M(settings, rhTermStructure));

        Date startDate = calendar.advance(settlementDate,1,Years,
                                          floatingLegConvention);
        Date maturity = calendar.advance(startDate,5,Years,
                                         floatingLegConvention);
        Schedule fixedSchedule(settings->evaluationDate(), startDate,maturity,Period(fixedLegFrequency),
                               calendar,fixedLegConvention,fixedLegConvention,
                               DateGeneration::Forward,false);
        Schedule floatSchedule(settings->evaluationDate(), startDate,maturity,Period(floatingLegFrequency),
        ...
        swap->setPricingEngine(ext::shared_ptr<PricingEngine>(
                                 new DiscountingSwapEngine(settings, rhTermStructure)));
        ...
        for (i=0; i<numRows; i++) {
            ...
            swaptions.push_back(ext::shared_ptr<BlackCalibrationHelper>(new
                SwaptionHelper(settings, swaptionMaturities[i],
        ...
        for (i=0; i<swaptions.size(); i++)
            swaptions[i]->setPricingEngine(ext::shared_ptr<PricingEngine>(
                new G2SwaptionEngine(settings, modelG2, 6.0, 16)));
        ...
        for (i=0; i<swaptions.size(); i++)
            swaptions[i]->setPricingEngine(ext::shared_ptr<PricingEngine>(
                new JamshidianSwaptionEngine(settings, modelHW)));
        ...
        for (i=0; i<swaptions.size(); i++)
            swaptions[i]->setPricingEngine(ext::shared_ptr<PricingEngine>(
                                     new TreeSwaptionEngine(settings, modelHW2, grid)));
        ...
        for (i=0; i<swaptions.size(); i++)
            swaptions[i]->setPricingEngine(ext::shared_ptr<PricingEngine>(
                                      new TreeSwaptionEngine(settings, modelBK, grid)));
        ...
        Swaption bermudanSwaption(settings, atmSwap, bermudanExercise);
        ...
        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new TreeSwaptionEngine(settings, modelG2, 50)));
        std::cout << "G2 (tree):      " << bermudanSwaption.NPV() << std::endl;
        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdG2SwaptionEngine(settings, modelG2)));
        std::cout << "G2 (fdm) :      " << bermudanSwaption.NPV() << std::endl;

        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new TreeSwaptionEngine(settings, modelHW, 50)));
        std::cout << "HW (tree):      " << bermudanSwaption.NPV() << std::endl;
        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdHullWhiteSwaptionEngine(settings, modelHW)));
        std::cout << "HW (fdm) :      " << bermudanSwaption.NPV() << std::endl;

        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new TreeSwaptionEngine(settings, modelHW2, 50)));
        std::cout << "HW (num, tree): " << bermudanSwaption.NPV() << std::endl;
        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new FdHullWhiteSwaptionEngine(settings, modelHW2)));
        std::cout << "HW (num, fdm) : " << bermudanSwaption.NPV() << std::endl;

        bermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new TreeSwaptionEngine(settings, modelBK, 50)));
        ...
        Swaption otmBermudanSwaption(settings, otmSwap, bermudanExercise);

        // Do the pricing for each model
        otmBermudanSwaption.setPricingEngine(ext::shared_ptr<PricingEngine>(
            new TreeSwaptionEngine(settings, modelG2, 300)));
        ...
        Swaption itmBermudanSwaption(settings, itmSwap,bermudanExercise);
